...
class SolrClient:
    ...
    def __init__(self, url, collections, recreate_collections):
        ...
        if recreate_collections:
            recreate_collections = list(
                map(lambda x: x.strip(), recreate_collections.split(","))
            )
        ...
        for c in collections:
            force = (
                True if recreate_collections and c in recreate_collections else False
            )
            c = c.split("_")
            cd = {
    ...
    def zip_configset(self, collection_dict):
        try:
            base_path = (
                Path(collection_dict["CONFIGSET_PATH"])
                .expanduser()
                .resolve(strict=True)
            )
    ...
    def maybe_upload_configset(self, collection_dict):
        ...
        if not collection_dict["CONFIGSET_NAME"] in configsets or force:
            ...
            files = {
                "file": (
                    f"{collection_dict['CONFIGSET_NAME']}.zip",
                    data,
            ...
            print(
                "O %s já presente no servidor, NÃO enviando."
                % collection_dict["CONFIGSET_NAME"]
            )

    def create_collection(
        self, collection_dict, shards=1, replication_factor=1, max_shards_per_node=1
    ):
        ...
        if res.ok:
            print(
                "Collection '%s' created succesfully"
                % collection_dict["COLLECTION_NAME"]
            )
        else:
            print("Error creating collection '%s'" % collection_dict["COLLECTION_NAME"])
            try:
                as_json = res.json()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs=1,
        required=True,
        help="Collections Solr a serem criadas",
    )
    ...
    client = SolrClient(
        url=url, collections=collections, recreate_collections=recreate_collections
    )
    for collection in client.COLLECTIONS:

        if not client.exists_collection(collection):
            print(
                "Collection '%s' doesn't exists. Creating a new one..."
