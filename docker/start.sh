...
create_env() {
    ...
    if [ -f "/var/cmjatai/cmj/data/secret.key" ]; then
        KEY=`cat /var/cmjatai/cmj/data/secret.key`
    else
        KEY=`python3 genkey.py`
        echo $KEY > data/secret.key
    fi

    FILENAME="/var/cmjatai/cmj/cmj/.env"
    ...
    echo "SOLR_COLLECTION = ""${SOLR_COLLECTION-cmj}" >> $FILENAME
...
SOLR_COLLECTION="${SOLR_COLLECTION:=cmj}"
...
mkdir -p /var/cmjatai/cmj/media/logs
celery -A cmj multi start worker1 --logfile="logs/celery/%n%I.log" --pidfile="run/celery/%n.pid"

#/bin/sh /opt/libreoffice/libreoffice_entrypoint.sh
