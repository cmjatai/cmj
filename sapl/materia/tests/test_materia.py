...
from sapl.utils import lista_anexados, models_with_gr_for_model


@pytest.mark.django_db(transaction=False)
def test_lista_materias_anexadas():
    tipo_materia = mommy.make(TipoMateriaLegislativa, descricao="Tipo_Teste")
    regime_tramitacao = mommy.make(RegimeTramitacao, descricao="Regime_Teste")
...
def make_norma():
    ...
    tipo = mommy.make(TipoNormaJuridica, sigla="T1", descricao="Teste_Tipo_Norma")
    mommy.make(
        NormaJuridica,
        tipo=tipo,
        numero=1,
...
def make_materia_principal():
    ...
    tipo = mommy.make(
        TipoMateriaLegislativa, sigla="T1", descricao="Teste_MateriaLegislativa"
    )
    mommy.make(
        MateriaLegislativa,
        tipo=tipo,
...
def test_materia_anexada_submit(admin_client):
    ...
    tipo_anexada = mommy.make(TipoMateriaLegislativa, sigla="T2", descricao="Teste_2")
    regime_tramitacao = mommy.make(RegimeTramitacao, descricao="Teste_Regime")
    mommy.make(
        MateriaLegislativa,
        tipo=tipo_anexada,
    ...
    response = admin_client.post(
        reverse("sapl.materia:anexada_create", kwargs={"pk": materia_principal.pk}),
        {
...
def test_autoria_submit(admin_client):
    ...
    # Testa POST
    response = admin_client.post(
        reverse("sapl.materia:autoria_create", kwargs={"pk": materia_principal.pk}),
        {
            "autor": autor.pk,
...
def test_despacho_inicial_submit(admin_client):
    ...
    response = admin_client.post(
        reverse(
            "sapl.materia:despachoinicial_create", kwargs={"pk": materia_principal.pk}
        ),
        {"comissao": comissao.pk, "salvar": "salvar"},
        follow=True,
    )
    assert response.status_code == 200
...
def test_numeracao_submit(admin_client):
    ...
    # Testa POST
    response = admin_client.post(
        reverse("sapl.materia:numeracao_create", kwargs={"pk": materia_principal.pk}),
        {
...
def test_documento_acessorio_submit(admin_client):
    ...
    tipo = mommy.make(TipoDocumento, descricao="Teste")
...
def test_legislacao_citada_submit(admin_client):
    ...
    response = admin_client.post(
        reverse(
            "sapl.materia:legislacaocitada_create", kwargs={"pk": materia_principal.pk}
        ),
        {
            "tipo": norma.tipo.pk,
            "numero": norma.numero,
...
def test_tramitacao_submit(admin_client):
    ...
    status_tramitacao = mommy.make(
        StatusTramitacao, indicador="F", sigla="ST", descricao="Status_Teste"
    )
    # Testa POST
    response = admin_client.post(
...
def test_form_errors_anexada(admin_client):
    ...
    assert response.context_data["form"].errors["ano"] == ["Este campo é obrigatório."]
...
def test_form_errors_tramitacao(admin_client):
    ...
    assert response.context_data["form"].errors["unidade_tramitacao_local"] == [
        "Este campo é obrigatório."
    ]
    assert response.context_data["form"].errors["status"] == [
        "Este campo é obrigatório."
    ]
    assert response.context_data["form"].errors["unidade_tramitacao_destino"] == [
        "Este campo é obrigatório."
    ]
...
def test_form_errors_relatoria(admin_client):
    ...
    response = admin_client.post(
        reverse("sapl.materia:relatoria_create", kwargs={"pk": materia_principal.pk}),
        {"salvar": "salvar"},
        follow=True,
    )
...
def test_numeracao_materia_legislativa_por_ano(admin_client):
    ...
    materia = mommy.make(MateriaLegislativa, tipo=tipo_materia, ano=2017, numero=1)
...
def test_tramitacoes_materias_anexadas(admin_client):
    ...
    tipo_materia = mommy.make(TipoMateriaLegislativa, descricao="Tipo_Teste")
    materia_principal = mommy.make(
        MateriaLegislativa, ano=2018, data_apresentacao="2018-01-04", tipo=tipo_materia
    )
    materia_anexada = mommy.make(
        MateriaLegislativa, ano=2019, data_apresentacao="2019-05-04", tipo=tipo_materia
    )
    materia_anexada_anexada = mommy.make(
        MateriaLegislativa, ano=2020, data_apresentacao="2020-01-05", tipo=tipo_materia
    )
    ...
    tramitacao_anexada = materia_anexada.tramitacao_set.first()
    tramitacao_anexada_anexada = materia_anexada_anexada.tramitacao_set.first()

    # Verifica se foram criadas as tramitações para as matérias anexadas e anexadas às anexadas
    assert materia_principal.tramitacao_set.first() == tramitacao_principal
    ...
    assert MateriaLegislativa.objects.get(
        id=materia_anexada_anexada.pk
    ).em_tramitacao == (tramitacao_anexada_anexada.status.indicador != "F")
    ...
    url = reverse(
        "sapl.materia:tramitacao_delete", kwargs={"pk": tramitacao_principal.pk}
    )
    ...
    url = reverse(
        "sapl.materia:tramitacao_delete", kwargs={"pk": tramitacao_anexada.pk}
    )
    response = admin_client.post(url, {"confirmar": "confirmar"}, follow=True)
    assert Tramitacao.objects.filter(id=tramitacao_anexada.pk).count() == 0
    assert Tramitacao.objects.filter(id=tramitacao_anexada_anexada.pk).count() == 0
    ...
    tramitacao_anexada = form.save()
    tramitacao_principal = materia_principal.tramitacao_set.first()
    tramitacao_anexada_anexada = materia_anexada_anexada.tramitacao_set.first()

    assert materia_anexada.tramitacao_set.first() == tramitacao_anexada
