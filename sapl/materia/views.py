...
from sapl.base.tasks import (
    task_analise_similaridade_entre_materia_via_classificacao,
...
from sapl.crispy_layout_mixin import SaplFormHelper, SaplFormLayout, form_actions
...
from sapl.materia.forms import (
    ...
    LegislacaoCitadaForm,
    MateriaPesquisaSimplesForm,
    OrgaoForm,
    ProposicaoForm,
    TipoProposicaoForm,
    TramitacaoForm,
...
RegimeTramitacaoCrud = CrudAux.build(RegimeTramitacao, "regime_tramitacao")

TipoDocumentoCrud = CrudAux.build(TipoDocumento, "tipo_documento")

TipoFimRelatoriaCrud = CrudAux.build(TipoFimRelatoria, "fim_relatoria")
...
def proposicao_texto__deprecated(request, pk):
    ...
    if proposicao.texto_original:
        if (
            not proposicao.data_recebimento
            and proposicao.autor.user_id != request.user.id
        ):
            logger.error(
                "user="
                + username
                + ". Usuário ({}) não tem permissão para acessar o texto original.".format(
                    request.user.id
                )
            )
            messages.error(
                request, _("Você não tem permissão para acessar o texto original.")
            )
            return redirect(
                reverse("sapl.materia:proposicao_detail", kwargs={"pk": pk})
            )
        ...
        return response
    logger.error(
        "user=" + username + ". Objeto Proposicao com pk={} não encontrado.".format(pk)
    )
    raise Http404
...
class AdicionarVariasAutorias(PermissionRequiredForAppCrudMixin, FilterView):
    ...
    def get_filterset_kwargs(self, filterset_class):
        super(AdicionarVariasAutorias, self).get_filterset_kwargs(filterset_class)
        kwargs = {"data": self.request.GET or None}

        qs = self.get_queryset()
        qs = qs.exclude(id__in=autores_ja_adicionados(self.kwargs["pk"])).distinct()

        kwargs.update({"queryset": qs})
        return kwargs

    def get_context_data(self, **kwargs):
        context = super(AdicionarVariasAutorias, self).get_context_data(**kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        for m in marcados:
            Autoria.objects.create(materia_id=self.kwargs["pk"], autor_id=m)

        return HttpResponseRedirect(
            reverse("sapl.materia:autoria_list", kwargs={"pk": self.kwargs["pk"]})
        )
...
class CriarProtocoloMateriaView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(CriarProtocoloMateriaView, self).get_context_data(**kwargs)
        ...
        try:
            ...
            self.logger.error(
                "user="
                + username
                + ". Objeto Protocolo com pk={} não encontrado. ".format(
                    self.kwargs["pk"]
                )
                + str(e)
            )
        ...
        try:
            self.logger.debug(
                "user=" + username + ". Tentando obter materias do último ano."
            )
            materias_ano = MateriaLegislativa.objects.filter(
                ano=protocolo.ano, tipo=protocolo.tipo_conteudo_protocolado
            ).latest("numero")
            ...
            self.logger.error(
                "user="
                + username
                + ". Não foram encontradas matérias no último ano ({}). "
        ...
        if protocolo:
            if protocolo.timestamp:
                context["form"].fields[
                    "data_apresentacao"
                ].initial = protocolo.timestamp.date()
            elif protocolo.timestamp_data_hora_manual:
                context["form"].fields[
                    "data_apresentacao"
                ].initial = protocolo.timestamp_data_hora_manual.date()
            elif protocolo.data:
    ...
    def form_valid(self, form):
        ...
        try:
            self.logger.info(
                "user="
                + username
                + ". Tentando obter objeto Procolo com pk={}.".format(self.kwargs["pk"])
            )
            protocolo = Protocolo.objects.get(pk=self.kwargs["pk"])
        ...
        if protocolo.autor:
            Autoria.objects.create(
                materia=materia, autor=protocolo.autor, primeiro_autor=True
            )
...
class ProposicaoTaView(IntegracaoTaView):
    ...
    map_funcs = {"publicacao_func": False}
    ta_values = {
    ...
    def get(self, request, *args, **kwargs):
        ...
        if sapl.base.models.AppConfig.attr("texto_articulado_proposicao"):
            ...
            if (
                not proposicao.data_envio
                and request.user not in proposicao.autor.operadores.all()
            ):
                raise Http404()
...
def recuperar_materia(request):
    ...
    max_numero, ano = MateriaLegislativa.get_proximo_numero(
        tipo=tipo, ano=int(ano) if ano else None
    )
...
class TipoProposicaoCrud(CrudAux):
    ...
    class BaseMixin(CrudAux.BaseMixin):
        list_field_names = ["descricao", "content_type", "tipo_conteudo_related"]
...
def criar_materia_proposicao(proposicao):
    ...
    numero = (
        MateriaLegislativa.objects.filter(ano=timezone.now().year)
        .order_by("numero")
        .last()
        .numero
        + 1
...
def criar_doc_proposicao(proposicao):
    tipo_doc = TipoDocumento.objects.get(descricao=proposicao.tipo.descricao)
...
class ProposicaoPendente(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["tipos_autores_materias"] = tipos_autores_materias(None).items()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                cod_hash = form.cleaned_data["cod_hash"].replace(
                    "/", SEPARADOR_HASH_PROPOSICAO
                )
                id = cod_hash.split(SEPARADOR_HASH_PROPOSICAO)[1]
                proposicao = Proposicao.objects.get(
                    id=id, data_envio__isnull=False, data_recebimento__isnull=True
                )
                ...
                if proposicao.texto_articulado.exists():
                    ...
                    hasher = (
                        "P" + ta.hash() + SEPARADOR_HASH_PROPOSICAO + str(proposicao.id)
                    )
                else:
                    hasher = (
                        gerar_hash_arquivo(
                            proposicao.texto_original.path, str(proposicao.id)
                        )
                        if proposicao.texto_original
                        else None
                    )
                if hasher == cod_hash:
                    return HttpResponseRedirect(
                        reverse(
                            "sapl.materia:proposicao-confirmar",
                            kwargs={
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            self.logger.info(
                "user="
                + username
                + ". Tentando obter objeto Proposicao com id={}.".format(kwargs["pk"])
            )
            ...
            self.logger.error(
                "user="
                + username
                + ". Objeto Proposicao com id={} não encontrado.".format(kwargs["pk"])
            )
            raise Http404()
        ...
        if not p.autor.operadores.filter(pk=request.user.id).exists():
            self.logger.error(
                "user="
                + username
                + ". Usuário ({}) sem acesso a esta opção.".format(request.user)
            )
            messages.error(request, "Usuário sem acesso a esta opção." % request.user)
            return redirect("/")

        return super(RetornarProposicao, self).dispatch(request, *args, **kwargs)
...
class ConfirmarProposicao(PermissionRequiredForAppCrudMixin, UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.object and self.object.proposicao_vinculada:
            messages.error(
                request,
                (
                    "Esta proposição está vinculada a: {}. Acesse-a e faça a incorporação primeiro dela."
                ).format(self.object.proposicao_vinculada),
            )
    ...
    def get_initial(self):
        ...
        initial["emendaloa"] = getattr(obj, "emendaloa", None)
    ...
    def get_object(self, queryset=None):
        ...
        try:
            ...
            """
            self.logger.debug(
                "user=" + username + ". Tentando obter objeto Proposicao."
            )
            proposicao = Proposicao.objects.get(
                pk=self.kwargs["pk"],
                data_envio__isnull=False,
            ...
            self.logger.error(
                "user="
                + username
                + ". Objeto Proposicao com atributos (pk={}, data_envio=Not Null, "
                "data_recebimento=Null) não encontrado. ".format(self.kwargs["pk"])
                + str(e)
            )
            raise Http404()
...
class ProposicaoCrud(Crud):
    ...
    class BaseMixin(Crud.BaseMixin):
        ...
        def impedimentos_de_envio(self, obj=None):
            ...
            limite_minimo_coletivo = (
                obj.tipo.tipo_conteudo_related.limite_minimo_coletivo
            )
            limite_por_autor_tramitando = (
                obj.tipo.tipo_conteudo_related.limite_por_autor_tramitando
            )
            if not obj.metadata or not limite_minimo_coletivo:
                return imp, limite_minimo_coletivo, limite_por_autor_tramitando, 0

            signs = (
                obj.metadata.get("signs", {}).get("texto_original", {}).get("signs", [])
            )
            ...
            autores_key_nome_completo = dict(
                map(
                    lambda el: ((el[0], el[0].autor_related.nome_completo), el[1]),
    ...
    class BaseLocalMixin:
        ...
        def get(self, request, *args, **kwargs):
            ...
            if (
                oa
                and oa.visibilidade_restrita
                and u != p.user
                and p.data_recebimento is None
            ):
            ...
            if not self._action_is_valid(request, *args, **kwargs):
                return redirect(
                    reverse(
                        "sapl.materia:proposicao_detail", kwargs={"pk": kwargs["pk"]}
                    )
                )
    ...
    class DetailView(Crud.DetailView):
        ...
        def get(self, request, *args, **kwargs):
            ...
            if not p.texto_original:
                messages.error(request, _("Proposição não possui texto associado."))

            p_md_signs_texto_original = (
                (p.metadata or {}).get("signs", {}).get("texto_original", {})
            )
            ...
            if not action:
                ...
                if p.tipo.exige_assinatura_digital:
                    ...
                    if isinstance(p.autor.autor_related, Parlamentar):
                        ...
                        pronome_sexo = (
                            "do" if p.autor.autor_related.sexo == "M" else "da"
                        )
                        signs = {
                            s for s in signs if nc.startswith(s) or s.startswith(nc)
                        }
            ...
            if p:
                if action == "send":
                    if p.data_envio and p.data_recebimento:
                        ...
                        p.tipo.exige_assinatura_digital
                        and p_md_signs_texto_original.get("running_extraction", False)
                    ):
                        msg_error = _(
                        ...
                        p.tipo.exige_assinatura_digital
                        and not p_md_signs_texto_original.get("signs", {})
                    ):
                        ...
                        if p.tipo.exige_assinatura_digital:
                            ...
                            if isinstance(p.autor.autor_related, Parlamentar):
                                ...
                                signs = {
                                    s
                                    for s in signs
                                    if nc.startswith(s) or s.startswith(nc)
                                }
                                if not signs and p.texto_original:
                                    msg_error = _(
                        ...
                        if not msg_error:
                            ...
                            messages.success(
                                request, _("Proposição enviada com sucesso.")
                            )
                            try:
                                self.logger.debug(
                                    "user="
                                    + username
                                    + ". Tentando obter número do objeto MateriaLegislativa com "
                                    "atributos tipo={} e ano={}.".format(
                                        p.tipo.tipo_conteudo_related, p.ano
                                    )
                                )
                    ...
                    if not p.data_envio:
                        self.logger.error(
                            "user="
                            + username
                            + ". Proposição (numero={}) ainda não foi enviada.".format(
                                p.numero_proposicao
                            )
                        )
                        ...
                        self.logger.error(
                            "user="
                            + username
                            + ". Proposição (numero={}) já foi recebida, não é "
                        ...
                        self.logger.info(
                            "user="
                            + username
                            + ". Proposição (numero={}) Retornada com sucesso.".format(
                                p.numero_proposicao
                            )
                        )
                        messages.success(
                            request, _("Proposição Retornada com sucesso.")
                        )
            ...
            return redirect(
                reverse("sapl.materia:proposicao_detail", kwargs={"pk": kwargs["pk"]})
            )
        ...
        def dispatch(self, request, *args, **kwargs):
            ...
            try:
                self.logger.debug(
                    "user="
                    + username
                    + ". Tentando obter objeto Proposicao com pk={}".format(
                        kwargs["pk"]
                    )
                )
                ...
                self.logger.error(
                    "user="
                    + username
                    + ". Erro ao obter proposicao com pk={}. Retornando 404. ".format(
                        kwargs["pk"]
                    )
                    + str(e)
                )
    ...
    class DeleteView(BaseLocalMixin, Crud.DeleteView):
        ...
        def _action_is_valid(self, request, *args, **kwargs):
            proposicao = Proposicao.objects.filter(id=kwargs["pk"]).values_list(
            ...
            if proposicao:
                if proposicao[0][0] and proposicao[0][1]:
                    self.logger.error(
                        "user="
                        + username
                        + ". Proposição (id={}) já foi enviada e recebida."
                        "Não pode mais ser excluida.".format(kwargs["pk"])
                    )
                    msg = _(
                        "Proposição já foi enviada e recebida."
                    ...
                    self.logger.error(
                        "user="
                        + username
                        + ". Proposição (id={}) já foi enviada mas ainda não recebida "
                        "pelo protocolo. Use a opção Recuperar Proposição "
                        "para depois excluí-la.".format(kwargs["pk"])
                    )
                    msg = _(
                        "Proposição já foi enviada mas ainda não recebida "
                        "pelo protocolo. Use a opção Recuperar Proposição "
    ...
    class UpdateView(BaseLocalMixin, Crud.UpdateView):
        ...
        def form_valid(self, form):
            ...
            if (
                tipo_texto == "D"
                and objeto_antigo.texto_articulado.exists()
                or tipo_texto == "T"
                and not objeto_antigo.texto_articulado.exists()
            ):
        ...
        def get_success_url(self):
            ...
            if tipo_texto == "T":
                messages.info(
                    self.request,
                    _(
                        "Sempre que uma Proposição é inclusa ou "
                ...
                self.logger.debug(
                    "user=" + username + ". Sempre que uma Proposição é inclusa ou "
                    'alterada e a opção "Texto Articulado " for '
                    "marcada, você será redirecionado para a "
                    "edição do Texto Eletrônico."
                )
                return reverse(
                    "sapl.materia:proposicao_ta", kwargs={"pk": self.object.pk}
                )
            else:
        ...
        def get_queryset(self):
            ...
            if oa and oa.visibilidade_restrita:
                ...
                q = Q(
                    ...
                    autor__operadores=u,
                    user=u,
                ) | Q(data_recebimento__isnull=False, autor__operadores=u)

                qs = qs.filter(q).order_by("-data_recebimento", "-data_envio", "-id")
        ...
        def get_rows(self, object_list):
            ...
            for obj in object_list:
                ...
                if obj.data_envio is None:
                    ...
                    if impedimentos[0]:
                        parls = ", ".join([x[1] for x in impedimentos[0]])
                        obj.data_envio = f"""
                ...
                if obj.data_recebimento is None:
                    obj.data_recebimento = (
                        "Não recebida" if obj.data_envio else "Não enviada"
                    )
...
class ReciboProposicaoView(TemplateView):
    ...
    def has_permission(self):
        ...
        return Proposicao.objects.filter(
            id=self.kwargs["pk"], autor__operadores=self.request.user
        ).exists()

    def get_context_data(self, **kwargs):
        from sapl.utils import create_barcode

        context = super(ReciboProposicaoView, self).get_context_data(**kwargs)
        ...
        if proposicao.texto_original:
            _hash = gerar_hash_arquivo(
                proposicao.texto_original.path, self.kwargs["pk"]
            )
        ...
        context.update({"proposicao": proposicao, "hash": _hash, "barcode": barcode})
        return context
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not proposicao.data_envio and not proposicao.data_devolucao:
            self.logger.error(
                "user=" + username + ". Não é possível gerar recibo para uma "
                "Proposição (pk={}) ainda não enviada.".format(self.kwargs["pk"])
            )
            messages.error(
                request,
                _(
                    "Não é possível gerar recibo para uma "
            ...
            self.logger.error(
                "user="
                + username
                + ". Não é possível gerar recibo para proposicao de pk={}.".format(
                    self.kwargs["pk"]
                )
            )
        ...
        return redirect(
            reverse("sapl.materia:proposicao_detail", kwargs={"pk": proposicao.pk})
        )
        ...
        def get_initial(self):
            ...
            composicoes = [
                ...
                for p in Participacao.objects.filter(
                    parlamentar=parlamentar, composicao__comissao=comissao
                )
            ]
            ...
            for c in composicoes:
                ...
                data_fim = (
                    c.periodo.data_fim if c.periodo.data_fim else timezone.now().date()
                )
                if data_inicial <= data_designacao <= data_fim:
                    composicao = c.id
                    break
            return {
                "comissao": relatoria.comissao.id,
...
class TramitacaoCrud(MasterDetailCrud):
    ...
    class CreateView(MasterDetailCrud.CreateView):
        ...
        def get_success_url(self):
            return reverse(
                "sapl.materia:tramitacao_list", kwargs={"pk": self.kwargs["pk"]}
            )

        def get_initial(self):
            initial = super(CreateView, self).get_initial()
            local = (
                MateriaLegislativa.objects.get(pk=self.kwargs["pk"])
                .tramitacao_set.order_by("-data_tramitacao", "-id")
                .first()
            )
        ...
        def get_context_data(self, **kwargs):
            ...
            ultima_tramitacao = (
                Tramitacao.objects.filter(materia_id=self.kwargs["pk"])
            ...
            primeira_tramitacao = not (
                Tramitacao.objects.filter(materia_id=int(kwargs["root_pk"])).exists()
            )
        ...
        def form_valid(self, form):
            ...
            try:
                self.logger.debug(
                    "user="
                    + username
                    + ". Tentando enviar Tramitacao (sender={}, post={}, request={}).".format(
                        Tramitacao, self.object, self.request
                    )
                )
            except Exception as e:
                msg = _(
    ...
    class UpdateView(CheckCheckMixin, MasterDetailCrud.UpdateView):
        ...
        def form_valid(self, form):
            ...
            try:
                ...
                msg = _(
                    "Tramitação atualizada, mas e-mail de acompanhamento "
                    "de matéria não enviado. Há problemas na configuração "
                    "do e-mail."
                )
                self.logger.warning(
    ...
    class ListView(MasterDetailCrud.ListView):
        ...
        def get_queryset(self):
            ...
            return qs.filter(**kwargs).order_by("-data_tramitacao", "-timestamp", "-id")
    ...
    class DeleteView(CheckCheckMixin, MasterDetailCrud.DeleteView):
        ...
        def delete(self, request, *args, **kwargs):
            ...
            url = reverse("sapl.materia:tramitacao_list", kwargs={"pk": materia.id})

            ultima_tramitacao = materia.tramitacao_set.order_by(
                "-data_tramitacao", "-timestamp", "-id"
            ).first()
...
def montar_helper_documento_acessorio(self):
    ...
    self.helper.layout[0][3][0].insert(
        1,
        form_actions(
            more=[
                HTML(
...
class AutoriaMultiCreateView(PermissionRequiredForAppCrudMixin, FormView):
    ...
    def get_success_url(self):
        messages.add_message(
            self.request, messages.SUCCESS, _("Autorias adicionadas com sucesso.")
        )
        return reverse("sapl.materia:autoria_list", kwargs={"pk": self.materia.pk})
    ...
    def form_valid(self, form):
        ...
        for autor in autores_selecionados:
            Autoria.objects.create(
                materia=self.materia, autor=autor, primeiro_autor=primeiro_autor
            )
...
class DespachoInicialMultiCreateView(PermissionRequiredForAppCrudMixin, FormView):
    ...
    def get_success_url(self):
        messages.add_message(
            self.request, messages.SUCCESS, _("Despachos adicionados com sucesso.")
        )
        return reverse(
...
class LegislacaoCitadaCrud(MasterDetailCrud):
    ...
    class BaseMixin(MasterDetailCrud.BaseMixin):
        ...
        def resolve_url(self, suffix, args=None):
            ...
            return reverse("%s:%s" % (namespace, self.url_name(suffix)), args=args)
...
class AnexadaCrud(MasterDetailCrud):
    ...
    class BaseMixin(MasterDetailCrud.BaseMixin):
        list_field_names = ["materia_anexada", "data_anexacao", "data_desanexacao"]
    ...
    class UpdateView(CheckCheckMixin, Crud.UpdateView):
        ...
        def form_valid(self, form):
            ...
            atributos = [
                ...
                "polemica",
                "objeto",
                "regime_tramitacao_id",
                "em_tramitacao",
                "data_fim_prazo",
                "data_publicacao",
                "complementar",
                "tipo_origem_externa_id",
                "numero_origem_externa",
                "ano_origem_externa",
                "local_origem_externa_id",
                "data_origem_externa",
                "ementa",
                "indexacao",
                "observacao",
            ...
            if Anexada.objects.filter(materia_principal=self.kwargs["pk"]).exists():
                ...
                for anexada in anexadas:
                    anexada.em_tramitacao = (
                        True if form.instance.em_tramitacao else False
                    )
                    anexada.save()
    ...
    class DetailView(BtnCertMixin, GoogleRecapthaViewMixin, Crud.DetailView):
                        ...
                        [
                            (
                                reverse(
                                    ...
                                    kwargs={
                                        "pk": self.object.texto_articulado.first().pk
                                    },
                                ),
                                "btn-danger",
            ...
            return [
                (
                    "{}?ia_run=generate".format(
                        reverse(viewname, kwargs={"pk": self.kwargs["pk"]})
                    ),
                    "btn-primary",
        ...
        def ia_run(self):
            ...
            if not settings.DEBUG or (
                ...
                logger.info(
                    "Iniciando tarefa assíncrona de análise de similaridade entre matérias via classificação."
                )
                task_analise_similaridade_entre_materia_via_classificacao.apply_async(
                    (gen.object.id,), countdown=10
                )
            else:
                task_analise_similaridade_entre_materias_function(
                    only_materia_id=gen.object.id
                )
        ...
        def get(self, request, *args, **kwargs):
            ...
            if "homologar" in request.GET and request.user.has_perm(
                "materia.add_materialegislativa"
            ):
                self.object = self.get_object()
            ...
            if obj.protocolo_gr.exists():
                ...
                if self.request.user.has_perm("protocoloadm.list_protocolo"):
                    value = '<a href="{}">{}</a>'.format(
                        reverse(
                            "sapl.protocoloadm:protocolo_mostrar", kwargs={"pk": p.id}
                        ),
                        value,
                    )
        ...
        def hook_diariooficial_set(self, obj):
            if obj.diariosoficiais.exists():
                ...
                value = '<a href="{}">{}</a> <small><i>({})</i></small>'.format(
                    reverse("cmj.diarios:diariooficial_detail", kwargs={"pk": d.id}),
                    d,
                    formats.date_format(d.data, "DATE_FORMAT"),
                )
        ...
        def hook_documentoadministrativo_set__deprecated(self, obj):
            ...
            for d in docs:
                ...
                if d.workspace.tipo != AreaTrabalho.TIPO_PUBLICO:
                    if not self.request.user.groups.filter(
                        name=GROUP_MATERIA_WORKSPACE_VIEWER
                    ).exists():

                        rede = self.request.META.get("HTTP_X_FORWARDED_FOR", "")
                        if (
                            not rede.startswith("168.228.184")
                            and self.request.META.get("REMOTE_ADDR", "") != "127.0.0.1"
                        ):
                            continue
            ...
            result[1] = "<br>".join(
                map(
                    lambda d: '<a class="w-100" href="{}">{}</a><br><small>{}</small>'.format(
                        d.texto_integral.url,
                        d,
                        (
                            d.observacao
                            if d.workspace.tipo == AreaTrabalho.TIPO_PROCURADORIA
                            else d.assunto
                        ),
                    ),
                    result[1],
        ...
        def hook_normajuridica_set(self, obj):
            ...
            return _("Norma Jurídica Vinculada"), "<br>".join(
                [
                    '<a href="{}">{}</a>'.format(
                        reverse("sapl.norma:normajuridica_detail", kwargs={"pk": n.id}),
                        n,
                    )
                    for n in normas
                ]
        ...
        def hook_autografo_set(self, obj):
            ...
            return _("Autógrafo"), "<br>".join(
                [
                    '<a href="{}">{}</a>'.format(
                        reverse("sapl.norma:normajuridica_detail", kwargs={"pk": a.id}),
                        a,
                    )
                    for a in autografos
                ]
...
class DocumentoAcessorioView(PermissionRequiredMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        docs = DocumentoAcessorio.objects.filter(materia_id=kwargs["pk"]).order_by(
            "data"
        )
        form = DocumentoAcessorioForm()

        return self.render_to_response({"object": materia, "form": form, "docs": docs})

    def post(self, request, *args, **kwargs):
        form = self.get_form()
        materia = MateriaLegislativa.objects.get(id=kwargs["pk"])
        docs_list = DocumentoAcessorio.objects.filter(materia_id=kwargs["pk"])
        ...
        if form.is_valid():
            ...
            return self.render_to_response(
                {"form": form, "object": materia, "docs": docs_list}
            )
...
class AcompanhamentoConfirmarView(TemplateView):
    ...
    def get_redirect_url(self, email):
        ...
        self.logger.debug(
            "user="
            + username
            + ". Esta matéria está sendo acompanhada pelo e-mail: %s" % (email)
        )
        msg = _("Esta matéria está sendo acompanhada pelo e-mail: %s") % (email)
        messages.add_message(self.request, messages.SUCCESS, msg)
        return reverse(
            "sapl.materia:materialegislativa_detail", kwargs={"pk": self.kwargs["pk"]}
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            self.logger.info(
                "user="
                + username
                + ". Tentando obter objeto AcompanhamentoMateria (materia_id={}, hash={}).".format(
                    materia_id, hash_txt
                )
            )
            ...
            self.logger.error(
                "user="
                + username
                + ". Objeto AcompanhamentoMateria(materia_id={}, hash={}) não encontrado.".format(
                    materia_id, hash_txt
                )
            )
            raise Http404()
...
class AcompanhamentoExcluirView(TemplateView):
    ...
    def get_success_url(self):
        username = self.request.user.username
        self.logger.debug(
            "user=" + username + ". Você parou de acompanhar esta matéria."
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            self.logger.info(
                "user="
                + username
                + ". Tentando deletar objeto AcompanhamentoMateria (materia_id={}, hash={}).".format(
                    materia_id, hash_txt
                )
            )
...
class MateriaLegislativaPesquisaView(
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        if (
            not context["show_results"]
            and not self.request.user.is_superuser
            and self.app_label in __name__
        ):
    ...
    def get_context_data(self, **kwargs):
        ...
        acesso_rapido_list = list(
            tipos_autores_materias(None, restricao_regimental=False).items()
        )
...
class AcompanhamentoMateriaView(CreateView):
    ...
    def get(self, request, *args, **kwargs):
        if not mail_service_configured():
            ...
            messages.error(
                request,
                _("Serviço de Acompanhamento de " "Matérias não foi configurado"),
            )
            return redirect("/")
        ...
        return self.render_to_response(
            {"form": AcompanhamentoMateriaForm(), "materia": materia}
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            acompanhar = AcompanhamentoMateria.objects.get_or_create(
                materia=materia, email=form.data["email"]
            )
            ...
            if acompanhar[1]:
                ...
                destinatario = AcompanhamentoMateria.objects.get(
                    materia=materia, email=email, confirmado=False
                )
                casa = CasaLegislativa.objects.first()

                do_envia_email_confirmacao(
                    base_url, casa, "materia", materia, destinatario
                )
                self.logger.debug(
                    "user="
                    + usuario.username
            ...
            # avisa ao usuário que essa matéria já está sendo acompanhada
                ...
                return self.render_to_response({"form": form, "materia": materia})
            return HttpResponseRedirect(self.get_success_url())
        else:
            return self.render_to_response({"form": form, "materia": materia})
...
class DocumentoAcessorioEmLoteView(PermissionRequiredMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(DocumentoAcessorioEmLoteView, self).get_context_data(**kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.FILES["arquivo"].size > MAX_DOC_UPLOAD_SIZE:
            msg = _(
                "O arquivo Anexo de Texto Integral deve ser menor que {0:.1f} MB, \
                o tamanho atual desse arquivo é {1:.1f} MB".format(
                    (MAX_DOC_UPLOAD_SIZE / 1024) / 1024,
                    (request.FILES["arquivo"].size / 1024) / 1024,
                )
            )
            messages.add_message(request, messages.ERROR, msg)
            return self.get(request, self.kwargs)
        ...
        try:
            doc_data = tz.localize(datetime.strptime(request.POST["data"], "%d/%m/%Y"))
        except Exception as e:
            msg = _(
        ...
        for materia_id in marcadas:
            ...
            try:
                ...
                for m in [
                    "%s: %s"
                    % (
                        DocumentoAcessorio()._meta.get_field(k).verbose_name,
                        "</br>".join(v),
                    )
            ...
            file_path = os.path.join(diretorio, request.FILES["arquivo"].name)
            shutil.copy2(tmp_name, file_path)
            doc.arquivo.name = file_path.split(MEDIA_ROOT + "/")[
                1
            ]  # Retira MEDIA_ROOT do nome
            doc.save()
        os.remove(tmp_name)
...
class MateriaAnexadaEmLoteView(PermissionRequiredMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(MateriaAnexadaEmLoteView, self).get_context_data(**kwargs)
        ...
        not_list = [self.kwargs["pk"]] + [
            m
            for m in principal.materia_principal_set.all().values_list(
                "materia_anexada_id", flat=True
            )
        ]
...
class PrimeiraTramitacaoEmLoteView(PermissionRequiredMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(PrimeiraTramitacaoEmLoteView, self).get_context_data(**kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        form = TramitacaoEmLoteForm(
            request.POST, initial={"materias": materias_ids, "user": user, "ip": ip}
        )
...
class EtiquetaPesquisaView(PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        materias = MateriaLegislativa.objects.all().order_by("-data_apresentacao")
        ...
        if form.cleaned_data["processo_inicial"]:
            materias = materias.filter(
                numeracao__numero_materia__gte=form.cleaned_data["processo_inicial"],
                numeracao__numero_materia__lte=form.cleaned_data["processo_final"],
            )
...
class FichaSelecionaView(PermissionRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        if (
            ...
            return HttpResponseRedirect(
                reverse("sapl.materia:impressos_ficha_pesquisa")
            )

        context = super(FichaSelecionaView, self).get_context_data(**kwargs)
        ...
        materia_list = MateriaLegislativa.objects.filter(
            tipo=tipo, data_apresentacao__range=(data_inicial, data_final)
        )
    ...
    def form_valid(self, form):
        ...
        try:
            self.logger.debug(
                "user="
                + username
                + ". Tentando obter objeto MateriaLegislativa com id={}".format(
                    form.data["materia"]
                )
            )
            materia = MateriaLegislativa.objects.get(id=form.data["materia"])
        except ObjectDoesNotExist:
            self.logger.error(
                "user="
                + username
                + ". Esta MáteriaLegislativa não existe (id={}).".format(
                    form.data["materia"]
                )
            )
...
class MateriaPesquisaSimplesView(PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        materias = MateriaLegislativa.objects.filter(**kwargs).order_by(
            "-numero", "ano"
        )
        ...
        context = {
            "quantidade": quantidade_materias,
...
class TipoMateriaCrud(CrudAux):
    ...
    class ListView(CrudAux.ListView):
        ...
        def hook_sigla(self, obj, default, url):
            return '<a href="{}" pk="{}">{}</a>'.format(url, obj.id, obj.sigla), ""

        def get(self, request, *args, **kwargs):
            if TipoMateriaLegislativa.objects.filter(sequencia_regimental=0).exists():
                TipoMateriaLegislativa.objects.reordene()
    ...
    class CreateView(CrudAux.CreateView):
        ...
        def form_valid(self, form):
            ...
            if not TipoMateriaLegislativa.objects.exclude(
                ...
                self.object.sequencia_regimental = (
                    sr__max["sequencia_regimental__max"] + 1
                )
                self.object.save()
...
class AnaliseSimilaridadeRankingCrud(Crud):
    ...
    class ListView(Crud.ListView):
        ...
        def get_queryset(self):
            ...
            qs = qs.filter(similaridade__gte=filtro_minino).order_by(
                "-materia_1__ano",
                "-data_analise__year",
                "-data_analise__month",
                "-similaridade",
    ...
    class ListView(MasterDetailCrud.ListView):
        def get_queryset(self):
            ...
            qs = AnaliseSimilaridade.objects.filter(
                Q(materia_1__id=pk) | Q(materia_2__id=pk), similaridade__gt=0
