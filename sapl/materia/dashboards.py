...
class MateriaFilterSet(FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.helper.layout = Layout(row1, row2, row3)
...
class MateriaTotalizer(Dashcard):
    ...
    datasets = [
        {"label": _("Qtd. de Matérias Legislativas"), "data_field": ("id", Count)}
    ]

    def get_datasets(self, request, queryset=None):
        return [{"data": [queryset.count()]}]
...
class OrderedResultMixin:
    ...
    def chartdata(self, request, queryset=None, limit=20):
        ...
        sorted_data = sorted(
            filter(lambda y: y[0], zip(labels, datasets__data)),
            key=lambda x: x[1],
            reverse=True,
        )
        if sorted_data:
        ...
        if len(labels) > limit:
            ...
            datasets__data = list(datasets__data[:limit]) + [
                sum(datasets__data[limit:])
            ]
        else:
            labels = list(labels)
...
class MateriaParlamentarDashboard(GridDashboard):
    ...
    app_config = "materia"
    cards = [MateriaMonthlyDashboard, MateriaTotalizerFiltered]
