...
from sapl.utils import FileFieldCheckMixin, timezone
    ...
    def clean(self):
        ...
        if self.cleaned_data["hora_inicio"] and self.cleaned_data["hora_fim"]:
            if self.cleaned_data["hora_fim"] < self.cleaned_data["hora_inicio"]:
                msg = _(
                    "A hora de fim ({}) não pode ser anterior a hora "
                    "de início({})".format(
        ...
        if upload_pauta and upload_pauta.size > MAX_DOC_UPLOAD_SIZE:
            raise ValidationError(
                "O arquivo Pauta da Audiência Pública deve ser menor que {0:.1f} mb, o tamanho atual desse arquivo é {1:.1f} mb".format(
        ...
        if upload_ata and upload_ata.size > MAX_DOC_UPLOAD_SIZE:
            raise ValidationError(
                "O arquivo Ata da Audiência Pública deve ser menor que {0:.1f} mb, o tamanho atual desse arquivo é {1:.1f} mb".format(
        ...
        if upload_anexo and upload_anexo.size > MAX_DOC_UPLOAD_SIZE:
            raise ValidationError(
                "O arquivo Anexo da Audiência Pública deve ser menor que {0:.1f} mb, o tamanho atual desse arquivo é {1:.1f} mb".format(
                    (MAX_DOC_UPLOAD_SIZE / 1024) / 1024,
