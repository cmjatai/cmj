...
class View(RelatorioMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["html_capa"] = self.get_html([self.get_capa()])
    ...
    def get_capa(self):
        ...
        self.legislatura = Legislatura.objects.filter(
            data_inicio__year__lte=self.ano, data_fim__year__gte=self.ano
        ).first()
        ...
        for m in self.membros_da_mesa:
            mark1.append(
                f"* {m.cargo.descricao.upper()}: **{m.parlamentar.nome_parlamentar}**"
            )
    ...
    def get_comissoes(self):
        ...
        ).order_by("composicao__comissao__nome", "cargo_id")
        comissoes = []
        comissao = []
        ...
        for participacao in participacoes:
            if participacao.composicao.comissao != comissao_atual:
                ...
                comissao.append(
                    f"#### [{comissao_atual.sigla} - {comissao_atual.nome}]({self.site_url}/comissao/{comissao_atual.pk})"
                )

            comissao.append(
                f"* {participacao.cargo.nome.upper()}: **{participacao.parlamentar.nome_parlamentar}**"
            )
    ...
    def get_normas(self):
        ...
        mark2 = []
        mark2.append(
            f"### Assuntos tratados nos Atos Normativos/Legislativos de {self.ano}"
        )

        assuntos = (
            AssuntoNorma.objects.filter(normajuridica__data__year=self.ano)
            .annotate(Count("normajuridica"))
            .order_by("assunto")
        )
    ...
    def get_materias(self):
        ...
        assuntos = (
            AssuntoMateria.objects.filter(
                materialegislativa__ano=self.ano, materialegislativa__tipo_id=3
            )
            .annotate(Count("materialegislativa"))
            .order_by("assunto")
        )
