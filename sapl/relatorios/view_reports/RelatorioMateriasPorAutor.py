...
class View(RelatorioMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if data and stop_filter:
            messages.error(self.request, "Informe ao menos um dos filtros abaixo.")
            context["object_list"] = []
            return context
        ...
        if qr["data_apresentacao_0"]:
            context["periodo"] = (
                qr["data_apresentacao_0"] + " - " + qr["data_apresentacao_1"]
            )
        ...
        qs_autores = (
            context["object_list"]
            .values("autoria__autor__id", "autoria__autor__nome")
            .order_by("autoria__autor__id", "autoria__autor__nome")
            .distinct()
        )
        ...
        for autor, anos in r.items():
            context["result_dict"][autor] = dict(sorted(anos.items(), reverse=True))
