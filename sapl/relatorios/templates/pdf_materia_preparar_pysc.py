...
materias = []
REQUEST = context.REQUEST
for materia in context.zsql.materia_pesquisar_zsql(
    tip_id_basica=REQUEST["lst_tip_materia"],
    ...
    nom_relator=REQUEST["txt_relator"],
    ...
    cod_partido=REQUEST["lst_cod_partido"],
    cod_autor=REQUEST["hdn_cod_autor"],
    rd_ordem=REQUEST["rd_ordenacao"],
    rd_ordem_td=REQUEST["rd_ordem_td"],
):
    ...
    for tramitacao in context.zsql.tramitacao_obter_zsql(
        ...
        for unidade_tramitacao in context.zsql.unidade_tramitacao_obter_zsql(
    ...
    dic["norma_vinculada"] = " "
    for norma_vinculada in context.zsql.materia_buscar_norma_juridica_zsql(
...
if REQUEST.lst_tip_materia != "":
    for tipo_materia in context.zsql.tipo_materia_legislativa_obter_zsql(
        ind_excluido=0, tip_materia=REQUEST.lst_tip_materia
    ):
        filtro["tipo_materia"] = (
            tipo_materia.sgl_tipo_materia + " - " + tipo_materia.des_tipo_materia
        )

filtro["partido"] = ""
if REQUEST.lst_cod_partido != "":
    for partido in context.zsql.partido_obter_zsql(
        ind_excluido=0, cod_partido=REQUEST.lst_cod_partido
    ):
...
if REQUEST.lst_status != "":
    for status in context.zsql.status_tramitacao_obter_zsql(
        ind_excluido=0, cod_status=REQUEST.lst_status
    ):
