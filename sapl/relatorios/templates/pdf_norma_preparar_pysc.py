...
normas = []
REQUEST = context.REQUEST
for norma in context.zsql.norma_juridica_obter_zsql(
    tip_norma=REQUEST["lst_tip_norma"],
    ...
    dic["titulo"] = (
        norma.sgl_tipo_norma
        + " Nº  "
        + str(norma.num_norma)
        + " de "
        + str(norma.dat_norma)
        + " - "
        + norma.des_tipo_norma
    )
    dic["txt_ementa"] = norma.txt_ementa

    dic["materia_vinculada"] = " "
    if norma.cod_materia != None:
        for materia_vinculada in context.zsql.materia_obter_zsql(
            cod_materia=str(norma.cod_materia)
        ):
            dic["materia_vinculada"] = (
                materia_vinculada.sgl_tipo_materia
                + " "
...
if REQUEST.lst_tip_norma != "":
    for tipo_norma in context.zsql.tipo_norma_juridica_obter_zsql(
        ind_excluido=0, tip_norma=REQUEST.lst_tip_norma
    ):
        filtro["tipo_norma"] = (
            tipo_norma.sgl_tipo_norma + " - " + tipo_norma.des_tipo_norma
        )
