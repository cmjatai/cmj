...
def cabecalho(dic_inf_basicas, imagem):
    ...
    if (
        ...
        and str(dic_inf_basicas["nom_projeto"]) != None
    ):
        tmp += '\t\t\t\t<setFont name="Helvetica" size="15"/>\n'
        tmp += (
            '\t\t\t\t<drawCentredString x="10.5cm" y="25.2cm">'
            + str(dic_inf_basicas["nom_projeto"])
            + "</drawCentredString>\n"
        )
    if (
        str(dic_inf_basicas["cod_projeto"]) != ""
        and str(dic_inf_basicas["cod_projeto"]) != None
    ):
        tmp += '\t\t\t\t<setFont name="Helvetica" size="15"/>\n'
        tmp += (
            '\t\t\t\t<drawCentredString x="10.5cm" y="24.7cm">'
            + str(dic_inf_basicas["cod_projeto"])
            + "</drawCentredString>\n"
        )
    return tmp
...
def inf_basicas(dic_inf_basicas):
    ...
    if (
        ...
        and str(dic_inf_basicas["apresentada"]) != None
    ):
        tmp += (
            '\t\t<para style="P2"><b>Apresentada em: </b> '
            + str(dic_inf_basicas["apresentada"])
            + "</para>\n"
        )
    ...
    if (
        ...
        and str(dic_inf_basicas["formato"]) != None
    ):
        tmp += (
            '\t\t<para style="P2"><b>Formato: </b> '
            + str(dic_inf_basicas["formato"])
            + "</para>\n"
        )
    ...
    if (
        ...
        and str(dic_inf_basicas["reg_tramitacao"]) != None
    ):
        tmp += (
            '\t\t<para style="P2"><b>Regime: </b> '
            + str(dic_inf_basicas["reg_tramitacao"])
            + "</para>\n"
        )
    ...
    if str(dic_inf_basicas["prazo"]) != "" and str(dic_inf_basicas["prazo"]) != None:
        tmp += (
            '\t\t<para style="P2"><b>Dias de prazo: </b> '
    ...
    if (
        ...
        and str(dic_inf_basicas["fim_prazo"]) != None
    ):
        tmp += (
            '\t\t<para style="P2"><b>Data do fim do prazo: </b> '
            + str(dic_inf_basicas["fim_prazo"])
            + "</para>\n"
        )
...
def orig_externa(dic_orig_externa):
    ...
    try:
        ...
        if (
            ...
            and dic_orig_externa["numero_ano"] != None
        ):
            tmp += (
                '\t\t<para style="P2"><b>Número/Ano:</b> '
                + dic_orig_externa["numero_ano"]
                + "</para>\n"
            )
    except:
...
def mat_anexadas(lst_mat_anexadas):
    ...
    for dic_mat in lst_mat_anexadas:
        if dic_mat["nom_mat"] != "" and dic_mat["nom_mat"] != None:
            tmp += (
                '\t\t<para style="P2"><b>Nome da matéria:</b> '
                + dic_mat["nom_mat"]
                + "</para>\n"
            )
            tmp += '\t\t<para style="P2"><b>Data:</b> ' + dic_mat["data"] + "</para>\n"
            tmp += (
                '\t\t<para style="P2"><b>Data final:</b> '
...
def autoria(lst_autoria):
    ...
    for dic_autor in lst_autoria:
        if dic_autor["nom_autor"] != "" and dic_autor["nom_autor"] != None:
            tmp += (
                '\t\t<para style="P2"><b>Nome do Autor:</b> '
                + dic_autor["nom_autor"]
                + "</para>\n"
            )
        ...
        if dic_autor["nom_autor"] != "" and dic_autor["cargo"] != None:
            tmp += (
                '\t\t<para style="P2"><b>Cargo:</b> ' + dic_autor["cargo"] + "</para>\n"
            )

        if dic_autor["nom_autor"] != "" and dic_autor["tipo"] != None:
            tmp += (
                '\t\t<para style="P2"><b>Tipo:</b> ' + dic_autor["tipo"] + "</para>\n"
            )
    return tmp
...
def despachos_iniciais(lst_des_iniciais):
    ...
    for dic_dados in lst_des_iniciais:
        ...
        tmp += (
            '\t\t<para style="P2"><b>Nome da comissão:</b> '
            + dic_dados["nom_comissao"]
            + "</para>\n"
        )
    return tmp
...
def tramitacoes(dic_tramitacoes):
    ...
    try:
        tmp += (
            '\t\t<para style="P2"><b>Data Ação:</b> '
            + str(dic_tramitacoes["data"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Unidade Local:</b> '
            + dic_tramitacoes["unidade"]
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Encaminhada em:</b> '
            + str(dic_tramitacoes["data_enc"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Destino:</b> '
            + dic_tramitacoes["destino"]
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Turno:</b> '
            + dic_tramitacoes["turno"]
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Status:</b> '
        ...
        if (
            ...
            and dic_tramitacoes["texto_acao"] != None
        ):
            tmp += (
                '\t\t<para style="P2"><b>Texto da Ação:</b> '
...
def relatorias(lst_relatorias):
    ...
    for dic_comissao in lst_relatorias:
        tmp += (
            '\t\t<para style="P2"><b>Comissão:</b> '
            + dic_comissao["nom_comissao"]
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Data Designação:</b> '
            + str(dic_comissao["data_desig"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Parlamentar:</b> '
            + dic_comissao["parlamentar"]
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Data Destituição:</b> '
            + str(dic_comissao["data_dest"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Motivo Fim Relatoria:</b> '
            + dic_comissao["motivo"]
            + "</para>\n"
        )
    return tmp
...
def legislacoes_citadas(lst_leg_citadas):
    ...
    for dic_dados in lst_leg_citadas:
        tmp += (
            '\t\t<para style="P2"><b>Tipo Norma:</b> '
            + str(dic_dados["nome_lei"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Disposição:</b> '
            + str(dic_dados["disposicao"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Parte:</b> '
            + str(dic_dados["parte"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Livro:</b> '
            + str(dic_dados["livro"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Tí­tulo:</b> '
            + str(dic_dados["titulo"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Capí­tulo:</b> '
            + str(dic_dados["capitulo"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Seção:</b> '
            + str(dic_dados["secao"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Subseção:</b> '
            + str(dic_dados["subsecao"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Artigo:</b> '
            + str(dic_dados["artigo"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Parágrafo:</b> '
            + str(dic_dados["paragrafo"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Inciso:</b> '
            + str(dic_dados["inciso"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Alí­nea:</b> '
            + str(dic_dados["alinea"])
            + "</para>\n"
        )
        tmp += (
            '\t\t<para style="P2"><b>Item:</b> ' + str(dic_dados["item"]) + "</para>\n"
        )
    return tmp
...
def documentos_acessorios(lst_acessorios):
    ...
    for dic_dados in lst_acessorios:
        if dic_dados["tipo"] != None:
            tmp += (
                '\t\t<para style="P2"><b>Tipo:</b> ' + dic_dados["tipo"] + "</para>\n"
            )

        if dic_dados["nome"] != None:
            tmp += (
                '\t\t<para style="P2"><b>Nome:</b> ' + dic_dados["nome"] + "</para>\n"
            )

        tmp += '\t\t<para style="P2"><b>Data:</b> ' + dic_dados["data"] + "</para>\n"
        if dic_dados["autor"] != None:
            tmp += (
                '\t\t<para style="P2"><b>Autor:</b> ' + dic_dados["autor"] + "</para>\n"
            )

        if dic_dados["ementa"] != None:
            tmp += (
                '\t\t<para style="P2"><b>Ementa:</b> '
