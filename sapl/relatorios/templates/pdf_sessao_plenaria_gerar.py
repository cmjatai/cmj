...
def cabecalho(inf_basicas_dic, imagem):
    ...
    tmp += (
        '\t\t\t\t<drawString x="5cm" y="27.1cm">'
        + str(inf_basicas_dic["nom_camara"])
        + "</drawString>\n"
    )
    ...
    tmp += (
        '\t\t\t\t<drawString x="4.2cm" y="25cm">Resumo da '
        + str(inf_basicas_dic["num_sessao_plen"])
        + "ª Reunião "
        + str(inf_basicas_dic["nom_sessao"])
        + " da "
        + str(inf_basicas_dic["num_sessao_leg"])
        + "ª Sessão Legislativa da </drawString>\n"
    )
    tmp += (
        '\t\t\t\t<drawString x="6.7cm" y="24.5cm">'
...
def rodape(rodape_dic):
    ...
    tmp += '\t\t\t\t<drawString x="2cm" y="3.3cm">' + rodape_dic[2] + "</drawString>\n"
    tmp += (
...
def inf_basicas(inf_basicas_dic):
    ...
    if (
        ...
        if nom_sessao:
            tmp += (
                '\t\t<para style="P2" spaceAfter="5"><b>Tipo da Sessão: </b> '
                + nom_sessao
                + "</para>\n"
            )
        if hr_inicio_sessao:
            tmp += (
                '\t\t<para style="P2" spaceAfter="5"><b>Abertura: </b> '
                + dat_inicio_sessao
                + " <b>- </b> "
                + hr_inicio_sessao
                + "</para>\n"
        ...
        if dat_fim_sessao or hr_fim_sessao:
            tmp += (
                '\t\t<para style="P2" spaceAfter="5"><b>Encerramento: </b> '
                + dat_fim_sessao
                + " <b>- </b> "
                + hr_fim_sessao
                + "</para>\n"
    ...
    if inf_basicas_dic.get("tema_solene"):
        tmp += (
            '\t\t<para style="P2" spaceAfter="5"><b>Tema da Sessão Solene: </b> '
...
def multimidia(cont_mult_dic):
    ...
    if mul_audio or mul_video:
        ...
        if mul_audio:
            tmp += (
                '\t\t<para style="P2" spaceAfter="5"><b>Audio: </b> '
                + mul_audio
                + "</para>\n"
            )
        if mul_video:
            tmp += (
                '\t\t<para style="P2" spaceAfter="5"><b>Video: </b> '
                + mul_video
                + "</para>\n"
            )
...
def mesa(lst_mesa):
    ...
    if lst_mesa:
        ...
        for mesa in lst_mesa:
            tmp += (
                '\t\t<para style="P2" spaceAfter="5"><b>'
                + str(mesa["des_cargo"])
                + ":</b> "
                + str(mesa["nom_parlamentar"])
                + "/"
...
def expediente_materia(lst_expediente_materia):
    ...
    if lst_expediente_materia:
        ...
        for expediente_materia in lst_expediente_materia:
            tmp += (
                '<tr><td><para style="P3"><b>'
                + str(expediente_materia["num_ordem"])
                + "</b> - "
                + expediente_materia["id_materia"]
                + "</para>\n"
                + '<para style="P3"><b>Turno: </b>'
                + expediente_materia["des_turno"]
                + "</para>\n"
                + '<para style="P3"><b>'
                + expediente_materia["num_autores"]
                + ": </b>"
                + str(expediente_materia["nom_autor"])
                + "</para></td>\n"
            )
...
def expediente_materia_vot_nom(lst_expediente_materia_vot_nom):
    ...
    if lst_expediente_materia_vot_nom:
        tmp += (
            '\t\t<para style="P1">Votações Nominais - Matérias do Expediente</para>\n\n'
        )
        tmp += '\t\t<para style="P2">\n'
        tmp += '\t\t\t<font color="white"> <br/></font>\n'
        tmp += "\t\t</para>\n"
        ...
        for expediente_materia_vot_nom in lst_expediente_materia_vot_nom:
            tmp += (
                '<tr><td><para style="P3">'
                + str(expediente_materia_vot_nom["titulo"])
                + "</para></td>"
            )
...
def oradores_expediente(lst_oradores_expediente):
    ...
    if lst_oradores_expediente:
        ...
        for orador_expediente in lst_oradores_expediente:
            tmp += (
                '\t\t<para style="P2" spaceAfter="5"><b>'
                + str(orador_expediente["num_ordem"])
                + "</b> - "
                + orador_expediente["nom_parlamentar"]
                + "/"
                + str(orador_expediente["sgl_partido"])
                + " - "
                + str(orador_expediente["observacao"])
                + "</para>\n"
            )
    return tmp
...
def oradores(lst_oradores):
    ...
    if lst_oradores:
        ...
        for orador in lst_oradores:
            tmp += (
                '\t\t<para style="P2" spaceAfter="5"><b>'
                + str(orador["num_ordem"])
                + "</b> - "
                + orador["nom_parlamentar"]
                + "/"
...
def ocorrencias(lst_ocorrencias):
    ...
    if lst_ocorrencias:
        ...
        for ocorrencia in lst_ocorrencias:
            tmp += '\t\t<para style="P3">' + str(ocorrencia.conteudo) + "</para>\n"
            tmp += '\t\t<para style="P2">\n'
            tmp += '\t\t\t<font color="white"> </font>\n'
            tmp += "\t\t</para>\n"
    return tmp
