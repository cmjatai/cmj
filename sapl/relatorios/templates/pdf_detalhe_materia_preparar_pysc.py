...
for materia in context.zsql.materia_obter_zsql(cod_materia=REQUEST["cod_materia"]):
    ...
    inf_basicas_dic["cod_projeto"] = (
        materia.sgl_tipo_materia
        + "   "
    ...
    for tramitacao in context.zsql.regime_tramitacao_obter_zsql(
    ...
    for origem in context.zsql.origem_obter_zsql(
        cod_origem=materia.cod_local_origem_externa
    ):
        #  #tratando possíveis erros
        ...
        orig_externa_dic["numero_ano"] = (
            str(materia.num_origem_externa) + "/" + str(materia.ano_origem_externa)
        )

    # #o bloco abaixo gera o dicionario das materias anexadas (ln 55)
    lst_mat_anexadas = []
    dic_mat = {}
    for anexada in context.zsql.anexada_obter_zsql(
        cod_materia_principal=materia.cod_materia
    ):
        ...
        #  """#"""
        dic_mat["nom_mat"] = (
            aux2[0].sgl_tipo_materia
            + "/"
            + str(aux1[0].num_ident_basica)
            + "/"
            + str(aux1[0].ano_ident_basica)
        )
    ...
    for autoria in context.zsql.autoria_obter_zsql(cod_materia=materia.cod_materia):
        ...
        for autor in context.zsql.autor_obter_zsql(cod_autor=autoria.cod_autor):
            ...
            if autor.des_tipo_autor == "Parlamentar":
                for parlamentar in context.zsql.parlamentar_obter_zsql(
                    cod_parlamentar=autor.cod_parlamentar
                ):
                    dic_autor["nom_autor"] = parlamentar.nom_completo
            elif autor.des_tipo_autor == "Comissao":
                for comissao in context.zsql.comissao_obter_zsql(
    ...
    lst_des_iniciais = []
    for despacho in context.zsql.despacho_inicial_obter_zsql(
        ...
        for comissao in context.zsql.comissao_obter_zsql(
            ...
            dic_dados["nom_comissao"] = (
                comissao.nom_comissao + " - " + comissao.sgl_comissao
            )
            lst_des_iniciais.append(dic_dados)

    # #o bloco abaixo gera o dicionário de Tramitacoes(ln 87)
    dic_tramitacoes = {}
    for tramitacao in context.zsql.tramitacao_obter_zsql(
        cod_materia=REQUEST["cod_materia"], ind_ult_tramitacao=1
    ):
        ...
        for unidade in context.zsql.unidade_tramitacao_obter_zsql(
            cod_unid_tramitacao=tramitacao.cod_unid_tram_local
        ):
            # -----------------se unidade for comissao--------------------------
            if unidade.cod_orgao == None:
                for comissao in context.zsql.comissao_obter_zsql(
                    cod_comissao=unidade.cod_comissao
                ):
                    if tramitacao.cod_unid_tram_dest != None:
                        for unidade_dest in context.zsql.unidade_tramitacao_obter_zsql(
                            cod_unid_tramitacao=tramitacao.cod_unid_tram_dest
                        ):
                            # se unidade destino for comissao
                            if unidade_dest.cod_orgao == None:
                                for comissao_dest in context.zsql.comissao_obter_zsql(
                                    cod_comissao=unidade_dest.cod_comissao
                                ):
                                    dic_tramitacoes["unidade"] = comissao.nom_comissao
                                    dic_tramitacoes["destino"] = (
                                        comissao_dest.nom_comissao
                                    )
                            # se unidade destino for orgao
                            if unidade_dest.cod_comissao == None:
                                for orgao_dest in context.zsql.orgao_obter_zsql(
                                    cod_orgao=unidade_dest.cod_orgao
                                ):
                                    dic_tramitacoes["unidade"] = comissao.nom_comissao
    ...
    for relatoria in context.zsql.relatoria_obter_zsql(cod_materia=materia.cod_materia):
        for comissao in context.zsql.comissao_obter_zsql(
            cod_comissao=relatoria.cod_comissao
        ):
            for parlamentar in context.zsql.parlamentar_obter_zsql(
                cod_parlamentar=relatoria.cod_parlamentar
            ):
                ...
                if (
                    relatoria.tip_fim_relatoria == None
                    or relatoria.tip_fim_relatoria == "0"
                ):
                    num = 0
                    dic_comissao["motivo"] = ""
                else:
                    num = relatoria.tip_fim_relatoria
                    for tipo_fim in context.zsql.tipo_fim_relatoria_obter_zsql(
                        tip_fim_relatoria=num
                    ):
    ...
    for numeracao in context.zsql.numeracao_obter_zsql(cod_materia=materia.cod_materia):
        for tipo_materia in context.zsql.tipo_materia_legislativa_obter_zsql(
            tip_materia=numeracao.tip_materia
        ):
            dic_dados["nome"] = (
    ...
    lst_legis_citadas = []
    dic_dados = {}
    for legislacao in context.zsql.legislacao_citada_obter_zsql(
        cod_materia=materia.cod_materia
    ):
        ...
        dic_dados["nome_lei"] = (
            str(norma[0].tip_norma_sel)
            + " nº"
            + str(norma[0].num_norma)
            + " de"
            + str(norma[0].ano_norma)
        )
    ...
    lst_acessorios = []
    for documento in context.zsql.documento_acessorio_obter_zsql(
        cod_materia=materia.cod_materia
    ):
        dic_dados = {}
...
caminho = context.pdf_detalhe_materia_gerar(
    imagem,
    rodape,
    inf_basicas_dic,
    orig_externa_dic,
