...
def protocolos(lst_protocolos):
    ...
    for dic in lst_protocolos:
        ...
        if dic["txt_interessado"] != None:
            tmp_data += (
                '\t\t<para style="P2"><b>Interessado:</b> '
                + dic["txt_interessado"]
                + "</para>\n"
            )
        elif dic["nom_autor"] != None:
            tmp_data += (
                '\t\t<para style="P2"><b>Autor:</b> ' + dic["nom_autor"] + "</para>\n"
            )
        if dic["natureza"] != None:
            tmp_data += (
                '\t\t<para style="P2"><b>Natureza Processo:</b> '
                + dic["natureza"]
                + "</para>\n"
            )
        if dic["processo"] != None:
            tmp_data += (
                '\t\t<para style="P2"><b>Classificação:</b> '
                + dic["processo"]
                + "</para>\n"
            )
        if dic["data"] != None:
            tmp_data += (
                '\t\t<para style="P2"><b>Data Protocolo:</b> '
                + dic["data"]
                + "</para>\n"
            )
        if dic["anulado"] != "":
            tmp_data += (
                '\t\t<para style="P2"><b>** PROTOCOLO ANULADO **</b> ' "</para>\n"
            )
...
def principal(imagem, lst_protocolos, dic_cabecalho, lst_rodape):
    ...
    tmp_data += "\t\t\t</pageGraphics>\n"
    tmp_data += (
        '\t\t\t<frame id="first" x1="2cm" y1="3cm" width="17cm" height="21cm"/>\n'
    )
