...
if context.REQUEST["data"] != "":
    ...
    for sessao in context.zsql.sessao_plenaria_obter_zsql(
        dat_inicio_sessao=data, cod_sessao_plen=codigo, ind_excluido=0
    ):
        inf_basicas_dic = (
            {}
        )  # dicionário que armazenará as informacoes basicas da sessao plenaria
        # seleciona o tipo da sessao plenaria
        tipo_sessao = context.zsql.tipo_sessao_plenaria_obter_zsql(
            tip_sessao=sessao.tip_sessao, ind_excluido=0
        )[0]
        ...
        lst_mesa = []
        for composicao in context.zsql.composicao_mesa_sessao_obter_zsql(
            cod_sessao_plen=sessao.cod_sessao_plen, ind_excluido=0
        ):
            for parlamentar in context.zsql.parlamentar_obter_zsql(
                cod_parlamentar=composicao.cod_parlamentar, ind_excluido=0
            ):
                for cargo in context.zsql.cargo_mesa_obter_zsql(
                    cod_cargo=composicao.cod_cargo, ind_excluido=0
                ):
                    dic_mesa = {}
        ...
        lst_presenca_sessao = []
        for presenca in context.zsql.presenca_sessao_obter_zsql(
            cod_sessao_plen=sessao.cod_sessao_plen, ind_excluido=0
        ):
            for parlamentar in context.zsql.parlamentar_obter_zsql(
                cod_parlamentar=presenca.cod_parlamentar, ind_excluido=0
            ):
                dic_presenca = {}
        ...
        for tip_expediente in context.zsql.tipo_expediente_obter_zsql():
            for expediente in context.zsql.expediente_obter_zsql(
                cod_sessao_plen=sessao.cod_sessao_plen,
                cod_expediente=tip_expediente.cod_expediente,
                ind_excluido=0,
            ):
                dic_expedientes = {}
        ...
        for expediente_materia in context.zsql.votacao_expediente_materia_obter_zsql(
            dat_ordem=data, cod_sessao_plen=sessao.cod_sessao_plen, ind_excluido=0
        ):
            ...
            dic_expediente_materia["id_materia"] = (
                materia.sgl_tipo_materia
                + " "
                + materia.des_tipo_materia
                + " "
            ...
            if len(numeracao):
                ...
                dic_expediente_materia["des_numeracao"] = (
                    str(numeracao.num_materia) + "/" + str(numeracao.ano_materia)
                )
            ...
            dic_expediente_materia["ordem_observacao"] = (
                expediente_materia.ordem_observacao
            )
            ...
            if expediente_materia.tip_resultado_votacao:
                resultado = context.zsql.tipo_resultado_votacao_obter_zsql(
                    tip_resultado_votacao=expediente_materia.tip_resultado_votacao,
                    ind_excluido=0,
                )
                for i in resultado:
        ...
        for orador_expediente in context.zsql.oradores_expediente_obter_zsql(
            ...
            for parlamentar in context.zsql.parlamentar_obter_zsql(
                cod_parlamentar=orador_expediente.cod_parlamentar, ind_excluido=0
            ):
        ...
        for presenca_ordem_dia in context.zsql.presenca_ordem_dia_obter_zsql(
            cod_sessao_plen=sessao.cod_sessao_plen, ind_excluido=0
        ):
            for parlamentar in context.zsql.parlamentar_obter_zsql(
                cod_parlamentar=presenca_ordem_dia.cod_parlamentar, ind_excluido=0
            ):
        ...
        # votacoes
        lst_votacao = []
        for votacao in context.zsql.votacao_ordem_dia_obter_zsql(
            dat_ordem=data, cod_sessao_plen=sessao.cod_sessao_plen, ind_excluido=0
        ):

            # seleciona os detalhes de uma matéria
            materia = context.zsql.materia_obter_zsql(cod_materia=votacao.cod_materia)[
                0
            ]
        ...
        lst_oradores = []
        for orador in context.zsql.oradores_obter_zsql(
            cod_sessao_plen=sessao.cod_sessao_plen, ind_excluido=0
        ):
            for parlamentar in context.zsql.parlamentar_obter_zsql(
                cod_parlamentar=orador.cod_parlamentar, ind_excluido=0
            ):
                dic_oradores = {}
    ...
    REQUEST = context.REQUEST
    for local in context.zsql.localidade_obter_zsql(
        cod_localidade=casa["cod_localidade"]
    ):
    ...
    #    return lst_votacao
    sessao = session.id
    caminho = context.pdf_sessao_plenaria_gerar(
        rodape,
        sessao,
        imagem,
        inf_basicas_dic,
