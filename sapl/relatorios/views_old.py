...
from sapl.sessao.views.resumos import ResumoMixin
from sapl.settings import MEDIA_URL, STATIC_ROOT
...
def get_materias(mats):
    ...
    for materia in mats:
        ...
        dic["nom_autor"] = ", ".join([str(autor) for autor in materia.autores.all()])
        ...
        tramitacoes = Tramitacao.objects.filter(
            unidade_tramitacao_destino__isnull=True
...
def relatorio_materia(request):
    ...
    response["Content-Disposition"] = 'inline; filename="relatorio_materia.pdf"'
    ...
    kwargs = get_kwargs_params(
        request,
        ["numero", "ano", "autor", "tipo_autor", "relator", "interessado__icontains"],
    )
    ...
    pdf = pdf_materia_gerar.principal(imagem, materias, cabecalho, rodape)
...
def get_capa_processo(prot):
    ...
    for p in prot:
        ...
        dic["num_documento"] = ""
        for documento in DocumentoAdministrativo.objects.filter(numero=p.numero):
            dic["num_documento"] = str(documento.numero) + "/" + str(documento.ano)
        ...
        for materia_num in MateriaLegislativa.objects.filter(
            ...
            for numera in Numeracao.objects.filter(materia=materia_num):
                # FIXME i18n
                dic["numeracao"] = (
                    "PROCESSO N&#176; " + str(numera.numero) + "/" + str(numera.ano)
                )
        dic["anulado"] = ""
        if p.anulado == 1:
            dic["anulado"] = "Nulo"
...
def relatorio_capa_processo(request):
    ...
    response = HttpResponse(content_type="application/pdf")
    response["Content-Disposition"] = 'inline; filename="relatorio_processo.pdf"'
    ...
    pdf = pdf_capa_processo_gerar.principal(imagem, protocolos_pdf, cabecalho, rodape)
...
def relatorio_ordem_dia(request):
    ...
    response["Content-Disposition"] = 'inline; filename="relatorio_ordem_dia.pdf"'
    ...
    pdf = pdf_ordem_dia_gerar.principal(imagem, ordem_pdf, cabecalho, rodape)
...
def relatorio_documento_administrativo(request):
    ...
    pdf = pdf_documento_administrativo_gerar.principal(
        imagem, doc_pdf, cabecalho, rodape
    )
    response.write(pdf)
...
def get_documento_administrativo(docs):
    ...
    for d in docs:
        ...
        for t in TramitacaoAdministrativo.objects.filter(documento=d, ultima=True):
...
def relatorio_espelho(request):
    ...
    response = HttpResponse(content_type="application/pdf")
    response["Content-Disposition"] = 'inline; filename="relatorio_espelho.pdf"'
    ...
    pdf = pdf_espelho_gerar.principal(imagem, mat_pdf, cabecalho, rodape)
    response.write(pdf)
...
def get_sessao_plenaria(sessao, casa):
    ...
    inf_basicas_dic["dat_inicio_sessao"] = sessao.data_inicio.strftime("%d/%m/%Y")
    ...
    for composicao in IntegranteMesa.objects.filter(sessao_plenaria=sessao):
        for parlamentar in Parlamentar.objects.filter(id=composicao.parlamentar.id):
    ...
    lst_presenca_sessao = []
    presenca = SessaoPlenariaPresenca.objects.filter(sessao_plenaria=sessao).order_by(
    ...
    lst_ausencia_sessao = []
    ausencia = JustificativaAusencia.objects.filter(sessao_plenaria=sessao).order_by(
        "parlamentar__nome_parlamentar"
    )
    ...
    lst_expedientes = []
    expedientes = ExpedienteSessao.objects.filter(sessao_plenaria=sessao).order_by(
        "tipo__nome"
    )
    ...
    for expediente_materia in ExpedienteMateria.objects.filter(
        sessao_plenaria=sessao, parent__isnull=True
        ...
        dic_expediente_materia["epigrafe_materia"] = (
            materia.tipo.sigla
            + " "
            +
            # materia.tipo.descricao + ' '
        ...
        dic_expediente_materia["num_autores"] = (
            "Autores" if len(autoria) > 1 else "Autor"
        )
        if autoria:
            for a in autoria:
                if a.autor.nome:
        ...
        if expediente_materia.tipo_votacao == AbstractOrdemDia.LEITURA:
    ...
    materias_expediente_votacao_nominal = ExpedienteMateria.objects.filter(
        sessao_plenaria=sessao, tipo_votacao=AbstractOrdemDia.NOMINAL
    ).order_by("-materia")
    ...
    for orador_expediente in OradorExpediente.objects.filter(
        ...
        dic_oradores_expediente["nom_parlamentar"] = parlamentar.nome_parlamentar
    ...
    for parlamentar in [p.parlamentar for p in presenca_ordem_dia]:
        ...
        dic_presenca_ordem_dia["nom_parlamentar"] = parlamentar.nome_parlamentar
    ...
    for votacao in OrdemDia.objects.filter(
        ...
        if resultados:
            ...
            dic_votacao["nom_resultado"] = "Matéria não {}".format(
                "Lida" if votacao.tipo_votacao == AbstractOrdemDia.LEITURA else "Votada"
            )
    ...
    for modvn in materias_ordem_dia_votacao_nominal:
        ...
        if registro_od:
            for vp_od in VotoParlamentar.objects.filter(votacao=registro_od).order_by(
                "parlamentar"
            ):
    ...
    oradores_ordem_dia = OradorOrdemDia.objects.filter(sessao_plenaria=sessao).order_by(
        "numero_ordem"
    )
    ...
    lst_oradores = []
    for orador in Orador.objects.filter(sessao_plenaria=sessao).order_by(
        "numero_ordem"
    ):
        ...
        if tramitacao.turno:
            for t in Tramitacao.TURNO_CHOICES:
                if t[0] == tramitacao.turno:
                    ...
                    break
        descricao_tramitacao = (
            tramitacao.status.descricao if tramitacao.status else "Não informada"
        )
...
def relatorio_sessao_plenaria(request, pk):
    ...
    try:
        logger.debug(
            "user=" + username + ". Tentando obter SessaoPlenaria com id={}.".format(pk)
        )
        ...
        logger.error(
            "user="
            + username
            + ". Essa SessaoPlenaria não existe (pk={}). ".format(pk)
            + str(e)
        )
...
def get_protocolos(prots):
    ...
    for protocolo in prots:
        ...
        if protocolo.timestamp:
            dic["data"] = (
                ts.strftime("%d/%m/%Y") + " - <b>Horário:</b>" + ts.strftime("%H:%m")
            )
        else:
            dic["data"] = (
                protocolo.data.strftime("%d/%m/%Y")
                + " - <b>Horário:</b>"
...
def relatorio_protocolo(request):
    ...
    pdf = pdf_protocolo_gerar.principal(imagem, protocolo_data, cabecalho, rodape)
...
def relatorio_etiqueta_protocolo(request, nro, ano):
    ...
    pdf = pdf_etiqueta_protocolo_gerar.principal(
        imagem, protocolo_data, cabecalho, rodape
    )
...
def get_etiqueta_protocolos(prots):
    ...
    for p in prots:
        ...
        if p.timestamp:
            ...
            dic["data"] = (
                "<b>Data: </b>"
                + tz_hora.strftime("%d/%m/%Y")
                + " - <b>Horário: </b>"
                + tz_hora.strftime("%H:%M")
            )
        else:
            dic["data"] = (
                "<b>Data: </b>"
        ...
        dic["num_documento"] = ""
        for documento in DocumentoAdministrativo.objects.filter(protocolo=p):
            dic["num_documento"] = (
                documento.tipo.sigla
                + " "
        ...
        dic["processo"] = str(p.tipo_materia) or str(p.tipo_documento)
...
def relatorio_pauta_sessao_trml2pdf__deprecated(request, pk):
    ...
    pdf = pdf_pauta_sessao_gerar.principal(
        rodape,
        imagem,
        inf_basicas_dic,
        lst_expediente_materia,
...
def get_pauta_sessao(sessao, casa):
    ...
    for expediente_materia in ExpedienteMateria.objects.filter(
        ...
        materia = MateriaLegislativa.objects.filter(
            id=expediente_materia.materia.id
        ).first()

        dic_expediente_materia = {}
        dic_expediente_materia["tipo_materia"] = (
            materia.tipo.sigla + " - " + materia.tipo.descricao
        )
        dic_expediente_materia["num_ordem"] = str(expediente_materia.numero_ordem)
        dic_expediente_materia["id_materia"] = materia.id
        dic_expediente_materia["epigrafe_materia"] = (
            materia.tipo.sigla + " " + str(materia.numero) + "/" + str(materia.ano)
        )
        ...
        dic_expediente_materia["ordem_observacao"] = str(expediente_materia.observacao)
        ...
        if tram_expediente:
            ...
            tramitacao = (
                f"<b>{tram_expediente.status}</b><br><em>{tram_expediente.texto}</em>"
            )
    ...
    for votacao in OrdemDia.objects.filter(
        ...
        materia = MateriaLegislativa.objects.filter(id=votacao.materia.id).first()
        dic_votacao = {}
        dic_votacao["tipo_materia"] = (
            materia.tipo.sigla + " - " + materia.tipo.descricao
        )
        dic_votacao["num_ordem"] = votacao.numero_ordem
        dic_votacao["id_materia"] = materia.id
    ...
    expediente = ExpedienteSessao.objects.filter(sessao_plenaria_id=sessao.id)
    expedientes = []
    for e in expediente:
        tipo = e.tipo
        ...
        conteudo = re.sub("&nbsp;", " ", strip_tags(conteudo))
...
class RelatorioPautaSessao(TemplateView):
    ...
    def render_to_response(self, context):
        ...
        for s in d.sections:
            ...
            bh = p.add_run()
            bh.add_picture(settings.FRONTEND_BRASAO_PATH["1024"], width=Cm(3))
        ...
        d.add_paragraph("first item in unordered list", style="List Bullet")
        d.add_paragraph("first item in ordered list", style="List Number")
        ...
        d.save(
            f"/home/leandrojatai/Downloads/portalcmj_{self.request.resolver_match.url_name}.docx"
        )
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="portalcmj_{self.request.resolver_match.url_name}.docx"'
        )
