...
TipoDispositivoCrud = CrudAux.build(TipoDispositivo, "tipo_dispositivo")
    ...
    class DetailView(CrudAux.DetailView):
        ...
        def hook_urlize_referencias(self, obj, **kwargs):
            ...
            for ur in obj.urlize_referencias.all():
                url = reverse(
                    "sapl.compilacao:urlizereferencia_detail", kwargs={"pk": ur.pk}
                )
                items.append(f'<li><a href="{url}">{ur.chave}</a></li>')
            items.append("</ul>")
            items = "".join(items)
...
class UrlizeReferenciaCrud(CrudAux):
        ...
        def form_valid(self, form):
            ...
            try:
                ...
                if "compilacao_urlizereferencia_chave_key" in str(e):
                    form.add_error("chave", _("Chave já existe."))
            ...
            UrlizeReferencia.objects.filter(url__exact=urlize.url).update(
                citacao=urlize.citacao
            )
    ...
    class DetailView(CrudAux.DetailView):
        ...
        def hook_normas(self, obj, **kwargs):
            ds = Dispositivo.objects.filter(texto__icontains=obj.chave).order_by(
                "ta", "ordem"
            )
        ...
        try:
            ...
            print(
                format_lazy(
                    "{} {}",
                    _(
                        "Ocorreu erro na importação do arquivo base dos Tipos de"
    ...
    def ta(self):
        try:
            ...
            if tipo_norma not in ("LOM", "RI"):
                self.normajuridica = (
                    NormaJuridica.objects.filter(**params).order_by("-ano").first()
                )
...
class PublicacaoListView(PublicacaoMixin, ListView):
    ...
    def title(self):
        return _(
            "%s <small>(%s)</small>" % (self.model._meta.verbose_name_plural, self.ta)
        )
...
class TextView(CompMixin, ListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.object:
            return redirect(
                reverse(
                    ...
                    kwargs={"pk": self.normajuridica.id},
                )
            )
        ...
        if perm is None:
            ...
            messages.error(
                ...
                _(
                    ...
                    """.format(
                        ...
                        """
                            if self.object.content_object.texto_integral
                            else ""
                        ),
                    )
                ),
            ...
            return redirect(
                reverse(
                    "{}:{}_detail".format(
                        co._meta.app_config.name, co._meta.model_name
                    ),
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.is_reader():
            ...
            if not sign and request.path.startswith("/ta/"):
                ...
                if (
                    content_object
                    and content_object._meta.model_name == "normajuridica"
                ):
            ...
            if self.object.is_cached(sign=sign):
                ...
                context["normas_relacionadas"] = (
                    self.object.content_object.get_normas_relacionadas()
                )
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        if self.is_reader() and self.object.is_cached(sign=sign):
            embedded_cache = media_cache_storage.open(
                self.object.get_path_cache(sign=sign)
            ).read()
    ...
    def get_context_data(self, **kwargs):
        ...
        notas = Nota.objects.filter(dispositivo__ta_id=self.object.id).select_related(
            "owner", "tipo"
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user.is_superuser:
            if "inverter_clone_original" in request.GET:
                ...
                return redirect(
                    to=reverse_lazy(
                        "sapl.compilacao:ta_text", kwargs={"ta_id": clone.id}
                    )
                )
        ...
        if self.object.editing_locked:
            ...
            if "lock" in request.GET or "check" in request.GET:
                ...
                if "lock" in request.GET:
                    ...
                    for d in self.object.dispositivos_alterados_pelo_ta_set.order_by(
                        "ta_id"
                    ).distinct("ta_id"):
                        d.ta.clear_cache()
    ...
    def dispositivos_list(self):
        ...
        dpts = {
            d.pk: {
                ...
                "na": (
                    self.nota_alteracao(d, lista_ta_publicado)
                    if d.ta_id == ta_id
                    else None
                ),
            }
                ...
                if pai_base:
                    ...
                    for d in dcc:  # ultimo DCC do tipo encontrado
                        ...
                        if d.tipo_dispositivo.class_css not in base_adicao:
                            ...
                            base_adicao[d.tipo_dispositivo.class_css] = (
            ...
            result = [
                {
                    "tipo_insert": force_str(
                        format_lazy(
                            "{} {}", _("Inserir Após"), base.tipo_dispositivo.nome
                        )
                    ),
                    "icone": "&#8631;&nbsp;",
                ...
                },
                {
                    "tipo_insert": force_str(
                        format_lazy(
                            "{} {}", _("Inserir em"), base.tipo_dispositivo.nome
                        )
                    ),
                    "icone": "&#8690;&nbsp;",
                    "action": "json_add_in",
    ...
    def get_queryset(self):
        ...
        try:
            ...
            if texto_articulado_do_editor:
                try:
                    ta = TextoArticulado.objects.get(id=texto_articulado_do_editor)
                except:
                    ta = None
...
class DispositivoSearchModalView(FormView):
    ...
    def get_initial(self):
        ...
        initial.update(
            {"texto_articulado_do_editor": self.request.GET.get("ta_base_id", 0)}
        )
