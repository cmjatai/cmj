...
urlpatterns_compilacao = [
    ...
    re_path(r"^(?P<pk>[0-9]+)/edit$", views.TaUpdateView.as_view(), name="ta_edit"),
    re_path(r"^(?P<pk>[0-9]+)/delete$", views.TaDeleteView.as_view(), name="ta_delete"),
    re_path(r"^(?P<ta_id>[0-9]+)/text$", views.TextView.as_view(), name="ta_text"),
    re_path(
        r"^(?P<ta_id>[0-9]+)/text/vigencia/(?P<sign>.+)/$",
...
urlpatterns = [
    ...
    re_ipath(
        r"^(?P<tipo_norma>(LOM|RI))$",
        views.TextView.as_view(),
        name="slug_ta_text_letters",
    ),
    re_ipath(
        r"^(?P<tipo_norma>(L|ATG|DL|DE|ELO|LC|LEI|LOM|LE|PLE|PR|RI|RES))(?P<numero_norma>[0-9]*)-?(?P<ano_norma>[0-9]*)?-?(?P<sufixo_urlize>[A-Za-z]*)$",
        views.TextView.as_view(),
        name="slug_ta_text",
    ),
    re_path(r"^sistema/ta/config/tipo-nota", include(TipoNotaCrud.get_urls())),
    re_path(r"^sistema/ta/config/tipo-vide", include(TipoVideCrud.get_urls())),
    re_path(
        r"^sistema/ta/config/tipo-publicacao", include(TipoPublicacaoCrud.get_urls())
    ),
    re_path(
        r"^sistema/ta/config/veiculo-publicacao",
        include(VeiculoPublicacaoCrud.get_urls()),
    ),
    re_path(r"^sistema/ta/config/tipo", include(TipoTextoArticuladoCrud.get_urls())),
    re_path(r"^sistema/ta/config/urlize", include(UrlizeReferenciaCrud.get_urls())),
    re_path(r"^sistema/ta/config/citacao", include(CitacaoDeReferenciaCrud.get_urls())),
]
