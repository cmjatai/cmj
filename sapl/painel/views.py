...
def votacao_aberta(request):
    ...
    if len(votacoes_abertas) > 1:
        ...
        logger.info(
            "user="
            + username
            + ". Existe mais de uma votações aberta. Elas se encontram "
        ...
        ordens = OrdemDia.objects.filter(
            sessao_plenaria=votacoes_abertas.first(), votacao_aberta=True
        )
...
def votacao(context, context_vars):
    ...
    if parlamentar.id in context_vars["presentes"]:
        ...
        context.update(
            {"error_message": "Não há presentes na Sessão com a " "matéria em votação."}
        )

    if parlamentar_presente:
        voto = []
        if context_vars["ordem_dia"]:
            voto = VotoParlamentar.objects.filter(ordem=context_vars["ordem_dia"])
        elif context_vars["expediente"]:
            voto = VotoParlamentar.objects.filter(expediente=context_vars["expediente"])

        if voto:
            try:
                logger.debug(
                    "Tentando obter objeto VotoParlamentar com parlamentar={}.".format(
                ...
                logger.error(
                    "Voto do parlamentar {} não computado.".format(
                        context_vars["parlamentar"]
                    )
                )
        ...
        logger.error(
            "Parlamentar com id={} não está presente na "
            "Ordem do Dia/Expediente em votação.".format(parlamentar.id)
        )
        context.update(
            {
                "error_message": "Você não está presente na "
...
def sessao_votacao(context, context_vars):
    ...
    errors_msgs = {
        "materia": "Não há nenhuma matéria aberta.",
    ...
    if ordem_dia:
        ...
        presentes = (
            PresencaOrdemDia.objects.filter(sessao_plenaria_id=pk)
        ...
        presentes = (
            SessaoPlenariaPresenca.objects.filter(sessao_plenaria_id=pk)
            .values_list("parlamentar_id", flat=True)
            .distinct()
        )

    context_vars.update(
        {"ordem_dia": ordem_dia, "expediente": expediente, "presentes": presentes}
    )
    ...
    if not erro:
        context.update(
            {"materia": materia_aberta.materia, "ementa": materia_aberta.materia.ementa}
        )
...
def can_vote(context, context_vars, request):
    ...
    if sessao and not msg:
        ...
        context.update({"error_message": "Não há nenhuma sessão com matéria aberta."})
    return context, context_vars


def votante_view(request):
    logger = logging.getLogger(__name__)
    username = request.user.email if request.user.is_authenticated else "AnonymousUser"
    ...
    if "parlamentares.can_vote" in request.user.get_all_permissions():
        ...
        logger.debug(
            "user="
            + username
            + ". Verificando se usuário {} possui permissão para votar.".format(
                request.user
            )
        )
    else:
        ...
        context.update(
            {"permissao": False, "error_message": "Usuário sem permissão para votar."}
        )
        ...
        if context_vars["ordem_dia"]:
            try:
                logger.info(
                    "user="
                    + username
                    + ". Tentando obter objeto VotoParlamentar para parlamentar={} e ordem={}.".format(
                        context_vars["parlamentar"], context_vars["ordem_dia"]
                    )
                )
                ...
                logger.error(
                    "user="
                    + username
                    + ". Erro ao obter VotoParlamentar para parlamentar={} e ordem={}. Criando objeto.".format(
                        context_vars["parlamentar"], context_vars["ordem_dia"]
                    )
                )
                ...
                logger.info(
                    "user="
                    + username
                    + ". VotoParlamentar para parlamentar={} e ordem={} obtido com sucesso.".format(
                        context_vars["parlamentar"], context_vars["ordem_dia"]
                    )
                )
            ...
            try:
                logger.info(
                    "user="
                    + username
                    + ". Tentando obter objeto VotoParlamentar para parlamentar={} e expediente={}.".format(
                        context_vars["parlamentar"], context_vars["expediente"]
                    )
                )
                voto = VotoParlamentar.objects.get(
                    parlamentar=context_vars["parlamentar"],
                ...
                logger.error(
                    "user="
                    + username
                    + ". Erro ao obter VotoParlamentar para parlamentar={} e expediente={}. Criando objeto.".format(
                        context_vars["parlamentar"], context_vars["expediente"]
                    )
                )
                voto = VotoParlamentar.objects.create(
                ...
                logger.info(
                    "user="
                    + username
                    + ". VotoParlamentar para parlamentar={} e expediente={} obtido com sucesso.".format(
                        context_vars["parlamentar"], context_vars["expediente"]
                    )
                )
                voto.voto = request.POST["voto"]
...
def get_cronometro_status(request, name):
    ...
    try:
        ...
        logger.error(
            "user="
            + username
            + ". Erro ao obter cronometro. Retornado como vazio. "
            + str(e)
        )
        cronometro = ""
    return cronometro
...
def get_presentes(pk, response, materia):
    ...
    oradores = OradorExpediente.objects.filter(sessao_plenaria_id=pk).order_by(
        "numero_ordem"
    )
    ...
    for o in oradores:
        ...
        oradores_list.append(
            {"nome": o.parlamentar.nome_parlamentar, "numero": o.numero_ordem}
        )
...
def response_nenhuma_materia(response):
    response.update({"msg_painel": str(_("Nenhuma matéria disponivel para votação."))})
...
def get_votos(response, materia, mostrar_voto):
    ...
    if type(materia) == OrdemDia:
        if materia.tipo_votacao != 4:
            registro = (
                RegistroVotacao.objects.filter(ordem=materia, materia=materia.materia)
                .order_by("data_hora")
                .last()
            )
            leitura = None
            ...
            leitura = (
                RegistroLeitura.objects.filter(ordem=materia, materia=materia.materia)
                .order_by("data_hora")
                .last()
            )
            registro = None
        ...
        if materia.tipo_votacao != 4:
            registro = (
                RegistroVotacao.objects.filter(
                    expediente=materia, materia=materia.materia
                )
                .order_by("data_hora")
                .last()
            )
    ...
    if not registro and not leitura:
        ...
        if materia.tipo_votacao == 2:
            if tipo == "ordem":
                ...
                votos_parlamentares = VotoParlamentar.objects.filter(
                    expediente_id=materia.id
                ).order_by("parlamentar__nome_parlamentar")

            for i, p in enumerate(response["presentes"]):
                try:
                    logger.info(
                        "Tentando obter votos do parlamentar (id={}).".format(
                            p["parlamentar_id"]
                        )
                    )
                    voto = votos_parlamentares.get(
        ...
        if materia.tipo_votacao == 2:
            ...
            for i, p in enumerate(response["presentes"]):
                try:
                    ...
                    logger.error(
                        "Votos do parlamentar (id={}) não encontrados. Retornado None.".format(
                            p["parlamentar_id"]
                        )
                    )
...
def get_dados_painel(request, pk):
    ...
    last_ordem_voto = (
        RegistroVotacao.objects.filter(ordem__sessao_plenaria=sessao)
        .order_by("data_hora")
        .last()
    )
    last_expediente_voto = (
        RegistroVotacao.objects.filter(expediente__sessao_plenaria=sessao)
        .order_by("data_hora")
        .last()
    )

    last_ordem_leitura = (
        RegistroLeitura.objects.filter(ordem__sessao_plenaria=sessao)
        .order_by("data_hora")
        .last()
    )
    last_expediente_leitura = (
        RegistroLeitura.objects.filter(expediente__sessao_plenaria=sessao)
        .order_by("data_hora")
        .last()
    )
    ...
    if (
        last_expediente_voto
        and ultimo_timestamp
        and last_expediente_voto.data_hora > ultimo_timestamp
    ) or (not ultimo_timestamp and last_expediente_voto):
    ...
    ) or (not ultimo_timestamp and last_ordem_leitura):
    ...
    ) or (not ultimo_timestamp and last_expediente_leitura):
