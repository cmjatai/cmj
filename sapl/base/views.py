...
from cmj.core.models import groups_add_user, groups_remove_user
from sapl import settings
from sapl.audiencia.models import AudienciaPublica, TipoAudienciaPublica
from sapl.base.forms import AutorForm, OperadorAutorForm, TipoAutorForm
from sapl.base.models import Autor, OperadorAutor, TipoAutor
from sapl.comissoes.models import Comissao, Reuniao
from sapl.crud.base import CrudAux, MasterDetailCrud, make_pagination
...
def filtra_url_materias_em_tramitacao(qr, qs, campo_url, local_ou_status):
    ...
    if local_ou_status == "local":
        id_materias = [
            item.id
            for item in qs
            if item.tramitacao_set.order_by("-id").first().unidade_tramitacao_destino_id
            == int(filtro_url)
        ]
        ...
        id_materias = [
            item.id
            for item in qs
            if item.tramitacao_set.order_by("-id").first().status_id == int(filtro_url)
        ]
...
class AutorCrud(CrudAux):
    ...
    class UpdateView(CrudAux.UpdateView):
        ...
        def get_success_url(self):
            ...
            url_reverse = reverse("sapl.base:autor_detail", kwargs={"pk": pk_autor})
            ...
            try:
                self.logger.debug(
                    "user={}. Enviando email na edição " "de Autores.".format(username)
                )
                kwargs = {}
                ...
                self.logger.error(
                    "user={}. Erro no envio de email na edição de"
...
class RelatoriosListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["estatisticas_acesso_normas"] = (
            True if estatisticas_acesso_normas == "S" else False
        )
...
class RelatorioDocumentosAcessoriosView(FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(RelatorioDocumentosAcessoriosView, self).get_context_data(
            **kwargs
        )
        ...
        context["periodo"] = data_inicial + " - " + data_final
...
class RelatorioAtasView(FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(RelatorioAtasView, self).get_context_data(**kwargs)
...
class RelatorioHistoricoTramitacaoView(FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(RelatorioHistoricoTramitacaoView, self).get_context_data(
            **kwargs
        )
        ...
        context["show_results"] = show_results_filter_set(qr)
        context["data_tramitacao"] = (
            self.request.GET["tramitacao__data_tramitacao_0"]
            + " - "
        ...
        if self.request.GET["tramitacao__status"]:
            ...
            context["tramitacao__status"] = str(
                StatusTramitacao.objects.get(id=tramitacao_status)
            )
        else:
            context["tramitacao__status"] = ""

        if self.request.GET["tramitacao__unidade_tramitacao_local"]:
            context["tramitacao__unidade_tramitacao_local"] = str(
        ...
        if self.request.GET["tramitacao__unidade_tramitacao_destino"]:
            context["tramitacao__unidade_tramitacao_destino"] = str(
                UnidadeTramitacao.objects.get(
...
class RelatorioDataFimPrazoTramitacaoView(FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(RelatorioDataFimPrazoTramitacaoView, self).get_context_data(
            **kwargs
        )
...
class RelatorioReuniaoView(FilterView):
    ...
    def get_filterset_kwargs(self, filterset_class):
        super(RelatorioReuniaoView, self).get_filterset_kwargs(filterset_class)

        kwargs = {"data": self.request.GET or None}
        return kwargs

    def get_context_data(self, **kwargs):
        context = super(RelatorioReuniaoView, self).get_context_data(**kwargs)
...
class RelatorioAudienciaView(FilterView):
    ...
    def get_filterset_kwargs(self, filterset_class):
        super(RelatorioAudienciaView, self).get_filterset_kwargs(filterset_class)

        kwargs = {"data": self.request.GET or None}
        return kwargs

    def get_context_data(self, **kwargs):
        context = super(RelatorioAudienciaView, self).get_context_data(**kwargs)
...
class RelatorioMateriasTramitacaoView(FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(RelatorioMateriasTramitacaoView, self).get_context_data(
            **kwargs
        )
        ...
        li = [
            li1
            for li1 in qs
            if li1.tramitacao_set.first()
            and li1.tramitacao_set.first().status.indicador != "F"
        ]
...
class RelatorioMateriasPorAnoAutorTipoView(FilterView):
    ...
    def get_materias_autor_ano(self, ano, primeiro_autor):
        ...
        autorias = (
            Autoria.objects.filter(materia__ano=ano, primeiro_autor=primeiro_autor)
            .values("autor", "materia__tipo__sigla", "materia__tipo__descricao")
            .annotate(total=Count("materia__tipo"))
            .order_by("autor", "materia__tipo")
        )

        autores_ids = set([i["autor"] for i in autorias])

        autores = dict((a.id, a) for a in Autor.objects.filter(id__in=autores_ids))
    ...
    def get_filterset_kwargs(self, filterset_class):
        super(RelatorioMateriasPorAnoAutorTipoView, self).get_filterset_kwargs(
            filterset_class
        )
    ...
    def get_context_data(self, **kwargs):
        context = super(RelatorioMateriasPorAnoAutorTipoView, self).get_context_data(
            **kwargs
        )
...
class RelatorioNormasPublicadasMesView(FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(RelatorioNormasPublicadasMesView, self).get_context_data(
            **kwargs
        )
...
class RelatorioNormasVigenciaView(FilterView):
    ...
    def get_filterset_kwargs(self, filterset_class):
        super(RelatorioNormasVigenciaView, self).get_filterset_kwargs(filterset_class)
        ...
        if kwargs["data"]:
            ...
            if vigencia == "True":
                ...
                qs = (
                    qs_dt_not_null
                    | qs.filter(data_vigencia__gte=datetime.datetime.now().date())
                ).distinct()
            else:
    ...
    def get_context_data(self, **kwargs):
        context = super(RelatorioNormasVigenciaView, self).get_context_data(**kwargs)
...
class ListarLegislaturaInfindavelView(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        page_obj = context["page_obj"]
        context["page_range"] = make_pagination(page_obj.number, paginator.num_pages)
        context["NO_ENTRIES_MSG"] = "Nenhuma encontrado."
        return context
...
def get_estatistica(request):
    ...
    datas = [
        ...
        .values_list("data_ultima_atualizacao", flat=True)
        .first(),
    ]  # Retorna [None, None] se inexistem registros
...
class PesquisarUsuarioView(PermissionRequiredMixin, FilterView):
    ...
    def get_filterset_kwargs(self, filterset_class):
        super(PesquisarUsuarioView, self).get_filterset_kwargs(filterset_class)
...
class DeleteUsuarioView(PermissionRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        try:
            ...
            error_url = reverse_lazy(
                "sapl.base:user_delete", kwargs={"pk": self.kwargs["pk"]}
            )
            error_message = (
                "O usuário não pode ser removido, pois é referenciado por:<br><ul>"
            )

            for e in exception.protected_objects:
                error_message += "<li>{} - {}</li>".format(e._meta.verbose_name, e)
            error_message += "</ul>"
    ...
    @property
    def cancel_url(self):
        return reverse("cmj.core:user_update", kwargs={"pk": self.kwargs["pk"]})
...
class RelatorioHistoricoTramitacaoAdmView(FilterView):
    ...
    def get_context_data(self, **kwargs):
        context = super(RelatorioHistoricoTramitacaoAdmView, self).get_context_data(
            **kwargs
        )
        ...
        context["show_results"] = show_results_filter_set(qr)
        context["data_tramitacao"] = (
            self.request.GET["tramitacaoadministrativo__data_tramitacao_0"]
            + " - "
            + self.request.GET["tramitacaoadministrativo__data_tramitacao_1"]
        )
        if self.request.GET["tipo"]:
            tipo = self.request.GET["tipo"]
            context["tipo"] = str(TipoDocumentoAdministrativo.objects.get(id=tipo))
        else:
            context["tipo"] = ""

        if self.request.GET["tramitacaoadministrativo__status"]:
            tramitacao_status = self.request.GET["tramitacaoadministrativo__status"]
            context["tramitacaoadministrativo__status"] = str(
                StatusTramitacaoAdministrativo.objects.get(id=tramitacao_status)
            )
        else:
            context["tramitacaoadministrativo__status"] = ""

        if self.request.GET["tramitacaoadministrativo__unidade_tramitacao_local"]:
            context["tramitacaoadministrativo__unidade_tramitacao_local"] = str(
                UnidadeTramitacao.objects.get(
                    id=self.request.GET[
        ...
        if self.request.GET["tramitacaoadministrativo__unidade_tramitacao_destino"]:
            context["tramitacaoadministrativo__unidade_tramitacao_destino"] = str(
                UnidadeTramitacao.objects.get(
                    id=self.request.GET[
...
class RelatorioNormasPorAutorView(FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.GET["tipo"]:
            ...
            context["tipo"] = str(TipoNormaJuridica.objects.get(id=tipo))
        else:
            context["tipo"] = ""
        ...
        if self.request.GET["autorianorma__autor"]:
            ...
            context["autor"] = ""
        context["periodo"] = (
            self.request.GET["data_0"] + " - " + self.request.GET["data_1"]
        )
    ...
    class CreateView(MasterDetailCrud.CreateView):
        ...
        def form_valid(self, form):
            ...
            oper = OperadorAutor.objects.filter(
                user_id=self.object.user_id, autor_id=self.object.autor_id
            ).exists()

            if oper:
                form._errors["user"] = ErrorList(
                    [_("Este Operador já está registrado " "para este Autor.")]
                )
                return self.form_invalid(form)

            oper = (
                OperadorAutor.objects.filter(user_id=self.object.user_id)
                .exclude(autor_id=self.object.autor_id)
                .exists()
            )

            if oper:
                form._errors["user"] = ErrorList(
                    [_("Este Operador já está registrado " "para outro Autor.")]
                )
                return self.form_invalid(form)
