...
class TipoSessaoPlenaria(models.Model):
    ...
    quorum_minimo = models.PositiveIntegerField(verbose_name=_("Quórum mínimo"))

    tipo_numeracao = models.PositiveIntegerField(
        verbose_name=_("Tipo de Numeração"), choices=TIPO_NUMERACAO_CHOICES, default=11
    )

    tipogeral = models.PositiveIntegerField(
        verbose_name=_("Tipo Geral"), choices=TIPOGERAL_CHOICES, default=10
    )

    gera_selo_votacao = models.BooleanField(
        blank=True, default=True, verbose_name=_("Gera Selo de Votação?")
    )
...
class SessaoPlenaria(models.Model):
    ...
    painel_aberto = models.BooleanField(
        blank=True, default=False, verbose_name=_("Painel está aberto?")
    )

    titulo = models.CharField(
        max_length=256, verbose_name=_("Título"), blank=True, default=""
    )

    tipo = models.ForeignKey(
        TipoSessaoPlenaria, on_delete=models.PROTECT, verbose_name=_("Tipo")
    )
    ...
    selo_votacao_adicionado = models.BooleanField(
        null=True,
        blank=True,
        choices=YES_NO_CHOICES,
    ...
    _videoparte = SaplGenericRelation(
        ...
        fields_search=(("video__titulo", "__icontains"),),
    )
    ...
    def __str__(self):
        ...
        if self.tipo.tipo_numeracao <= tnc.mensal:
            base += " do mês de {}".format(formats.date_format(self.data_inicio, "F"))
        ...
        if self.tipo.tipo_numeracao <= tnc.sessao_legislativa:
            base += " da {}ª Sessão Legislativa".format(self.sessao_legislativa.numero)
    ...
    def str_subtitle(self):
        ...
        base = ""
        if self.tipo.tipo_numeracao <= tnc.sessao_legislativa:
            base = "{}ª Sessão Legislativa".format(self.sessao_legislativa.numero)
    ...
    def save(
        ...
        self.ordemdia_set.exclude(data_ordem=F("sessao_plenaria__data_inicio")).update(
            data_ordem=self.data_inicio
        )
...
class AbstractOrdemDia(models.Model):
    ...
    sessao_plenaria = models.ForeignKey(SessaoPlenaria, on_delete=models.CASCADE)
    materia = models.ForeignKey(
        MateriaLegislativa, on_delete=models.PROTECT, verbose_name=_("Matéria")
    )
    ...
    votacao_aberta_pedido_prazo = models.BooleanField(
        null=True,
        blank=True,
        choices=YES_NO_CHOICES,
...
class TipoExpediente(models.Model):
    ...
    ordenacao = models.PositiveIntegerField(
        blank=True, null=True, verbose_name=_("Ordenação")
    )
...
class ExpedienteSessao(models.Model):  # ExpedienteSessaoPlenaria
    sessao_plenaria = models.ForeignKey(
        SessaoPlenaria, on_delete=models.CASCADE, related_name="expedientesessao_set"
    )
    tipo = models.ForeignKey(TipoExpediente, on_delete=models.PROTECT)
    conteudo = models.TextField(blank=True, verbose_name=_("Conteúdo do expediente"))
...
class IntegranteMesa(models.Model):  # MesaSessaoPlenaria
    sessao_plenaria = models.ForeignKey(
        SessaoPlenaria, related_name="integrantemesa", on_delete=models.CASCADE
    )
...
class AbstractOrador(models.Model):  # Oradores
    ...
    parlamentar = models.ForeignKey(
        Parlamentar, on_delete=models.PROTECT, verbose_name=_("Parlamentar")
    )
...
class RegistroVotacao(models.Model):
    ...
    expediente = models.ForeignKey(
        ExpedienteMateria, blank=True, null=True, on_delete=models.CASCADE
    )
    ...
    numero_abstencoes = models.PositiveIntegerField(verbose_name=_("Abstenções"))
    ...
    user = models.ForeignKey(
        get_settings_auth_user_model(), on_delete=models.PROTECT, null=True, blank=True
    )
    ip = models.CharField(verbose_name=_("IP"), max_length=30, blank=True, default="")
    data_hora = models.DateTimeField(
        verbose_name=_("Data/Hora"), auto_now_add=True, blank=True, null=True
    )

    selo_votacao = models.BooleanField(
        null=True,
        blank=True,
        default=False,
        choices=YES_NO_CHOICES,
    ...
    def __str__(self):
        return _(
            "Ordem: %(ordem)s - Votação: %(votacao)s - " "Matéria: %(materia)s"
        ) % {
            "ordem": self.ordem,
...
class VotoParlamentar(models.Model):
    ...
    votacao = models.ForeignKey(
        RegistroVotacao, blank=True, null=True, on_delete=models.CASCADE
    )
    parlamentar = models.ForeignKey(Parlamentar, on_delete=models.PROTECT)
    ...
    ordem = models.ForeignKey(OrdemDia, blank=True, null=True, on_delete=models.CASCADE)
    expediente = models.ForeignKey(
        ExpedienteMateria, blank=True, null=True, on_delete=models.CASCADE
    )
...
class ResumoOrdenacao(models.Model):
    ...
    primeiro = models.CharField(max_length=50, default=ORDENACAO_RESUMO[0][0])
    segundo = models.CharField(max_length=50, default=ORDENACAO_RESUMO[1][0])
    terceiro = models.CharField(max_length=50, default=ORDENACAO_RESUMO[2][0])
    quarto = models.CharField(max_length=50, default=ORDENACAO_RESUMO[3][0])
    quinto = models.CharField(max_length=50, default=ORDENACAO_RESUMO[4][0])
    sexto = models.CharField(max_length=50, default=ORDENACAO_RESUMO[5][0])
    setimo = models.CharField(max_length=50, default=ORDENACAO_RESUMO[6][0])
    oitavo = models.CharField(max_length=50, default=ORDENACAO_RESUMO[7][0])
    nono = models.CharField(max_length=50, default=ORDENACAO_RESUMO[8][0])
    decimo = models.CharField(max_length=50, default=ORDENACAO_RESUMO[9][0])
    decimo_primeiro = models.CharField(max_length=50, default=ORDENACAO_RESUMO[10][0])
    decimo_segundo = models.CharField(max_length=50, default=ORDENACAO_RESUMO[11][0])
    decimo_terceiro = models.CharField(max_length=50, default=ORDENACAO_RESUMO[12][0])
    decimo_quarto = models.CharField(max_length=50, default=ORDENACAO_RESUMO[13][0])
...
class JustificativaAusencia(models.Model):
    ...
    sessao_plenaria = models.ForeignKey(
        SessaoPlenaria, on_delete=models.CASCADE, verbose_name=_("Sessão Plenária")
    )
...
class RetiradaPauta(models.Model):
    ...
    data = models.DateField(verbose_name=_("Data"), default=timezone.now)
    observacao = models.TextField(blank=True, verbose_name=_("Observações"))
    parlamentar = models.ForeignKey(
        Parlamentar,
        on_delete=models.PROTECT,
    ...
    tipo_de_retirada = models.ForeignKey(
        TipoRetiradaPauta,
        on_delete=models.PROTECT,
    ...
    def __str__(self):
        return _(
            "Ordem: %(ordem)s - Requerente: %(requerente)s - " "Matéria: %(materia)s"
        ) % {
            "ordem": self.ordem,
...
class RegistroLeitura(models.Model):
    ...
    data_hora = models.DateTimeField(
        verbose_name=_("Data/Hora"), auto_now=True, blank=True, null=True
    )
    ...
    def __str__(self):
        return _("Leitura - " "Matéria: %(materia)s") % {"materia": self.materia}
