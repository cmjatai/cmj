...
def test_incluir_sessao_plenaria_submit(admin_client):
    ...
    response = admin_client.post(
        reverse("sapl.sessao:sessaoplenaria_create"),
        {
...
def test_incluir_sessao_errors(admin_client):
    ...
    response = admin_client.post(
        reverse("sapl.sessao:sessaoplenaria_create"), {"salvar": "salvar"}, follow=True
    )
...
class TestResumoView:
    def setup(self):
        ...
        self.integrante_mesa = IntegranteMesa(
            sessao_plenaria=self.sessao_plenaria,
    ...
    def test_get_identificacao_basica(self):
        ...
        assert (
            info_basica[1]
            == "Abertura: " + abertura + " - " + self.sessao_plenaria.hora_inicio
        )
        ...
        assert (
            info_basica[2]
            == "Encerramento: " + encerramento + " " + self.sessao_plenaria.hora_fim
        )
    ...
    def test_get_mesa_diretora(self):
        ...
        assert mesa == {
            "mesa": [{"cargo": self.cargo_mesa, "parlamentar": self.parlamentar}]
        }

    def test_get_presenca_sessao(self):
        justificativa = mommy.make(
            JustificativaAusencia, sessao_plenaria=self.sessao_plenaria
        )
        presenca = mommy.make(
            SessaoPlenariaPresenca, sessao_plenaria=self.sessao_plenaria
        )
    ...
    def test_get_expedientes(self):
        ...
        expediente = mommy.make(
            ExpedienteSessao, sessao_plenaria=self.sessao_plenaria, tipo=tipo_expediente
        )

        resposta_expediente = get_expedientes(self.sessao_plenaria)

        assert resposta_expediente["expedientes"] == [
            {"conteudo": expediente.conteudo, "tipo": tipo_expediente}
        ]
    ...
    def test_get_oradores_explicacoes_pessoais(self):
        ...
        orador = mommy.make(
            Orador, sessao_plenaria=self.sessao_plenaria, parlamentar=parlamentar
        )
