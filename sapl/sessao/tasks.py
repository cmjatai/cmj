...
def task_add_selo_votacao_function(list_pk):
    logger.info(f"task_add_selo_votacao_function RegistroVotacao {list_pk}")
    ...
    for rv in registros_votacao:
        ...
        if (
            votacoes_da_materia.count() == 1
            and materia_votada.tipo.turnos_aprovacao == 1
        ):
        ...
        for vot in votacoes_da_materia:
            ...
            compression = materia_votada.metadata["selos"]["cert_protocolo"].get(
                "compression", False
            )
            ...
            try:
                ...
                y = (
                    materia_votada.metadata["selos"]["cert_protocolo"]["y"]
                    + materia_votada.metadata["selos"]["cert_protocolo"]["h"]
                    + 10
                )
            except:
                x = 190
            ...
            params = {
                ...
                "cor": (
                    ...
                    if vot.tipo_resultado_votacao.natureza == "A"
                    else "150, 20, 0, 255"
                ),
