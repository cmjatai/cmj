...
class RegistroVotacaoFilterView(
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            return HttpResponseRedirect(reverse("sapl.sessao:votacoes_pesquisa"))
    ...
    def get_context_data(self, **kwargs):
        ...
        if cd.get("tipo_resultado_votacao", ""):
            filtro = (
                f'<strong>Resultado da Votação:</strong> {cd["tipo_resultado_votacao"]}'
            )
            filters.append(filtro)
...
class PesquisarSessaoPlenariaView(
    AudigLogFilterMixin, MultiFormatOutputMixin, FilterView
):
    ...
    def get_queryset(self):
        ...
        qs = (
            qs.select_related("tipo", "sessao_legislativa", "legislatura")
            .distinct()
