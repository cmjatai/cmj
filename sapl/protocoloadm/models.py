...
class TipoDocumentoAdministrativo(models.Model):
    ...
    prioridade = models.BooleanField(default=False, verbose_name=_("Prioridade"))
...
class Protocolo(models.Model):
    ...
    ano = models.PositiveSmallIntegerField(
        blank=False, null=False, choices=RANGE_ANOS, verbose_name=_("Ano do Protocolo")
    )
    ...
    user_data_hora_manual = models.CharField(
        ...
        verbose_name=_("IP"),
        help_text=_(
            "Usuário que está realizando Protocolo e informando "
    ...
    email = models.EmailField(blank=True, verbose_name=_("Email do Interessado"))
    comprovante_automatico_enviado = models.BooleanField(
        default=False, verbose_name=_("Comprovante Automático Enviado")
    )
    autor = models.ForeignKey(Autor, blank=True, null=True, on_delete=models.PROTECT)
    ...
    tipo_object_id = models.PositiveIntegerField(blank=True, null=True, default=None)
    tipo_conteudo_protocolado = SaplGenericForeignKey(
        "tipo_content_type",
        "tipo_object_id",
        verbose_name=_("Tipo do Conteúdo Protocolado"),
    )
    ...
    conteudo_protocolado = SaplGenericForeignKey(
        ...
        "conteudo_object_id",
        verbose_name=_("Conteúdo Protocolado"),
    )
    ...
    class Meta:
        ...
        indexes = (
            models.Index(fields=["conteudo_content_type", "conteudo_object_id"]),
        )

    def __str__(self):
        return _("%(numero)s/%(ano)s") % {"numero": self.numero, "ano": self.ano}
    ...
    def epigrafe(self):
        return "{}/{} - {}".format(
            ...
            (
                formats.date_format(
                    timezone.localtime(self.timestamp), "DATETIME_FORMAT"
                )
                if self.timestamp
                else "{} - {}".format(
    ...
    def materia_vinculada(self):
        try:
            materia = MateriaLegislativa.objects.get(
                tipo=self.tipo_materia, ano=self.ano, numero_protocolo=self.numero
            )
        except:
            return None
        return materia
    ...
    def get_proximo_numero_protocolo(numeracao):
        ...
        if numeracao == "A":
            numero = Protocolo.objects.filter(ano=timezone.now().year).aggregate(
                Max("numero")
            )
            ...
            raise ValueError(f"Tipo de numeração inválido: {numeracao}")
...
class DocumentoAdministrativo(CommonMixin):
    ...
    temp_migracao_sislegis = models.PositiveIntegerField(
        blank=True,
        null=True,
        verbose_name=_("Field temporário para migração dos docs adms do sislegis"),
    )
    ...
    valor_estimado = models.DecimalField(
        ...
        blank=True,
        null=True,
        verbose_name=_("Valor Efetivo/Estimado"),
    )
    ...
    assunto = models.TextField(blank=True, default="", verbose_name=_("Assunto"))
    ...
    link_share = models.CharField(
        _("Link de Compartilhamento"), max_length=128, blank=True, default=""
    )
    ...
    def is_signed(self):
        try:
            return (
                ...
                and self.metadata["signs"]["texto_integral"]
                and self.metadata["signs"]["texto_integral"]["signs"]
            )
        except:
            return False
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        result = models.Model.delete(self, using=using, keep_parents=keep_parents)
    ...
    def link_share_create(self):
        md5 = hashlib.md5()
        data = "{}{}".format(
            timezone.localtime(), serializers.serialize("json", [self])
        )
...
class DocumentoAcessorioAdministrativo(CommonMixin):
    ...
    documento = models.ForeignKey(DocumentoAdministrativo, on_delete=models.PROTECT)
    tipo = models.ForeignKey(
        TipoDocumentoAdministrativo, on_delete=models.PROTECT, verbose_name=_("Tipo")
    )
    ...
    autor = models.CharField(max_length=50, blank=True, verbose_name=_("Autor"))
...
class StatusTramitacaoAdministrativo(models.Model):
    ...
    filtro = models.BooleanField(
        verbose_name=_("Filtro ?"), choices=YES_NO_CHOICES, default=True
    )

    class Meta:
        verbose_name = _("Status de Tramitação de Documentos Administrativos")
        verbose_name_plural = _("Status de Tramitação de Documentos Administrativos")
        ordering = ["id"]
...
class Anexado(models.Model):
    ...
    def __str__(self):
        return _(
            "Anexado: %(documento_anexado_tipo)s %(documento_anexado_numero)s"
...
class VinculoDocAdminMateria(models.Model):
    ...
    class Meta:
        verbose_name = _("Vinculo entre Documento Administrativo e Matéria Legislativa")
