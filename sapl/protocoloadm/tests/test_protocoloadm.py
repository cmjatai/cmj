...
def test_anular_protocolo_submit(admin_client):
    ...
    response = admin_client.post(
        reverse("sapl.protocoloadm:anular_protocolo"),
        {
            "numero": "76",
            "ano": "2016",
...
def test_form_anular_protocolo_inexistente():
    form = AnularProtocoloAdmForm(
        {"numero": "1", "ano": "2016", "justificativa_anulacao": "TESTE"}
    )
...
def test_form_anular_protocolo_campos_obrigatorios():
    ...
    # numero ausente
    form = AnularProtocoloAdmForm(
        {"numero": "", "ano": "2016", "justificativa_anulacao": "TESTE"}
    )
    if form.is_valid():
        pytest.fail(_("Form deve ser inválido"))
    ...
    # ano ausente
    form = AnularProtocoloAdmForm(
        {"numero": "1", "ano": "", "justificativa_anulacao": "TESTE"}
    )
    if form.is_valid():
        pytest.fail(_("Form deve ser inválido"))
    ...
    form = AnularProtocoloAdmForm(
        {"numero": "1", "ano": "2016", "justificativa_anulacao": ""}
    )
    if form.is_valid():
        pytest.fail(_("Form deve ser inválido"))
...
def test_create_tramitacao(admin_client):
    tipo_doc = mommy.make(TipoDocumentoAdministrativo, descricao="Teste Tipo_DocAdm")

    documento_adm = mommy.make(DocumentoAdministrativo, tipo=tipo_doc)

    unidade_tramitacao_local_1 = mommy.make(UnidadeTramitacao, pk=1)

    unidade_tramitacao_destino_1 = mommy.make(UnidadeTramitacao, pk=2)

    unidade_tramitacao_destino_2 = mommy.make(UnidadeTramitacao, pk=3)

    status = mommy.make(StatusTramitacaoAdministrativo)
    ...
    assert msg in response.context_data["form"].errors["__all__"]
    ...
    msg = _(
        "A data da nova tramitação deve ser " + "maior que a data da última tramitação!"
    )
    ...
    response = admin_client.post(
        ...
        ),
        {
            ...
            "urgente": False,
            "texto": "teste",
            "data_tramitacao": timezone.now().date() + timedelta(days=1),
        },
        follow=True,
    )

    msg = force_str(
        _("A data de tramitação deve ser " + "menor ou igual a data de hoje!")
    )
    ...
    msg = force_str(
        _("A data de encaminhamento deve ser " + "maior que a data de tramitação!")
    )
    ...
    msg = _("A data fim de prazo deve ser " + "maior que a data de tramitação!")
...
def test_anular_protocolo_form_anula_protocolo_inexistente():
    form = AnularProtocoloAdmForm(
        data={"numero": "1", "ano": "2017", "justificativa_anulacao": "teste"}
    )
...
def test_anular_protocolo_form_anula_protocolo_anulado():
    ...
    assert form.errors["__all__"] == [_("Protocolo 1/2017 já encontra-se anulado")]
...
def test_anular_protocolo_form_anula_protocolo_com_doc_vinculado():
    ...
    mommy.make(Protocolo, numero=1, ano=2017, tipo_materia=tipo_materia, anulado=False)
    ...
    assert form.errors["__all__"] == [
        _(
            "Protocolo 1/2017 não pode ser removido pois existem "
    ...
    protocolo_documento = mommy.make(
        Protocolo, numero=2, ano=2017, tipo_documento=tipo_documento, anulado=False
    )

    mommy.make(DocumentoAdministrativo, protocolo=protocolo_documento)

    form = AnularProtocoloAdmForm(
        data={"numero": "2", "ano": "2017", "justificativa_anulacao": "teste"}
    )
    ...
    assert form.errors["__all__"] == [
        _(
            "Protocolo 2/2017 não pode ser removido pois existem "
            "documentos vinculados a ele."
        )
    ]
...
def test_documento_administrativo_protocolo_inexistente():
    ...
    protocolo = mommy.make(
        Protocolo, ano=2017, numero=10, anulado=False, tipo_documento=tipo
    )
...
def test_lista_documentos_anexados():
    tipo_documento = mommy.make(TipoDocumentoAdministrativo, descricao="Tipo_Teste")
...
def test_tramitacoes_documentos_anexados(admin_client):
    ...
    tramitacao_anexada_anexada = (
        documento_anexado_anexado.tramitacaoadministrativo_set.last()
    )

    # Verifica se foram criadas as tramitações para os documentos anexados e anexados aos anexados
    assert (
        documento_principal.tramitacaoadministrativo_set.last() == tramitacao_principal
    )
    assert tramitacao_principal.documento.tramitacao == (
    ...
    assert DocumentoAdministrativo.objects.get(
        id=documento_anexado_anexado.pk
    ).tramitacao == (tramitacao_anexada_anexada.status.indicador != "F")

    # Teste Edição de Tramitacao
    form = TramitacaoAdmEditForm(data={})
    # Alterando unidade_tramitacao_destino
    form.data = {
        "data_tramitacao": tramitacao_principal.data_tramitacao,
    ...
    assert (
        tramitacao_principal.unidade_tramitacao_destino == unidade_tramitacao_destino_2
    )
    assert tramitacao_anexada.unidade_tramitacao_destino == unidade_tramitacao_destino_2
    assert (
        tramitacao_anexada_anexada.unidade_tramitacao_destino
        == unidade_tramitacao_destino_2
    )

    # Teste Remoção de Tramitacao
    url = reverse(
        "sapl.protocoloadm:tramitacaoadministrativo_delete",
        kwargs={"pk": tramitacao_principal.pk},
    )
    response = admin_client.post(url, {"confirmar": "confirmar"}, follow=True)
    assert (
        TramitacaoAdministrativo.objects.filter(id=tramitacao_principal.pk).count() == 0
    )
    assert (
        TramitacaoAdministrativo.objects.filter(id=tramitacao_anexada.pk).count() == 0
    )
    assert (
        TramitacaoAdministrativo.objects.filter(
            id=tramitacao_anexada_anexada.pk
        ).count()
        == 0
    )
    ...
    # Alterando unidade_tramitacao_destino
    form.data = {
        "data_tramitacao": tramitacao_anexada.data_tramitacao,
    ...
    assert (
        tramitacao_principal.unidade_tramitacao_destino == unidade_tramitacao_destino_1
    )
    assert tramitacao_anexada.unidade_tramitacao_destino == unidade_tramitacao_destino_2
    assert (
        tramitacao_anexada_anexada.unidade_tramitacao_destino
    ...
    assert (
        TramitacaoAdministrativo.objects.filter(id=tramitacao_anexada.pk).count() == 1
    )
    assert (
        TramitacaoAdministrativo.objects.filter(
            id=tramitacao_anexada_anexada.pk
...
def test_tramitacao_lote_documentos_form(admin_client):
    ...
    form = TramitacaoEmLoteAdmForm(initial={"documentos": documentos}, data={})
    form.data = {
        "data_tramitacao": now + timedelta(days=5),
...
def test_tramitacao_lote_documentos_views(admin_client):
    ...
    response = admin_client.post(
        url, {"documentos": documentos, "salvar": "salvar"}, follow=True
    )
    msgs = [m.message for m in response.context["messages"]]
    ...
    response = admin_client.post(
        url_lote, {"documentos": documentos, "salvar": "salvar"}, follow=True
    )
    assert response.status_code == 200
