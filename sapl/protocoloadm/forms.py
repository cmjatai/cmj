...
NATUREZA_PROCESSO = [("0", "Administrativo"), ("1", "Legislativo")]
...
class ProtocoloFilterSet(django_filters.FilterSet):
    ...
    ano = django_filters.ChoiceFilter(
        required=False, label="Ano", choices=choice_anos_com_protocolo
    )

    assunto_ementa = django_filters.CharFilter(
        label=_("Assunto"), lookup_expr="icontains"
    )

    interessado = django_filters.CharFilter(
        label=_("Interessado"), lookup_expr="icontains"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        row1 = to_row([("numero", 4), ("ano", 4), ("data", 4)])

        row2 = to_row(
            [("tipo_documento", 4), ("tipo_protocolo", 4), ("tipo_materia", 4)]
        )
...
class DocumentoAdministrativoFilterSet(django_filters.FilterSet):
    ...
    ano = django_filters.ChoiceFilter(
        required=False, label="Ano", choices=choice_anos_com_documentoadministrativo
    )
    ...
    numero = django_filters.NumberFilter(label=_("Número"), method="filter_numero")
    ...
    dv = django_filters.CharFilter(method="filter_dv")
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.filters["tas"].queryset = StatusTramitacaoAdministrativo.objects.filter(
            workspace=workspace, filtro=True
        ).order_by("descricao")
        ...
        self.form.fields["data_vencimento"].help_text = (
            "Ao informar um período de vencimento, o campo ordenação se torna irrelevante."
        )
    ...
    def filter_dv(self, qs, name, v):
        ...
        if v == "1":  # documento a vencer
            qs = qs.filter(data_vencimento__gte=timezone.now()).order_by(
                "data_vencimento", "-data_ultima_atualizacao"
            )
        elif v == "-1":  # documento vencido
            qs = qs.filter(data_vencimento__lte=timezone.now()).order_by(
                "-data_vencimento", "-data_ultima_atualizacao"
            )
    ...
    def filter_signeds(self, queryset, name, value):
        ...
        if value == "1":
            ...
            q &= Q(metadata__signs__texto_integral__signs__isnull=True) | Q(
                metadata__signs__texto_integral__signs__len=0
            )
...
class AnularProtocoloAdmForm(ModelForm):
    ...
    numero = forms.CharField(
        required=True, label=Protocolo._meta.get_field("numero").verbose_name
    )
    ano = forms.ChoiceField(
        required=True,
        label=Protocolo._meta.get_field("ano").verbose_name,
        choices=RANGE_ANOS,
        widget=forms.Select(attrs={"class": "selector"}),
    )
    ...
    def clean(self):
        ...
        try:
            ...
            if protocolo.anulado:
                ...
                raise forms.ValidationError(
                    _("Protocolo %s/%s já encontra-se anulado") % (numero, ano)
                )
        except ObjectDoesNotExist:
            self.logger.error("Protocolo %s/%s não existe" % (numero, ano))
            raise forms.ValidationError(_("Protocolo %s/%s não existe" % (numero, ano)))
        ...
        if protocolo.tipo_materia:
            ...
            exists = (
                protocolo.documentoadministrativo_set.all()
                .order_by("-ano", "-numero")
                .exists()
            )
        ...
        if exists:
            self.logger.error(
                "Protocolo %s/%s não pode ser removido pois existem "
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(AnularProtocoloAdmForm, self).__init__(*args, **kwargs)
...
class ProtocoloDocumentForm(ModelForm):
    ...
    assunto = forms.CharField(widget=forms.Textarea, label=_("Assunto"), required=True)

    interessado = forms.CharField(required=True, label=_("Interessado"))

    email = forms.EmailField(required=False, label=_("Email do Interessado"))

    observacao = forms.CharField(
        required=False, widget=forms.Textarea, label=_("Observação")
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        row4 = to_row([("assunto", 12)])
        row5 = to_row([("email", 5), ("interessado", 7)])
        row6 = to_row([("observacao", 12)])
        # row7 = to_row(
        #    [('numero', 12)])
        ...
        self.helper = SaplFormHelper()
        self.helper.layout = Layout(
            Fieldset(_("Identificação de Documento"), Div(row1, css_class="small")),
            fieldset,
            row5,
            row4,
            row6,
        ...
        self.fields["tipo_documento"].choices = [
            (t.id, t)
            for t in TipoDocumentoAdministrativo.objects.filter(
...
class ProtocoloDocumentoAcessorioForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Fieldset(
                _("Identificação de Documento Acessório"), Div(row1, css_class="small")
            ),
            fieldset,
            row5,
            row4,
        ...
        super(ProtocoloDocumentoAcessorioForm, self).__init__(*args, **kwargs)
...
class ProtocoloMateriaForm(ModelForm):
    ...
    autor = forms.ModelChoiceField(
        required=True, empty_label="------", queryset=Autor.objects.all()
    )

    tipo_autor = forms.ModelChoiceField(
        required=True, empty_label="------", queryset=TipoAutor.objects.all()
    )
    ...
    assunto_ementa = forms.CharField(
        required=True, widget=forms.Textarea, label=_("Ementa")
    )
    ...
    def clean_autor(self):
        ...
        try:
            ...
            self.logger.error(
                "Autor com id={} não encontrado. Definido como None.".format(
                    autor_field.id
                )
            )
            autor_field = None
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper = SaplFormHelper()
        self.helper.layout = Layout(
            Fieldset(_("Identificação da Matéria"), row1, row2),
            fieldset,
            row4,
            row5,
            HTML("&nbsp;"),
...
class DocumentoAcessorioAdministrativoForm(FileFieldCheckMixin, ModelForm):
    ...
    class Meta:
        ...
        fields = ["tipo", "nome", "data", "autor", "arquivo", "assunto"]
    ...
    def clean(self):
        ...
        if arquivo and arquivo.size > MAX_DOC_UPLOAD_SIZE:
            raise ValidationError(
                "O arquivo deve ser menor que {0:.1f} mb, o tamanho atual desse arquivo é {1:.1f} mb".format(
    ...
    def clean(self):
        ...
        ultima_tramitacao = (
            TramitacaoAdministrativo.objects.filter(
                documento_id=self.instance.documento_id
            )
        ...
        if not self.instance.data_tramitacao:
            if ultima_tramitacao:
                ...
                if destino != self.cleaned_data["unidade_tramitacao_local"]:
                    self.logger.error(
                        "A origem da nova tramitação ({}) deve ser  "
                        "igual ao destino ({}) da última adicionada!".format(
            ...
            if self.cleaned_data["data_tramitacao"] > timezone.now().date():
                self.logger.error(
                    ...
                    "menor ou igual a data de hoje ({})!".format(
                        self.cleaned_data["data_tramitacao"], timezone.now().date()
                    )
                )
                msg = _(
    ...
    def save(self, commit=True):
        ...
        if tramitar_anexados:
            ...
            for da in anexados_list:
                if (
                    not da.tramitacaoadministrativo_set.all()
                    or da.tramitacaoadministrativo_set.last().unidade_tramitacao_destino
...
class TramitacaoAdmEditForm(TramitacaoAdmForm):
    ...
    def clean(self):
        ...
        ultima_tramitacao = (
            TramitacaoAdministrativo.objects.filter(documento_id=obj.documento_id)
            .order_by("-data_tramitacao", "-id")
            .first()
        )
        ...
        if not (
            ...
            or cd["unidade_tramitacao_destino"] != obj.unidade_tramitacao_destino
            or cd["status"] != obj.status
            or cd["texto"] != obj.texto
            or cd["data_encaminhamento"] != obj.data_encaminhamento
            or cd["data_fim_prazo"] != obj.data_fim_prazo
        ):
            cd["user"] = obj.user
            cd["ip"] = obj.ip
...
class AnexadoForm(ModelForm):
    ...
    def clean(self):
        ...
        try:
            self.logger.info(
                "Tentando obter objeto DocumentoAdministrativo (numero={}, ano={}, tipo={}).".format(
            ...
            msg = _(
                "O {} {}/{} não existe no cadastro de documentos administrativos.".format(
                    cleaned_data["tipo"], cleaned_data["numero"], cleaned_data["ano"]
                )
            )
            self.logger.error(
                "O documento a ser anexado não existe no cadastro"
...
class DocumentoAdministrativoForm(FileFieldCheckMixin, ModelForm):
    ...
    numero_protocolo = forms.IntegerField(
        required=False, label=Protocolo._meta.get_field("numero").verbose_name
    )

    materia_numero_protocolo = forms.IntegerField(
        required=False, label=Protocolo._meta.get_field("numero").verbose_name
    )
    ...
    numero_anexador = forms.CharField(label="Número", required=False)

    ano_anexador = forms.CharField(label="Ano", required=False)

    observacao = forms.CharField(
        label=DocumentoAdministrativo._meta.get_field("observacao").verbose_name,
    ...
    visibilidade = forms.ChoiceField(
        required=False,
        label=DocumentoAdministrativo._meta.get_field("visibilidade").verbose_name,
        choices=DocumentoAdministrativo.PRIVACIDADE_DOC_ADM_STATUS,
        widget=forms.Select(attrs={"class": "selector"}),
    )

    valor_estimado = DecimalField(
        label="Valor Efetivo/Estimado",
        required=False,
        max_digits=14,
        decimal_places=2,
    )
    ...
    class Meta:
        ...
        widgets = {"protocolo": forms.HiddenInput(), "workspace": forms.HiddenInput()}
    ...
    def clean(self):
        ...
        if self.instance.pk:
            mudanca_doc = (
                numero_documento != self.instance.numero
                or ano_documento != self.instance.ano
                or tipo_documento != self.instance.tipo.pk
            )

        if not self.instance.pk or mudanca_doc:
            doc_exists = DocumentoAdministrativo.objects.filter(
                numero=numero_documento, tipo=tipo_documento, ano=ano_documento
            ).exists()
            if doc_exists:
                self.logger.error(
                    "DocumentoAdministrativo (numero={}, tipo={} e ano={}) já existe.".format(
                        numero_documento, tipo_documento, ano_documento
                    )
                )
        ...
        if numero_protocolo and ano_protocolo:
            try:
                self.logger.debug(
                    "Tentando obter Protocolo com numero={} e ano={}.".format(
                        numero_protocolo, ano_protocolo
                    )
                )
                self.fields["protocolo"].initial = Protocolo.objects.get(
                    numero=numero_protocolo, ano=ano_protocolo
                ).pk
            except ObjectDoesNotExist:
                self.logger.error(
                    "Protocolo %s/%s inexistente." % (numero_protocolo, ano_protocolo)
                )
                msg = _(
                    "Protocolo %s/%s inexistente." % (numero_protocolo, ano_protocolo)
                )
                ...
                self.logger.error(
                    "Existe mais de um Protocolo com este ano ({}) e número ({}).".format(
            ...
            if str(protocolo_antigo) != numero_protocolo:
                exist_materia = MateriaLegislativa.objects.filter(
                    numero_protocolo=numero_protocolo, ano=ano_protocolo
                ).exists()

                exist_doc = DocumentoAdministrativo.objects.filter(
                    protocolo__numero=numero_protocolo, protocolo__ano=ano_protocolo
                ).exists()

                if exist_materia or exist_doc:
                    self.logger.error(
                        "Protocolo com numero=%s e ano=%s já possui"
                        " documento vinculado" % (numero_protocolo, ano_protocolo)
                    )
                    raise ValidationError(
                        _(
                            "Protocolo %s/%s já possui"
                            " documento vinculado" % (numero_protocolo, ano_protocolo)
                        )
                    )
        ...
        if tm and am and nm:
            try:
                ...
                params = {"tipo_id": tm, "ano": am, "numero": nm}
                if mtp:
                    params["numero_protocolo"] = mtp
                materia_de_vinculo = (
                    MateriaLegislativa.objects.filter(**params).order_by("-id").first()
                )
        ...
        tx, ax, nx = (
            cleaned_data.get("tipo_anexador", ""),
    ...
    def save(self, commit=True):
        ...
        if "protocolo" in self.fields and self.fields["protocolo"].initial:
            protocolo = Protocolo.objects.get(id=int(self.fields["protocolo"].initial))
    ...
    def __init__(self, *args, **kwargs):
        ...
        row1 = to_row([("tipo", 5), ("numero", 2), ("ano", 2), ("visibilidade", 3)])
        ...
        row5_5 = to_row(
            [
                ...
                ("numero_materia", 3),
                ("ano_materia", 2),
                ("materia_numero_protocolo", 2),
            ]
        )

        row6 = to_row([("numero_externo", 4), ("dias_prazo", 6), ("data_fim_prazo", 2)])
        ...
        fieldset.append(
            Fieldset(_("Identificação Básica"), row1, row2, row2_5, row3, row4, row5),
        )
        ...
        self.fields["tipo_anexador"].queryset = (
            TipoDocumentoAdministrativo.objects.filter(
                workspace=kwargs["initial"]["workspace"]
            )
        )
        ...
        if inst and inst.materia:
            ...
            self.fields["materia_numero_protocolo"].initial = (
                inst.materia.numero_protocolo
            )
...
class DesvincularDocumentoForm(ModelForm):
    ...
    def clean(self):
        ...
        try:
            self.logger.debug(
                "Tentando obter DocumentoAdministrativo com numero={}, ano={} e tipo={}.".format(
                    numero, ano, tipo
                )
            )
            ...
            if not documento.protocolo:
                self.logger.error(
                    "DocumentoAdministrativo %s %s/%s não se encontra vinculado a nenhum protocolo."
                    % (tipo, numero, ano)
                )
                raise forms.ValidationError(
                    _(
                        "%s %s/%s não se encontra vinculado a nenhum protocolo"
                        % (tipo, numero, ano)
                    )
                )
        except ObjectDoesNotExist:
            ...
            raise forms.ValidationError(_("%s %s/%s não existe" % (tipo, numero, ano)))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Fieldset(
                _("Identificação do Documento"),
                row1,
                HTML("&nbsp;"),
                form_actions(label="Desvincular"),
            )
        )
        super(DesvincularDocumentoForm, self).__init__(*args, **kwargs)
...
class DesvincularMateriaForm(forms.Form):
    ...
    numero = forms.CharField(required=True, label=_("Número da Matéria"))
    ano = forms.ChoiceField(
        required=True,
    ...
    def clean(self):
        ...
        try:
            self.logger.info(
                "Tentando obter MateriaLegislativa com numero={}, ano={} e tipo={}.".format(
                    numero, ano, tipo
                )
            )
            ...
            if not materia.numero_protocolo:
                self.logger.error(
                    "MateriaLegislativa %s %s/%s não se encontra vinculada a nenhum protocolo"
                    % (tipo, numero, ano)
                )
                raise forms.ValidationError(
                    _(
                        "%s %s/%s não se encontra vinculada a nenhum protocolo"
                        % (tipo, numero, ano)
                    )
                )
        except ObjectDoesNotExist:
...
class FichaPesquisaAdmForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        row1 = to_row([("tipo_documento", 6), ("data_inicial", 3), ("data_final", 3)])
...
class PrimeiraTramitacaoEmLoteAdmFilterSet(django_filters.FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        super(PrimeiraTramitacaoEmLoteAdmFilterSet, self).__init__(*args, **kwargs)
...
class TramitacaoEmLoteAdmForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        row3 = to_row([("status", 6), ("urgente", 6)])
...
class TramitacaoEmLoteAdmFilterSet(django_filters.FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        super(TramitacaoEmLoteAdmFilterSet, self).__init__(*args, **kwargs)
...
class VinculoDocAdminMateriaForm(ModelForm):
    ...
    def clean(self):
        ...
        if (
            VinculoDocAdminMateria.objects.filter(
                documento=self.instance.documento, materia=materia
            )
            .exclude(pk=self.instance.pk)
            .exists()
        ):
            self.logger.error("Matéria já se encontra vinculada a este documento.")
            raise ValidationError(
