...
from sapl.protocoloadm.views import (
    ...
    ProtocoloDocumentoAcessorioView,
    ProtocoloDocumentoView,
    ProtocoloHomologarView,
...
urlpatterns_protocolo = [
    ...
    re_path(r"^protocoloadm/$", ProtocoloPesquisaView.as_view(), name="protocolo"),
    re_path(
    ...
    re_path(
        r"^protocoloadm/protocolar-doc-acess$",
        ProtocoloDocumentoAcessorioView.as_view(),
        name="protocolar_doc_acess",
    ),
    re_path(
        r"^protocoloadm/(?P<pk>\d+)/protocolo-mostrar$",
        ProtocoloMostrarView.as_view(),
        name="protocolo_mostrar",
    ),
    re_path(
        r"^protocoloadm/(?P<pk>\d+)/homologar$",
        ProtocoloHomologarView.as_view(),
        name="protocolo_homologar",
    ),
    re_path(
        r"^protocoloadm/(?P<pk>\d+)/conteudo$",
        ProtocoloRedirectConteudoView.as_view(),
        name="redirect_conteudo",
    ),
    re_path(
        r"^doc/(?P<pk>\d+)/acompanhar-documento/$",
        AcompanhamentoDocumentoView.as_view(),
        name="acompanhar_documento",
    ),
    re_path(
        r"^doc/(?P<pk>\d+)/acompanhar-confirmar$",
        AcompanhamentoConfirmarView.as_view(),
        name="acompanhar_confirmar",
    ),
    re_path(
        r"^doc/(?P<pk>\d+)/acompanhar-excluir$",
        AcompanhamentoExcluirView.as_view(),
        name="acompanhar_excluir",
    ),
    re_path(
        r"^protocoloadm/(?P<pk>\d+)/continuar$",
    ...
    re_path(
        r"^protocoloadm/(?P<pk>\d+)/comprovante$",
        ComprovanteProtocoloView.as_view(),
        name="comprovante_protocolo",
    ),
    re_path(
        r"^protocoloadm/(?P<pk>\d+)/criar-documento$",
...
urlpatterns_sistema = [
    re_path(
        r"^sistema/tipo-documento-adm",
        include(TipoDocumentoAdministrativoCrud.get_urls()),
    ),
    re_path(
        r"^sistema/status-tramitacao-adm",
