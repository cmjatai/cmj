...
def signal_pre_save_protocolo(sender, instance, using, **kwargs):
    ...
    if instance.email and instance.conteudo_protocolado:
        try:
            ...
            autenticacao = (
                str(instance.tipo_processo) + data + str(instance.numero).zfill(6)
            )
            ...
            logger.error("""
                ...
                --------------------
                {}
                """.format(instance.email, instance, instance.conteudo_protocolado, e))


@receiver(
    pre_save,
    sender=DocumentoAdministrativo,
    dispatch_uid="signal_pre_save_docadm_segmenta_download",
)
    ...
    def tree_add_files(nd, zf, cc):
        ...
        if hasattr(nd, "texto_integral"):
            ...
            if nd.arquivo:
                ...
                if cc["zs"] > (1024**3) or (cc["zs"] + nd.arquivo.size > (1024**3) * 2):
                    # se já é maior que 1GB ou ao adicionar um arquivo fique
                    # maior 2GB
                    cc["zn"] += 1
                    key = "{:03d}".format(cc["zn"])
