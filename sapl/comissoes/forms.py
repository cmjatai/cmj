        ...
        if intersecao_periodo:
            if periodo.data_fim:
                self.logger.error(
                    ...
                    "cadastrados para esta comissão".format(
                ...
                self.logger.error(
                    "O período informado ({} - )"
                    "choca com períodos já "
                    "cadastrados para esta comissão".format(periodo.data_inicio)
                )
...
class ReuniaoForm(ModelForm):
    ...
    def clean(self):
        ...
        if upload_pauta and upload_pauta.size > MAX_DOC_UPLOAD_SIZE:
            raise ValidationError(
                "O arquivo Pauta da Reunião deve ser menor que {0:.1f} mb, o tamanho atual desse arquivo é {1:.1f} mb".format(
        ...
        if upload_ata and upload_ata.size > MAX_DOC_UPLOAD_SIZE:
            raise ValidationError(
                "O arquivo Ata da Reunião deve ser menor que {0:.1f} mb, o tamanho atual desse arquivo é {1:.1f} mb".format(
        ...
        if upload_anexo and upload_anexo.size > MAX_DOC_UPLOAD_SIZE:
            raise ValidationError(
                "O arquivo Anexo da Reunião deve ser menor que {0:.1f} mb, o tamanho atual desse arquivo é {1:.1f} mb".format(
                    (MAX_DOC_UPLOAD_SIZE / 1024) / 1024,
