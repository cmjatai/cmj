...
def make_composicao(comissao):
    periodo = mommy.make(Periodo, data_inicio="2016-01-01", data_fim="2016-12-31")
    mommy.make(Composicao, periodo=periodo, comissao=comissao)
...
def make_filiacao():
    partido = mommy.make(Partido, nome="Partido Meu", sigla="PM")
    ...
    mommy.make(Filiacao, data="2016-03-22", parlamentar=parlamentar, partido=partido)
...
def test_incluir_comissao_submit(admin_client):
    tipo = mommy.make(TipoComissao, sigla="T", nome="Teste")
