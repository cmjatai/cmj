...
from sapl.base.forms import PartidoForm, PartidoUpdateForm, SessaoLegislativaForm
...
from .forms import (
    AfastamentoParlamentarForm,
    BlocoForm,
    CargoBlocoForm,
    CargoBlocoPartidoForm,
    FiliacaoForm,
    FrenteForm,
    LegislaturaForm,
    MandatoForm,
    ParlamentarCreateForm,
    ParlamentarFilterSet,
    ParlamentarForm,
    VincularParlamentarForm,
    VotanteForm,
)
from .models import (
...
class BancadaCrud(Crud):
    ...
    class DetailView(Crud.DetailView):
        ...
        def hook_filiados(self, bancada, **kwargs):
            ...
            filiados = (
                bancada.partido.filiacao_set.filter(**params)
                .values("parlamentar__nome_parlamentar", "data")
                .distinct()
            )

            filiados = map(
                lambda x: f'<li>{x["parlamentar__nome_parlamentar"]} '
                f'<small>(Filiado ao Partido em: {formats.date_format(x["data"], "d/m/Y")})</small></li>',
                filiados,
            )
            r = "".join(filiados)
...
class BlocoCrud(Crud):
    ...
    class DetailView(CrudAux.DetailView):
        ...
        def get_context_data(self, **kwargs):
            context = super(BlocoCrud.DetailView, self).get_context_data(**kwargs)
    ...
    class DeleteView(MasterDetailCrud.DeleteView):
        ...
        def delete(self, *args, **kwargs):
            ...
            return HttpResponseRedirect(
                reverse(
                    "sapl.parlamentares:votante_list", kwargs={"pk": obj.parlamentar.pk}
                )
            )
...
class ProposicaoParlamentarCrud(CrudBaseForListAndDetailExternalAppView):
    ...
    class BaseMixin(CrudBaseForListAndDetailExternalAppView.BaseMixin):
        list_field_names = ["tipo", "descricao", "data_envio", "data_recebimento"]
    ...
    class ListView(CrudBaseForListAndDetailExternalAppView.ListView):
        ...
        def get_context_data(self, **kwargs):
            context = CrudBaseForListAndDetailExternalAppView.ListView.get_context_data(
                self, **kwargs
            )
            return context
...
class PesquisarParlamentarView(FilterView):
    ...
    def get_filterset_kwargs(self, filterset_class):
        super(PesquisarParlamentarView, self).get_filterset_kwargs(filterset_class)
    ...
    def get_context_data(self, **kwargs):
        context = super(PesquisarParlamentarView, self).get_context_data(**kwargs)
    ...
    if ativos:
        if "data_criacao" in request.GET and request.GET["data_criacao"]:
            ...
            if "data_extincao" in request.GET and request.GET["data_extincao"]:
                ...
                parlamentares = parlamentares_ativos(data_criacao, data_extincao)
            else:
...
def parlamentares_frente_selected(request):
    ...
    try:
        logger.info(
            "user="
            + username
            + ". Tentando objet objeto Frente com id={}.".format(
                request.GET["frente_id"]
            )
        )
        ...
        logger.error(
            "user="
            + username
            + ". Frente buscada (id={}) não existe. Retornada lista vazia.".format(
                request.GET["frente_id"]
            )
        )
        ...
        logger.info(
            "user="
            + username
            + ". Frente (id={}) encontrada com sucesso.".format(
                request.GET["frente_id"]
            )
        )
        lista_parlamentar_id = frente.parlamentares.all().values_list("id", flat=True)
...
class MandatoCrud(MasterDetailCrud):
    ...
    class CreateView(MasterDetailCrud.CreateView):
        ...
        def get_initial(self):
            return {"parlamentar": Parlamentar.objects.get(pk=self.kwargs["pk"])}
...
class LegislaturaCrud(CrudAux):
    ...
    class CreateView(CrudAux.CreateView):
        ...
        def get_initial(self):
            ...
            try:
                self.logger.error(
                    "user=" + username + ". Tentando obter última Legislatura."
                )
                ...
                self.logger.error(
                    "user="
                    + username
                    + ". Legislatura não encontrada. Número definido como 1."
                )
                numero = 1
...
class ParlamentarCrud(Crud):
    ...
    class BaseMixin(Crud.BaseMixin):
        ordered_list = False
        list_field_names = ["nome_parlamentar", "filiacao_atual", "ativo"]
    ...
    class ListView(Crud.ListView):
            ...
            if legislatura_id >= 0:
                ...
                try:
                    self.logger.debug(
                        "user="
                        + username
                        + ". Tentando obter o mais recente registro do objeto Legislatura."
                    )
                    ...
                    self.logger.error(
                        "user="
                        + username
                        + ". Objeto não encontrado. Retornando todos os registros."
                    )
                    return Legislatura.objects.all()
                else:
                    self.logger.info(
                        "user=" + username + ". Objeto encontrado com sucesso."
                    )
                    if l is None:
                        return Legislatura.objects.all()
        ...
        def get_context_data(self, **kwargs):
            ...
            if not legislaturas.exists():
                self.logger.error(
                    "user="
                    + username
                    + ". Nenhuma Legislatura encontrada. Contexto de Legislaturas não adicionado."
                )
            ...
            for row in context["rows"]:
                ...
                parlamentar = Parlamentar.objects.get(id=(row[0][1].split("/")[-1]))
                ...
                try:
                    ...
                    filiacao = parlamentar.filiacao_set.get(
                        Q(
                ...
                # Caso encontre UMA filiação nessas condições
                    ...
                    self.logger.debug(
                        "user=" + username + ". Filiação encontrada com sucesso."
                    )

                    partido_aux = filiacao.partido
                    historico = HistoricoPartido.objects.filter(
                        partido=partido_aux
                    ).order_by("-fim_historico")
                    if historico:
                        partido_aux = next(
                            iter(
                                [
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            self.logger.debug(
                "user="
                + username
                + ". Tentando obter Autor (object_id={}).".format(parlamentar_pk)
            )
            ...
            mensagem = _(
                "Este Parlamentar (pk={}) não é Autor de matéria.".format(
                    parlamentar_pk
                )
            )
            self.logger.error(
        ...
        autoria = (
            Autoria.objects.filter(autor=autor, primeiro_autor=True)
            .values(
                "materia__ano",
                "materia__tipo__pk",
                "materia__tipo__sigla",
                "materia__tipo__descricao",
            )
            .annotate(total=Count("materia__tipo__pk"))
            .order_by("-materia__ano", "materia__tipo")
        )

        coautoria = (
            Autoria.objects.filter(autor=autor, primeiro_autor=False)
            .values(
                "materia__ano",
                "materia__tipo__pk",
                "materia__tipo__sigla",
...
def parlamentares_filiados(request, pk):
    ...
    parlamentares_filiados = [
        (parlamentar, get_data_filicao(parlamentar))
        for parlamentar in parlamentares
        if parlamentar.filiacao_atual == partido.sigla
    ]
    return render(
        request,
        template_name,
        {"partido": partido, "parlamentares": parlamentares_filiados},
    )
...
class MesaDiretoraView(FormView):
    ...
    def validation(self, request):
        ...
        self.logger.info(
            "user="
            + username
            + ". Não há nenhuma Sessão Legislativa cadastrada. "
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not Legislatura.objects.exists() or not SessaoLegislativa.objects.exists():
            return self.validation(request)

        legislatura = Legislatura.objects.first()
        sessoes = SessaoLegislativa.objects.filter(legislatura=legislatura).order_by(
            "data_inicio"
        )
        ...
        sessao_atual = (
            sessoes.filter(data_inicio__year__lte=year)
            .exclude(data_inicio__gt=timezone.now())
            .order_by("-data_inicio")
            .first()
        )

        mesa = (
            sessao_atual.composicaomesa_set.all().order_by("cargo_id")
            if sessao_atual
            else []
        )
        ...
        parlamentares_vagos = list(
            set([p.parlamentar for p in parlamentares if p.parlamentar.ativo])
            - set(parlamentares_ocupados)
        )
...
def altera_field_mesa(request):
    ...
    # atual deve ser a primeira daquela legislatura
        ...
        try:
            logger.debug(
                "user="
                + username
                + ". Tentando obter id de sessoes com data_inicio.ano={}.".format(year)
            )
            ...
            logger.error(
                "user="
                + username
                + ". Id de sessoes com data_inicio.ano={} não encontrado. "
    ...
    parlamentares = Legislatura.objects.get(id=legislatura).mandato_set.all()
...
def insere_parlamentar_composicao(request):
    ...
    if request.user.has_perm(
        "%s.add_%s" % (AppConfig.label, ComposicaoMesa._meta.model_name)
    ):
        ...
        try:
            logger.debug(
                "user="
                + username
                + ". Tentando obter SessaoLegislativa com id={}.".format(
                    request.POST["sessao"]
                )
            )
            ...
            logger.error(
                "user="
                + username
                + ". 'MultiValueDictKeyError', nenhuma sessão foi inserida!"
            )
        ...
        try:
            ...
            logger.error(
                "user="
                + username
                + ". 'MultiValueDictKeyError', nenhum parlamentar foi inserido!"
            )
...
def remove_parlamentar_composicao(request):
    ...
    if request.POST and request.user.has_perm(
        "%s.delete_%s" % (AppConfig.label, ComposicaoMesa._meta.model_name)
    ):
        ...
        if "composicao_mesa" in request.POST:
            try:
                logger.debug(
                    "user="
                    + username
                    + ". Tentando obter ComposicaoMesa com id={}.".format(
                ...
                logger.error(
                    "user="
                    + username
                    + ". ComposicaoMesa com id={} não encontrada, portanto não pode ser removida.".format(
                        request.POST["composicao_mesa"]
                    )
                )
                return JsonResponse(
            ...
            logger.info(
                "user="
                + username
                + ". ComposicaoMesa com id={} excluido com sucesso!".format(
                    request.POST["composicao_mesa"]
                )
            )
            ...
            logger.info(
                "user=" + username + ". Nenhum parlamentar escolhido para ser excluído."
            )
            return JsonResponse(
...
def partido_parlamentar_sessao_legislativa(sessao, parlamentar):
    ...
    try:
        logger.debug(
            "Tentando obter filiação do parlamentar com (data<={} e data_desfiliacao>={}) "
            "ou (data<={} e data_desfiliacao=Null)).".format(
                sessao.data_fim, sessao.data_fim, sessao.data_fim
            )
        )

        logger.info("Tentando obter filiação correspondente.")
        filiacao = parlamentar.filiacao_set.get(
            Q(data__lte=sessao.data_fim, data_desfiliacao__gte=sessao.data_fim)
            | Q(data__lte=sessao.data_fim, data_desfiliacao__isnull=True)
        )

    # Caso não exista filiação com essas condições
    except ObjectDoesNotExist:
        logger.error(
            "Filiação do parlamentar com (data<={} e data_desfiliacao>={}) "
            "ou (data<={} e data_desfiliacao=Null não encontrada. Retornando vazio.".format(
                sessao.data_fim, sessao.data_fim, sessao.data_fim
            )
        )
        return ""
    ...
    # Caso encontre UMA filiação nessas condições
        ...
        logger.info(
            "Filiação do parlamentar com (data<={} e data_desfiliacao>={}) "
            "ou (data<={} e data_desfiliacao=Null encontrada com sucesso.".format(
                sessao.data_fim, sessao.data_fim, sessao.data_fim
            )
        )
...
def altera_field_mesa_public_view(request):
    ...
    # atual deve ser a primeira daquela legislatura
        ...
        try:
            ...
            logger.error(
                "user="
                + username
                + ". Sessões não encontradas com com data_inicio.ano = {}. "
                "Selecionado o id da primeira sessão.".format(year)
            )
            sessao_selecionada = sessoes.first().id
    ...
    cargos_ocupados = [(m.cargo.id, m.cargo.__str__()) for m in composicao_mesa]

    parlamentares_ocupados = [
        (m.parlamentar.id, m.parlamentar.__str__()) for m in composicao_mesa
    ]
    ...
    for p in parlamentares_ocupados:
        ...
        if parlamentar.fotografia:
            try:
                ...
                logger.error(e)
                logger.error(
                    "erro processando arquivo: %s" % parlamentar.fotografia.path
                )
        else:
...
def deleta_historico_partido(request, pk):
    ...
    return HttpResponseRedirect(
        reverse("sapl.parlamentares:partido_detail", kwargs={"pk": pk_partido})
    )
...
def vincula_parlamentar_ao_bloco(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            vinculo.save()
            return HttpResponseRedirect(
                reverse("sapl.parlamentares:bloco_detail", kwargs={"pk": pk})
            )
    else:
...
def edita_vinculo_parlamentar_bloco(request, pk):
    ...
    if request.method == "POST":
        form = CargoBlocoPartidoForm(
            request.POST, instance=vinculo, initial={"bloco_pk": vinculo.bloco.pk}
        )
        if form.is_valid():
            vinculo = form.save(commit=True)
            return HttpResponseRedirect(
                reverse(
                    "sapl.parlamentares:bloco_detail", kwargs={"pk": vinculo.bloco.pk}
                )
            )
    else:
        form = CargoBlocoPartidoForm(
            instance=vinculo, initial={"bloco_pk": vinculo.bloco.pk}
        )
...
def deleta_vinculo_parlamentar_bloco(request, pk):
    ...
    vinculo.delete()
    return HttpResponseRedirect(
        reverse("sapl.parlamentares:bloco_detail", kwargs={"pk": pk_bloco})
    )
