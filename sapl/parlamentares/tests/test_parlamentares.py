...
from sapl.parlamentares.forms import (
...
def test_filiacao_submit(admin_client):
    ...
    admin_client.post(
        reverse("sapl.parlamentares:filiacao_create", kwargs={"pk": 14}),
        {"partido": 32, "data": "2016-03-22", "salvar": "salvar"},
        follow=True,
    )
...
def test_dependente_submit(admin_client):
    ...
    admin_client.post(
        reverse("sapl.parlamentares:dependente_create", kwargs={"pk": 14}),
        {"nome": "Eduardo", "tipo": 3, "sexo": "M", "salvar": "salvar"},
        follow=True,
    )
...
def test_mandato_submit(admin_client):
    ...
    admin_client.post(
        reverse("sapl.parlamentares:mandato_create", kwargs={"pk": 14}),
        {
...
def test_mandato_form_datas_invalidas():
    ...
    legislatura = mommy.make(
        Legislatura, pk=1, data_inicio="2017-01-01", data_fim="2021-12-31"
    )
...
def test_legislatura_form_datas_invalidas():
    ...
    legislatura_form = LegislaturaForm(
        data={
            "numero": "1",
    ...
    expected = _(
        "A data início deve ser menor que a data fim "
...
def test_afastamentos_form_datas_invalidas():
    ...
    tipo_afastamento = mommy.make(TipoAfastamento, indicador="A", pk=1)

    form = AfastamentoParlamentarForm(
        data={
    ...
    assert not form.is_valid()
    assert form.errors["__all__"] == ["Existe Afastamento sem Data Fim."]
