...
def validar_datas_legislatura(eleicao, inicio, fim, pk=None):
    ...
    if inicio >= fim or eleicao >= inicio:
        logger.error(
            "A data início ({}) deve ser menor que a "
    ...
    intersecao_legislatura = (
        Legislatura.objects.filter(data_inicio__lte=fim, data_fim__gte=inicio)
        .exclude(pk=pk)
        .exists()
    )
    if intersecao_legislatura:
        logger.error(
            "Já existe uma legislatura neste intervalo de datas (data_inicio<={} e data_fim>={}).".format(
                fim, inicio
            )
        )
    ...
    if Legislatura.objects.filter(data_eleicao=eleicao).exclude(pk=pk).exists():
    ...
    def clean(self):
        ...
        if data_inicio_mandato:
            if (
                ...
                or data_inicio_mandato > legislatura.data_fim
            ):
                self.logger.error(
                    "Data início mandato ({}) fora do intervalo"
                    " de legislatura informada ({} a {}).".format(
                        data_inicio_mandato,
        ...
        if data_fim_mandato:
            if (
                data_fim_mandato < legislatura.data_inicio
                or data_fim_mandato > legislatura.data_fim
            ):
                self.logger.error(
                    "Data fim mandato ({}) fora do intervalo"
                    " de legislatura informada ({} a {}).".format(
                        data_fim_mandato, legislatura.data_inicio, legislatura.data_fim
                    )
                )
                raise ValidationError(
                    _("Data fim mandato fora do intervalo de" " legislatura informada")
                )

        data_expedicao_diploma = data["data_expedicao_diploma"]
        if data_expedicao_diploma and data_expedicao_diploma > data_inicio_mandato:
            self.logger.error(
                "A data da expedição do diploma ({}) deve ser anterior "
                "a data de início do mandato ({}).".format(
        ...
        if coligacao and not coligacao.legislatura == legislatura:
            self.logger.error(
                "A coligação selecionada ({}) não está cadastrada "
                "na mesma legislatura ({}) que o presente mandato ({}), "
                "favor verificar a coligação ou fazer o cadastro "
                "de uma nova coligação na legislatura correspondente".format(
                    coligacao, coligacao.legislatura, legislatura
                )
            )
            raise ValidationError(
                _(
        ...
        existe_mandato = Mandato.objects.filter(
            parlamentar=data["parlamentar"], legislatura=data["legislatura"]
        )
        ...
        if existe_mandato.exists() and data["titular"]:
            self.logger.error(
                "Mandato nesta legislatura (parlamentar={}, legislatura={}) já existe.".format(
                    data["parlamentar"], data["legislatura"]
                )
            )
...
class LegislaturaForm(ModelForm):
    ...
    def clean(self):
        ...
        ultima_legislatura = (
            Legislatura.objects.filter(data_inicio__lt=data_inicio)
            .order_by("-data_inicio")
            .exclude(id=pk)
            .first()
        )
        proxima_legislatura = (
            Legislatura.objects.filter(data_fim__gt=data_fim)
            .order_by("data_fim")
            .exclude(id=pk)
            .first()
        )
        ...
        if ultima_legislatura and ultima_legislatura.numero >= numero:
            self.logger.error(
                "Número ({}) deve ser maior que o da legislatura anterior ({}).".format(
                    numero, ultima_legislatura.numero
                )
            )
            raise ValidationError(
                _(
            ...
            self.logger.error(
                "O Número ({}) deve ser menor que {}, pois existe uma "
                "legislatura cronologicamente à frente desta que está sendo criada!".format(
                    numero, proxima_legislatura.numero
                )
            )
        ...
        valida_datas = validar_datas_legislatura(
            data_eleicao, data_inicio, data_fim, pk=pk
        )
...
class ParlamentarForm(FileFieldCheckMixin, ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "biografia": forms.Textarea(attrs={"id": "texto-rico"}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        fotografia.attrs.update(get_attrs(self.instance.fotografia, "fotografia"))

        if "class" in fotografia.attrs:
            fotografia.attrs.pop("class")

        capa = self.fields["capa"].widget
        capa.attrs.update(get_attrs(self.instance.capa, "capa"))
...
class ParlamentarFilterSet(django_filters.FilterSet):
    nome_parlamentar = django_filters.CharFilter(
        label=_("Nome do Parlamentar"), lookup_expr="icontains"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.helper.layout = Layout(
            Fieldset(
                _("Pesquisa de Parlamentar"), row0, form_actions(label="Pesquisar")
            )
        )
...
class ParlamentarCreateForm(ParlamentarForm):
    ...
    def clean(self):
        ...
        parlamentar = Parlamentar.objects.filter(
            nome_parlamentar=cleaned_data["nome_parlamentar"]
        ).exists()
...
def validar_datas(data_filiacao, data_desfiliacao, parlamentar, filiacao):
    ...
    if data_desfiliacao and data_desfiliacao < data_filiacao:
        logger.error(
            "A data de desfiliação ({}) é anterior à data de filiação ({}).".format(
                data_desfiliacao, data_filiacao
            )
        )
    ...
    range_livre_exigido = Q(
        data__range=[
            data_filiacao + timedelta(days=1),
            df_desfiliacao - timedelta(days=1),
        ]
    ) | Q(
        data_desfiliacao__range=[
            data_filiacao + timedelta(days=1),
            df_desfiliacao - timedelta(days=1),
        ]
    )
    ...
    if (
        not data_desfiliacao
        and filiacao_em_edicao_id
        and filiacao_em_edicao_id != filiacoes.last().pk
    ):
        logger.error(
            "Data de desfiliação do parlamentar não pode ser "
    ...
    #  já existe outra sem data de desfiliação
        ...
        not data_desfiliacao
        and not filiacao_em_edicao_id
        and not filiacoes.last().data_desfiliacao
    ):
        logger.error(
            "O parlamentar não pode se filiar a novo partido sem"
    ...
    if not error_msg:
        ...
        if filiacoes.filter(range_livre_exigido).exists():
            logger.error(
                "A data de filiação e desfiliação não podem estar"
                " no intervalo de outro período de filiação."
            )
            error_msg = _(
                "A data de filiação e desfiliação (intervalo de {} a {}) "
                "não podem estar no intervalo de outro período de filiação.".format(
                    data_filiacao,
                    df_desfiliacao,
                )
            )
    ...
    if not error_msg:
        ...
        if filiacoes.filter(
            data_desfiliacao__isnull=True, data__lte=data_filiacao
        ).exists():
            logger.error(
                "Não pode haver um registro de filiação com data de "
                "filiação igual ou superior a data de filiação em aberto ({}).".format(
                    data_filiacao
                )
            )
...
class FiliacaoForm(ModelForm):
    ...
    class Meta:
        model = Filiacao
        fields = ["partido", "data", "data_desfiliacao"]
        widgets = {
            "data": forms.DateInput(attrs={"autocomplete": "off"}),
...
class ComposicaoColigacaoForm(ModelForm):
    ...
    def clean(self):
        ...
        if ComposicaoColigacao.objects.filter(
            coligacao_id=pk, partido=cleaned_data.get("partido")
        ).exists():
            self.logger.error(
                "Esse partido (coligacao_id={} e partido={}) já foi cadastrado "
...
class FrenteForm(ModelForm):
    ...
    def clean(self):
        ...
        if cd["data_extincao"] and cd["data_criacao"] >= cd["data_extincao"]:
            self.logger.error(
                "Data Dissolução ({}) não pode ser anterior a Data Criação ({}).".format(
                    cd["data_extincao"], cd["data_criacao"]
                )
            )
            raise ValidationError(
...
class VotanteForm(ModelForm):
    ...
    email = forms.CharField(label=_("Usuário"), required=True, max_length=30)
    ...
    def clean(self):
        ...
        if not user.exists():
            self.logger.error(
                "Não foi possível vincular usuário. Usuário {} não existe.".format(
                    username
                )
            )
            raise ValidationError(
        ...
        if Votante.objects.filter(user=user[0].pk).exists():
            self.logger.error(
                "Não foi possível vincular usuário. Usuário {} já está "
...
class VincularParlamentarForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        row1 = to_row(
            [("parlamentar", 6), ("legislatura", 3), ("data_expedicao_diploma", 3)]
        )

        self.helper = SaplFormHelper()
        self.helper.layout = Layout(
            Fieldset("Vincular Parlamentar", row1, form_actions(label="Vincular"))
        )
    ...
    def clean(self):
        ...
        if parlamentar.mandato_set.filter(legislatura=legislatura):
            ...
            data_expedicao_diploma and legislatura.data_inicio <= data_expedicao_diploma
        ):
            self.logger.error(
...
class BlocoForm(ModelForm):
    ...
    class Meta:
        model = Bloco
        fields = ["nome", "partidos", "data_criacao", "data_extincao", "descricao"]
    ...
    def save(self, commit=True):
        ...
        tipo = TipoAutor.objects.get(content_type=content_type)
        Autor.objects.create(
            content_type=content_type, object_id=object_id, tipo=tipo, nome=bloco.nome
        )
        return bloco
...
class BancadaForm(ModelForm):
    ...
    def clean(self):
        ...
        if data_criacao:
            if (
                data_criacao < legislatura.data_inicio
                or data_criacao > legislatura.data_fim
            ):
                raise ValidationError(
                    _(
                        "Data de criação da bancada fora do intervalo"
                        " de legislatura informada"
                    )
                )
        ...
        if data_extincao:
            if (
                data_extincao < legislatura.data_inicio
                or data_extincao > legislatura.data_fim
            ):
                raise ValidationError(
                    _(
                        "Data fim da bancada fora do intervalo de"
    ...
    def save(self, commit=True):
        ...
        tipo = TipoAutor.objects.get(content_type=content_type)
        Autor.objects.create(
            content_type=content_type, object_id=object_id, tipo=tipo, nome=bancada.nome
        )
        return bancada
...
class CargoBlocoPartidoForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if bloco_pk:
            ...
            parlamentares_filiacao = (
                Filiacao.objects.select_related("partido")
    ...
    def clean(self):
        ...
        aux_data_fim = (
            cleaned_data["data_fim"]
            if cleaned_data["data_fim"]
            else (cleaned_data["data_inicio"] + timedelta(days=1))
        )

        if cleaned_data["cargo"].unico:
            for vinculo in CargoBlocoPartido.objects.filter(
                bloco=self.bloco, cargo=cleaned_data["cargo"]
            ):
        ...
        for mandato in Mandato.objects.filter(
            parlamentar=self.cleaned_data.get("parlamentar")
        ):
            if not intervalos_tem_intersecao(
        ...
        if self.instance.pk and (
            cleaned_data["parlamentar"].id != self.instance.parlamentar.id
        ):
            raise ValidationError(
...
class AfastamentoParlamentarForm(ModelForm):
    ...
    def clean(self):
        ...
        if data_inicio_afastamento < data_inicio_mandato:
            self.logger.error(
                "Data início de afastamento ({}) anterior ao inicio"
                " do mandato informado ({}).".format(
                    data_inicio_afastamento, data_inicio_mandato
                )
            )
            raise ValidationError(
                _(
                    "Data início do afastamento anterior ao início"
        ...
        if data_fim_mandato and data_inicio_afastamento > data_fim_mandato:
            self.logger.error(
                "Data início de afastamento ({}) posterior ao fim"
                " do mandato informado ({} a {}).".format(
                    data_inicio_afastamento, data_fim_mandato
                )
            )
            raise ValidationError(
                _(
                    "Data início do afastamento posterior ao fim"
                    " do mandato informado."
                )
            )
        ...
        if data_fim_afastamento:
            if data_fim_afastamento < data_inicio_afastamento:
                self.logger.error(
                    "Data fim de afastamento ({}) anterior à data início"
                    " do afastamento ({}).".format(
                        data_fim_afastamento, data_inicio_afastamento
                    )
                )
                raise ValidationError(
                    _(
                        "Data fim do afastamento anterior à data início do"
                        " afastamento."
                    )
                )
            ...
            if data_fim_afastamento < data_inicio_mandato:
                self.logger.error(
                    "Data fim de afastamento ({}) anterior ao início"
                    " do mandato informado ({} a {}).".format(
                        data_fim_afastamento, data_inicio_mandato
                    )
                )
                raise ValidationError(
                    _(
                        "Data fim do afastamento anterior ao início"
                        " do mandato informado."
                    )
                )
            ...
            if data_fim_mandato and data_fim_afastamento > data_fim_mandato:
                self.logger.error(
                    "Data fim de afastamento ({}) posterior ao fim"
                    " do mandato informado ({}).".format(
                        data_inicio_afastamento, data_fim_mandato
                    )
                )
                raise ValidationError(
                    _(
                        "Data fim do afastamento posterior ao fim"
                        " do mandato informado."
                    )
                )
        ...
        if (
            ultimo_afastamento
            and not ultimo_afastamento.data_fim
