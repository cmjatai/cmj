    ...
    def cache_legislatura_atual(cls):
        ...
        value = list(
            Legislatura.objects.filter(
                data_inicio__year__lte=current_year, data_fim__year__gte=current_year
            ).values()[:1]
        )
...
class HistoricoPartido(models.Model):
    ...
    nome = models.CharField(max_length=50, verbose_name=_("Nome"))
    inicio_historico = models.DateField(verbose_name=_("Data Alteração"))

    fim_historico = models.DateField(verbose_name=_("Data Alteração"))
...
class ComposicaoColigacao(models.Model):
    ...
    partido = models.ForeignKey(
        Partido, on_delete=models.CASCADE, verbose_name=_("Partidos da Coligação")
    )
...
class Parlamentar(models.Model):
    ...
    capa_cropping = ImageRatioField(
        ...
        verbose_name=_("Capa"),
        size_warning=True,
        help_text=_("A configuração da capa " "é possível após a atualização."),
    )
...
class Filiacao(models.Model):
    ...
    def __str__(self):
        return _("%(parlamentar)s - %(partido)s") % {
            "parlamentar": self.parlamentar or self.autor,
            "partido": self.partido,
        }
...
class Frente(models.Model):
    ...
    * Uma frente pode existir por mais de uma legislatura?
    ...
    parlamentares = models.ManyToManyField(
        Parlamentar, blank=True, verbose_name=_("Parlamentares")
    )
...
class Bancada(models.Model):
    ...
    nome = models.CharField(max_length=80, verbose_name=_("Nome da Bancada"))
    partido = models.ForeignKey(
        Partido,
        blank=True,
        null=True,
...
class CargoBloco(models.Model):
    ...
    nome = models.CharField(max_length=80, verbose_name=_("Nome do Cargo"))
    unico = models.BooleanField(
        choices=YES_NO_CHOICES, verbose_name=_("Cargo Único"), default=True
    )
    descricao = models.TextField(blank=True, verbose_name=_("Descrição"))
...
class CargoBancada(models.Model):
    nome_cargo = models.CharField(max_length=80, verbose_name=_("Cargo de Bancada"))
...
class CargoBlocoPartido(models.Model):
    ...
    bloco = models.ForeignKey(
        Bloco, on_delete=models.PROTECT, related_name="cargoblocopartido_set"
    )

    cargo = models.ForeignKey(CargoBloco, on_delete=models.PROTECT)
...
class AfastamentoParlamentar(models.Model):
    ...
    mandato = models.ForeignKey(
        Mandato, on_delete=models.CASCADE, verbose_name=_("Mandato")
    )
    tipo_afastamento = models.ForeignKey(
        TipoAfastamento,
        on_delete=models.PROTECT,
    ...
    data_inicio = models.DateField(
        verbose_name=_("Início do Afastamento"), blank=False, null=True
    )
    data_fim = models.DateField(
        verbose_name=_("Fim do Afastamento"), blank=True, null=True
    )
    observacao = models.TextField(verbose_name=_("Observação"), blank=True)
    ...
    def __str__(self):
        return _("%(parlamentar)s %(legislatura)s") % {
            "parlamentar": self.parlamentar,
            "legislatura": self.mandato.legislatura,
        }
