...
class TipoNormaJuridica(models.Model):
    ...
    origem_processo_legislativo = models.BooleanField(
        null=True,
        blank=True,
        default=True,
        verbose_name=_("Possui Origem no Processo Legislativo?"),
...
class NormaJuridica(CommonMixin):
    ...
    mostrar_deliberacao = models.BooleanField(
        verbose_name=_("Mostrar Deliberação?"), choices=YES_NO_CHOICES, default=False
    )
    ...
    sufixo_urlize = models.CharField(
        max_length=10, verbose_name=_("Sufixo URLize"), blank=True, default=""
    )
    ...
    complemento = models.BooleanField(
        null=True, blank=True, verbose_name=_("Complementar ?"), choices=YES_NO_CHOICES
    )
    ...
    norma_de_destaque = models.BooleanField(
        verbose_name=_("Norma de Destaque ?"), choices=YES_NO_CHOICES, default=False
    )
    ...
    def get_normas_relacionadas(self):
        nrs = (
            NormaRelacionada.objects.filter(
                models.Q(norma_principal=self.id) | models.Q(norma_relacionada=self.id)
            )
            .select_related(
                "tipo_vinculo",
...
class AutoriaNorma(models.Model):
    ...
    norma = models.ForeignKey(
        NormaJuridica, on_delete=models.CASCADE, verbose_name=_("Norma Jurídica")
    )
