...
FORM_MESSAGES = {
    ACTION_CREATE: (_("Registro criado com sucesso!"), _("O registro não foi criado.")),
    ACTION_UPDATE: (
        _("Registro alterado com sucesso!"),
        _("Suas alterações não foram salvas."),
    ),
    ACTION_DELETE: (
        _("Registro excluído com sucesso!"),
        _("O registro não foi excluído."),
    ),
}
FORM_MESSAGES = {
    k: (a, _form_invalid_message(b)) for k, (a, b) in FORM_MESSAGES.items()
}
    ...
    if num_pages <= PAGINATION_LENGTH:
        ...
        if index - 1 <= 5:
            ...
            if index + 1 >= num_pages - 3:
                ...
                tail = [index - 1, index, index + 1, None, num_pages - 1, num_pages]
...
class ListWithSearchForm(forms.Form):
    q = forms.CharField(
        required=False, label="", widget=forms.TextInput(attrs={"type": "search"})
    )

    o = forms.CharField(required=False, label="", widget=forms.HiddenInput())
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Field("o"),
            FieldWithButtons(
                Field("q", placeholder=_("Filtrar Lista"), css_class="input-lg"),
                StrictButton(
                    _("Filtrar"), css_class="btn-outline-primary btn-lg", type="submit"
                ),
            ),
        )
...
class PermissionRequiredContainerCrudMixin(PermissionRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if "pk" in kwargs:
            ...
            if len(container) > 1:
                container_model = getattr(self.model, container[0]).field.related_model

                params = {}
                params["__".join(container[1:])] = request.user.pk
    ...
    def container_field_set(self):
        if hasattr(self, "crud") and not hasattr(self.crud, "container_field_set"):
...
class CrudBaseMixin(CrispyLayoutFormMixin):
    ...
    def __init__(self, **kwargs):
        ...
        if hasattr(self, "permission_required") and self.permission_required:
            ...
            self.permission_required = tuple(
                (
                    self.permission(pr)
                    for pr in (set(self.permission_required) - set(obj.public))
                )
            )
    ...
    def url_name_set(self, suffix):
        obj = self.crud if hasattr(self, "crud") else self
        return "%s_%s" % (
            getattr(obj.model, obj.model_set).field.model._meta.model_name,
            suffix,
        )

    def permission(self, rad):
        return "%s%s%s" % (
            self.app_label if rad.endswith("_") else "",
            rad,
    ...
    def permission_set(self, rad):
        return "%s%s%s" % (
            self.app_label_set if rad.endswith("_") else "",
            rad,
    ...
    def resolve_url_set(self, suffix, args=None):
        obj = self.crud if hasattr(self, "crud") else self
        namespace = getattr(obj.model, obj.model_set).field.model._meta.app_config.name
        return reverse("%s:%s" % (namespace, self.url_name_set(suffix)), args=args)
    ...
    def list_url(self):
        ...
        if not obj.ListView.permission_required:
            ...
            return (
                self.resolve_url(ACTION_LIST)
                if self.request.user.has_perm(self.permission(RP_LIST))
                else ""
            )
    ...
    def create_url(self):
        ...
        if not obj.CreateView.permission_required:
            ...
            return (
                self.resolve_url(ACTION_CREATE)
    ...
    def detail_url(self):
        ...
        if not obj.DetailView.permission_required:
            ...
            return (
                self.resolve_url(ACTION_DETAIL, args=(self.object.id,))
                if self.request.user.has_perm(self.permission(RP_DETAIL))
                else ""
            )
    ...
    def update_url(self):
        ...
        if not obj.UpdateView.permission_required:
            ...
            return (
                self.resolve_url(ACTION_UPDATE, args=(self.object.id,))
    ...
    def delete_url(self):
        ...
        if not obj.DeleteView.permission_required:
            ...
            return (
                self.resolve_url(ACTION_DELETE, args=(self.object.id,))
                if self.request.user.has_perm(self.permission(RP_DELETE))
                else ""
            )
    ...
    def get_template_names(self):
        ...
        names.append("crud/%s.html" % self.template_name_suffix.lstrip("_"))
        return names
...
class CrudListView(PermissionRequiredContainerCrudMixin, ListView):
    ...
    def _as_row(self, obj):
        ...
        for i, name in enumerate(self.list_field_names):
            url = self.resolve_url(ACTION_DETAIL, args=(obj.id,)) if i == 0 else None
            css_class = ""
            """Caso o crud list seja para uma relação ManyToManyField"""
            if (
                url
                and hasattr(self, "crud")
                and hasattr(self.crud, "is_m2m")
                and self.crud.is_m2m
            ):
                url = url + ("?pkk=" + self.kwargs["pk"] if "pk" in self.kwargs else "")
    ...
    def get_context_data(self, **kwargs):
        ...
        if hasattr(self, "form_search_class"):
            q = str(self.request.GET.get("q")) if "q" in self.request.GET else ""

            o = self.request.GET["o"] if "o" in self.request.GET else "1"

            if "form" not in kwargs:
                initial = self.get_initial() if hasattr(self, "get_initial") else {}
                initial.update({"q": q, "o": o})
                kwargs["form"] = self.form_search_class(initial=initial)
        ...
        if self.ordered_list:
            if "o" in qr:
                del qr["o"]
            context["ordering_url"] = ("&" + qr.urlencode()) if len(qr) > 0 else ""
        return context
...
class CrudCreateView(
    PermissionRequiredContainerCrudMixin, FormMessagesMixin, CreateView
):
        ...
        if self.container_field:
            ...
            if len(container) > 1:
                ...
                if not container_data:
                    raise Exception(
                        _(
                            "Não é permitido adicionar um registro "
                ...
                if (
                    ...
                    setattr(
                        self.object, container[1], getattr(container_data, container[1])
                    )
...
class CrudDetailView(
    PermissionRequiredContainerCrudMixin, DetailView, MultipleObjectMixin
):
        ...
        try:
            ...
            for fieldname in list_field_names_set:
                ...
                if hasattr(self.object, fieldname):
                    fv = (
                        getattr(self.object, obj.model_set)
            ...
            self.logger.error("user=" + username + ". " + str(e))
            obj = self.crud if hasattr(self, "crud") else self
            return [getattr(self.object, obj.model_set).model._meta.verbose_name_plural]

    def url_model_set_name(self, suffix):
        return "%s_%s" % (
            getattr(self.object, self.crud.model_set).model._meta.model_name,
            suffix,
        )

    def resolve_model_set_url(self, suffix, args=None):
        obj = self.crud if hasattr(self, "crud") else self
        namespace = getattr(self.object, obj.model_set).model._meta.app_config.name
        return reverse(
            "%s:%s" % (namespace, self.url_model_set_name(suffix)), args=args
        )
    ...
    def model_set_verbose_name(self):
        obj = self.crud if hasattr(self, "crud") else self
        return getattr(self.object, obj.model_set).model._meta.verbose_name
...
class CrudUpdateView(
    PermissionRequiredContainerCrudMixin, FormMessagesMixin, UpdateView
):
...
class CrudDeleteView(
    PermissionRequiredContainerCrudMixin, FormMessagesMixin, DeleteView
):
    ...
    def delete(self, request, *args, **kwargs):
        try:
            ...
            for i in err.protected_objects:
                error_msg += "<li>{} - {}</li>".format(i._meta.verbose_name, i)
                error_msg2 += "{} - {}, ".format(i._meta.verbose_name, i)
            error_msg2 = error_msg2[: len(error_msg2) - 2] + "."
            error_msg += "</ul>"

            username = request.user.username
            self.logger.error(
                "user=" + username + ". Registro não pode ser removido, pois "
    ...
    def get_urls(cls):
        ...
        def _add_base(view):
            ...
            if (
                ...
                and view.permission_required
                and hasattr(cls, "public")
                and cls.public
            ):
        ...
        cruds_base = [
            [
                CrudListView.get_url_regex() if CrudListView else None,
                CrudListView,
                ACTION_LIST,
            ],
            [
                CrudCreateView.get_url_regex() if CrudCreateView else None,
                CrudCreateView,
                ACTION_CREATE,
            ],
            [
                CrudDetailView.get_url_regex() if CrudDetailView else None,
                CrudDetailView,
                ACTION_DETAIL,
            ],
            [
                CrudUpdateView.get_url_regex() if CrudUpdateView else None,
                CrudUpdateView,
                ACTION_UPDATE,
            ],
            [
                CrudDeleteView.get_url_regex() if CrudDeleteView else None,
                CrudDeleteView,
                ACTION_DELETE,
            ],
        ]
        ...
        for regex_list, view, suffix in cruds:
            for regex, suf in regex_list:
                ...
                u = re_path(regex, view.as_view(), name=view.url_name(suf))
                urls.append(u)
...
class CrudAux(Crud):
    ...
    def build(cls, _model, _help_topic, _model_set=None, list_field_names=[]):
        ...
        ModelCrud = Crud.build(_model, _help_topic, _model_set, list_field_names)
...
class MasterDetailCrud(Crud):
    ...
    class BaseMixin(Crud.BaseMixin):
        ...
        def list_url(self):
            ...
            if not obj.ListView:
                return ""
            return (
                self.resolve_url(ACTION_LIST, args=(self.kwargs["pk"],))
                if self.request.user.has_perm(self.permission(RP_LIST))
                else ""
            )
        ...
        def create_url(self):
            ...
            if not obj.CreateView:
                return ""
            return (
                self.resolve_url(ACTION_CREATE, args=(self.kwargs["pk"],))
                if self.request.user.has_perm(self.permission(RP_ADD))
                else ""
            )
        ...
        def detail_url(self):
            ...
            return (
                (super().detail_url + (("?pkk=" + pkk) if pkk else ""))
                if self.request.user.has_perm(self.permission(RP_DETAIL))
                else ""
            )
        ...
        def update_url(self):
            ...
            pkk = self.request.GET["pkk"] if "pkk" in self.request.GET else ""
            return (
                (super().update_url + (("?pkk=" + pkk) if pkk else ""))
                if self.request.user.has_perm(self.permission(RP_CHANGE))
                else ""
            )
            ...
            if "__" in obj.parent_field:
                ...
                for field in fields:
                    ...
                    if isinstance(pm.field, ForeignKey):
                        parent_model = getattr(parent_model, field).field.related_model
                    else:
                        parent_model = getattr(parent_model, field).rel.related_model
                    pm = parent_model

            else:
                parent_model = getattr(self.model, obj.parent_field)
                if isinstance(parent_model.field, (ForeignKey, ManyToManyField)):
    ...
    class CreateView(Crud.CreateView):
        ...
        def get_form(self, form_class=None):
            obj = self.crud if hasattr(self, "crud") else self
            form = super(MasterDetailCrud.CreateView, self).get_form(self.form_class)
        ...
        def get_context_data(self, **kwargs):
            ...
            obj = self.crud if hasattr(self, "crud") else self
            context = Crud.CreateView.get_context_data(self, **kwargs)

            params = {"pk": self.kwargs["pk"]}

            if self.container_field:
                # FIXME refatorar para parent_field com '__'
                parent_model = getattr(self.model, obj.parent_field).field.related_model
        ...
        def cancel_url(self):
            ...
            return reverse(
                "%s:%s"
                % (
                    parent_model._meta.app_config.name,
    ...
    class DetailView(Crud.DetailView):
        ...
        def detail_list_url(self):
            ...
            if (
                obj.ListView.permission_required not in obj.public
                or self.request.user.has_perm(self.permission(RP_LIST))
            ):
                if "__" in obj.parent_field:
                    fields = obj.parent_field.split("__")
                    parent_object = self.object
            ...
            if self.request.user.has_perm(self.permission(RP_ADD)):
                ...
                if not obj.is_m2m:
                    ...
                    if "__" in obj.parent_field:
                        ...
                        parent_object = parent_object.filter(pk=root_pk).first()
        ...
        def detail_set_create_url(self):
            obj = self.crud if hasattr(self, "crud") else self
            if (
                hasattr(obj, "model_set")
                and obj.model_set
                and self.request.user.has_perm(self.permission_set(RP_ADD))
            ):
        ...
        def detail_root_detail_url(self):
            ...
            if hasattr(obj, "parent_field"):
                ...
                if not obj.is_m2m or len(parent_field) > 1:
                    ...
                    if isinstance(getattr(self.object, parent_field[0]), models.Model):
                        ...
                        return reverse(
                            ...
                            % (
                                namespace,
                                "%s_%s"
                                % (parent_object._meta.model_name, ACTION_DETAIL),
                            ),
                            args=(pk,),
                        )
            return ""
    ...
    @classonlymethod
    def build(
        cls, model, parent_field, help_topic, _model_set=None, list_field_names=[]
    ):
        crud = super(MasterDetailCrud, cls).build(
            model, help_topic, _model_set=_model_set, list_field_names=list_field_names
        )
...
class CrudBaseForListAndDetailExternalAppView(MasterDetailCrud):
    ...
    class BaseMixin(Crud.PublicMixin, MasterDetailCrud.BaseMixin):
        ...
        def resolve_url(self, suffix, args=None):
            ...
            """
            return reverse("%s:%s" % (obj.namespace, self.url_name(suffix)), args=args)
