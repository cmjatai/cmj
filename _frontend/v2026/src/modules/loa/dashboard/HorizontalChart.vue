      ...
      >
        ...
        >
          <FontAwesomeIcon
            ...
            class="me-2"
          />
          <span v-if="props.ready">Nenhum Registro encontrado</span>
          <span v-else>
            <b-spinner
              small
              class="me-2"
...
const agrupado = computed(() => {
  const map = {}
  for (const obj of props.objectList) {
    const label = obj.label || 'Sem Referência Registrada'
    if (!map[label]) map[label] = 0
    map[label] += Number(obj.valor)
  }
