  ...
  return {
    ...
    plugins: [
      ...
      VitePWA({
        ...
        manifest: {
          id: 'br.leg.go.jatai.portalcmj.v6',
          name: 'PortalCMJ 2026',
          short_name: 'PortalCMJ 2026',
          description: 'Portal da Câmara Municipal de Jataí - GO',
          background_color: '#ffffff',
          theme_color: '#114d81',
          start_url: mode === 'production' ? '/v6/' : 'http://localhost:9098/v6/',
          scope: mode === 'production' ? '/v6/' : 'http://localhost:9098/v6/',
          display: 'fullscreen',
          orientation: 'any',
          lang: 'pt-BR',
          icons: [
            {
              src:  mode === 'production' ? '/static/v6/imgs/icons/android-chrome-192x192.png' : 'imgs/icons/android-chrome-192x192.png',
              sizes: '192x192',
              type: 'image/png'
            },
            {
              src:  mode === 'production' ? '/static/v6/imgs/icons/android-chrome-512x512.png' : 'imgs/icons/android-chrome-512x512.png',
              sizes: '512x512',
              type: 'image/png'
            }
          ]
      ...
      }),
      {
        ...
        closeBundle: () => {
          ...
          const path = './dist/v6/.vite/manifest.json';
          const manifest = JSON.parse(fs.readFileSync(path).toString());
          Object.keys(manifest).forEach(key => {
            if (manifest[key].file) {
              manifest[key].file = 'v6/' + manifest[key].file;
            }
            if (manifest[key].css) {
              manifest[key].css = manifest[key].css.map(cssFile => 'v6/' + cssFile);
            }
            if (manifest[key].assets) {
              manifest[key].assets = manifest[key].assets.map(assetFile => 'v6/' + assetFile);
            }
          });
          // eslint-disable-next-line
    ...
    base: mode === 'production' ? '/static/v6/' :  '/static/',
