...
function insertWaitAjax (element) {
  ...
  $(element).append('<div style="text-align:center;"><i style="font-size: 200%;" class="fa fa-refresh fa-spin"></i></div>')
}
  ...
  $(function () {
    ...
    fields.each(function () {
      ...
      let onChangeParamTA = function (event) {
        ...
        let tipoResultado = $('input[name="tipo_resultado"]:checked').val()
        ...
        let taEditor = $('input[name="texto_articulado_do_editor"]').val()
        let url = ''

        if (rotuloDispositivo.length > 0 || textoDispositivo.length > 0) {
          $('input[name="tipo_resultado"]').prop('disabled', false)
          $('input[name="tipo_resultado"]').each((idx, element) => {
            element.parentElement.classList.remove('disabled')
          })
          $('input[name="tipo_resultado"]').closest('#div_id_tipo_resultado').css('opacity', '1')
        } else {
          $('input[name="tipo_resultado"]').filter('[value="False"]').prop('checked', true)
          $('input[name="tipo_resultado"]').prop('disabled', true)

          $('input[name="tipo_resultado"]').each((idx, element) => {
            element.parentElement.classList.add('disabled')
          })
          $('input[name="tipo_resultado"]').closest('#div_id_tipo_resultado').css('opacity', '0.3')
          tipoResultado = 'False'
        }
        formData = {
          'tipo_ta': tipoTa,
