<template>
  <div :class="[
    'individuo-base', status_microfone === 0 ? 'muted' : '',
    individuo && individuo.com_a_palavra ? 'com-a-palavra' : (individuo.microfone_sempre_ativo ? 'always-on' : 'active'),
...
export default {
  ...
  data () {
    return {
      ...
      aparteante: this.individuo && this.individuo.aparteado,
  ...
  watch: {
    individuo: function (newVal, oldVal) {
      console.log(this.individuo_id, 'individuo mudou', newVal, oldVal, newVal.name)
      if (newVal && oldVal) {
        ...
        if (newVal.com_a_palavra !== oldVal.com_a_palavra) {
          ...
          if (!newVal.com_a_palavra && oldVal.aparteante) {
            this.$emit('refreshOldAparteante', oldVal.aparteante)
          }
        }
      }
      this.refreshState({
    ...
    aparteante: function (newVal, oldVal) {
      console.log(this.individuo_id, 'aparteante mudou', newVal, oldVal)
      ...
      const query_params = [
        `&aparteante_status=${newVal ? 1 : 0}`,
        `&default_timer=${this.default_timer_aparteante || 60}`
      ]
      this.utils.getModelAction(
        'painelset', 'individuo', this.individuo_id, 'toggle_aparteante',
        query_params.join('')
      )
        .then(response => {
          console.log(this.individuo_id, 'aparteante, toggle_aparteante response', response)
        })
        .catch(error => {
          console.error(this.individuo_id, 'aparteante, toggle_aparteante error', error)
        })
    },
  ...
  methods: {
    ...
    clickIndividuo: function (event) {
      ...
      console.log(event)
      this.$emit('select-to-aparte', this.individuo.id)
    }
  }
}
</script>
