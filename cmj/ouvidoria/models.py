...
class Solicitacao(models.Model):
    ...
    titulo = models.CharField(default="", max_length=254, verbose_name=_("Título"))

    descricao = models.TextField(default="", verbose_name=_("Descrição"))

    visibilidade = models.IntegerField(
        _("Visibilidade"), choices=VISIBILIDADE_STATUS, default=STATUS_RESTRICT
    )

    areatrabalho = models.ForeignKey(
        AreaTrabalho, verbose_name=_("Área de Trablho"), on_delete=PROTECT
    )
    ...
    hash_code = models.TextField(
        verbose_name=_("Hash de Acesso Anônimo"), max_length=200, blank=True
    )

    notificacoes = GenericRelation(Notificacao, related_query_name="notificacoes")
    ...
    def save(
        ...
        if not self.hash_code:
            md5 = hashlib.md5()
            data = "{}{}{}".format(timezone.now(), self.titulo, self.descricao)
...
def anexo_ouvidoria_path(instance, filename):
    return "./ouvidoria/mensagem/%s/%s" % (instance.id, filename)
