...
from cmj.api.serializers_painelset import (
    CronometroSerializer,
    CronometroTreeSerializer,
    EventoSerializer,
    IndividuoSerializer,
)
from cmj.painelset.cronometro_manager import CronometroManager
from cmj.painelset.models import (
    Cronometro,
    CronometroState,
    Evento,
    Individuo,
...
ApiViewSetConstrutor.build_class([apps.get_app_config("painelset")])
...
class EventoChangePermission(BasePermission):
    def has_permission(self, request, view):
        return (
            ...
            and request.user.has_perm("painelset.change_evento")
        )
...
class _EventoViewSet:
    ...
    def export(self, request, pk=None, format="yaml"):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="evento_{evento.id}_export.yaml"'
        )
        return response
    ...
    def start(self, request, pk=None):
        ...
        if cronometro:
            if (
                not evento.start_real
                and cronometro.started_at
                and not cronometro.finished_at
            ):
        ...
        return Response(
            {"error": "Cronometro not found"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    def copy(self, request, *args, **kwargs):
        ...
        if not evento_copiado.end_real:
            return Response(
                {
                    "error": "Evento não finalizado. Só é possível copiar eventos finalizados."
                },
    ...
    def toggle_microfones(self, request, *args, **kwargs):
        ...
        inclui_microfone_sempre_ativo = request.data.get(
        ...
        for individuo in evento.individuos.all():
            if (
                individuo.microfone_sempre_ativo
                and inclui_microfone_sempre_ativo == "off"
            ):
                logger.debug(
                    f"  Individuo {individuo} tem microfone sempre ativo, pulando"
                )
                continue
        ...
        if SEND_MESSAGE_MICROPHONE or not settings.DEBUG:
                ...
                for ip in ips_mesas:
                    if ip not in clients:
                        try:
                            ...
                            logger.error(
                                f"Erro ao criar cliente OSC para o IP {ip}: {e}"
                            )
            ...
            for individuo in evento.individuos.all():
                for ip in individuo.ips:
                    ...
                    if client:
                        client.send_message(
                            f"/ch/{individuo.channel_display}/mix/on",
                            1 if status_microfone == "on" else 0,
                        )

        return Response(
            {"status": "ok", "status_microfone": status_microfone, "evento": evento.id}
        )
...
class _IndividuoViewSet(ResponseFileMixin):
    ...
    def force_stop_cronometro(self, request, *args, **kwargs):
        ...
        logger.debug(
            f"Forçando stop do cronometro do indivíduo {individuo}: status_microfone {individuo.status_microfone}, com_a_palavra={individuo.com_a_palavra}"
        )
        cron, created = individuo.get_or_create_unique_cronometro()
        if cron.state != CronometroState.STOPPED:
            cronometro_manager.stop_cronometro(cron.id)
        return Response({"status": "ok", "individuo": individuo.id})
    ...
    def toggle_aparteante(self, request, *args, **kwargs):
        ...
        try:
            ...
            default_timer = timedelta(seconds=default_timer, microseconds=300_000)
        except ValueError:
            default_timer = timedelta(seconds=60, microseconds=300_000)

        logger.debug(
            f"timer: {default_timer} Toggle aparteante {individuoAparteante}: status_microfone {individuoAparteante.status_microfone}, com_a_palavra={individuoAparteante.com_a_palavra}"
        )

        individuoComPalavra = individuoAparteante.evento.individuos.filter(
            com_a_palavra=True
        ).first()

        if not individuoComPalavra:
            raise DRFValidationError(
                "A palavra não está sendo utilizada. Não é possível fazer aparte."
            )
        else:
            ...
            if individuoComPalavra.id == individuoAparteante.id:
                ...
                logger.debug(
                    f"  Indivíduo {individuoAparteante} já está com a palavra, nada a fazer"
                )
                return Response({"status": "ok", "individuo": individuoAparteante.id})
            else:
                ...
                cronAparteante, created = (
                    individuoAparteante.get_or_create_unique_cronometro()
                )
                cronAparteado, created = (
                    individuoComPalavra.get_or_create_unique_cronometro()
                )

                if not aparteante_status:
                    # Ele não está com a palavra, e quer desligar o aparteante
                    logger.debug(
                        f"  Indivíduo {individuoAparteante} não está com a palavra, e quer se desligar do aparteante. Desligando aparte."
                    )
                    individuoComPalavra.aparteante = None
                    individuoComPalavra.save(update_fields=["aparteante"])
                    ...
                    logger.debug(
                        f"  Indivíduo {individuoAparteante} não está com a palavra, mas o indivíduo {individuoComPalavra} está. Permitindo aparte."
                    )
                    cronometro_manager.start_cronometro(
                        cronAparteante.id, cronAparteado.id, duration=default_timer
                    )
    ...
    def toggle_microfone(self, request, *args, **kwargs):
        ...
        individuo = self.get_object()
        mic_old = {
            "order": individuo.channel_display,
            "status_microfone": "on" if individuo.status_microfone else "off",
        }
        ...
        logger.debug(
            f"Toggle microfone {individuo.id} - {individuo}: status_microfone {individuo.status_microfone}, com_a_palavra={individuo.com_a_palavra}"
        )
        status_microfone = request.data.get("status_microfone", "on")
        ...
        try:
            ...
            default_timer = timedelta(seconds=default_timer, microseconds=micro)
        except ValueError:
            default_timer = timedelta(seconds=300, microseconds=300_000)

        logger.debug(
            f"  timer: {default_timer}: status_microfone={status_microfone}, com_a_palavra={com_a_palavra}"
        )
        ...
        if com_a_palavra == 1:
            ...
            ind_com_a_palavra = individuo.evento.individuos.filter(
                com_a_palavra=True
            ...
            for ind in ind_com_a_palavra:
                ...
                if aparteante:
                    cronAparteante, created = (
                        aparteante.get_or_create_unique_cronometro()
                    )
                    aparteante.save()
                    cronometro_manager.stop_cronometro(cronAparteante.id)
            ...
            if hasattr(individuo, "aparteado") and individuo.aparteado:
                if individuo.status_microfone:
                    if cron.state == CronometroState.PAUSED:
                        ...
                        cronometro_manager.start_cronometro(
                            cron.id, duration=default_timer
                        )
                else:
                    cronometro_manager.pause_cronometro(cron.id)
            else:
        ...
        # obter status_microfone e com_a_palavra de todos os individuos do evento
        microfones_do_evento_depois = list(
            individuo.evento.individuos.values("order", "status_microfone")
        )
        ...
        if mic != mic_old:
            logger.debug(
                f'Enviando comando OSC para o microfone {mic["order"]}: status_microfone={mic["status_microfone"]}'
            )
            if SEND_MESSAGE_MICROPHONE or not settings.DEBUG:
                ips_mesas = individuo.ips
                for ip in ips_mesas:
                    try:
                        ...
                        client.send_message("/xremote", None)
                        client.send_message(
                            f"/ch/{mic['order']}/mix/on",
                            1 if mic["status_microfone"] == "on" else 0,
                        )
                    except Exception as e:
                        logger.error(
                            f'Erro ao enviar comando OSC para o microfone {mic["order"]} no IP {ip}: {e}'
                        )
