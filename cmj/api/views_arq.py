...
from cmj.api.serializers import (
    ArqClasseSerializer,
    ArqDocSerializer,
    DraftMidiaSerializer,
)
...
ApiViewSetConstrutor.build_class([apps.get_app_config("arq")])
...
class _Draft:
    ...
    def create(self, request, *args, **kwargs):
        def get_serializer(*args, **kwargs):
            ...
            serializer = super(type(self), self).get_serializer(*args, **kwargs)
            return serializer
    ...
    def uploadfiles(self, request, *args, **kwargs):
        ...
        smax = (
            DraftMidia.objects.filter(draft=draft)
            .aggregate(smax=Max("sequencia"))
            .get("smax", 1)
            or 0
        )
        ...
        for i, f in enumerate(files):
            files[i] = {"file": f, "lastmodified": lastmodified[i]}
        ...
        for seq, item in enumerate(files, start=smax + 1):
            ...
            if tipo not in (
                ...
                raise ValidationError(
                    _(
                        "Os arquivos possíveis de envio são do tipo: PDF, JPG, PNG, ODT, DOC, DOCX, ODS, XLS, XLSX, TIF, TIFF"
                    )
                )
            ...
            dm.metadata = {
                "uploadedfile": {"name": f.name, "paginas": 0},
                "ocrmypdf": {"pdfa": DraftMidia.METADATA_PDFA_NONE},
            }
            ...
            if tipo in ("jpg", "png"):
                ...
                if len(images) == 1:
                    ...
                    images[0].save(fpdf, save_all=True, append_images=images[1:])
                ...
                try:
                    ...
                    raise ValidationError("Erro na execução de conversão de arquivo")
                else:
    ...
    def zipfile(self, request, *args, **kwargs):
        ...
        with tempfile.SpooledTemporaryFile(max_size=512000000) as tmp:
            ...
            with zipfile.ZipFile(tmp, "w") as file:
                ...
                for dm in draft.draftmidia_set.all():
                    file.write(dm.arquivo.path, dm.metadata["uploadedfile"]["name"])
        ...
        response["Expires"] = 0
        response["Content-Disposition"] = "inline; filename=%s.zip" % (
    ...
    def unirmidias(self, request, *args, **kwargs):
        ...
        dm_primary.metadata["ocrmypdf"] = {"pdfa": DraftMidia.METADATA_PDFA_NONE}
    ...
    def pdf2pdfa(self, request, *args, **kwargs):
        ...
        if not DEBUG or (
            DEBUG and settings.FOLDER_DEBUG_CONTAINER == settings.PROJECT_DIR
        ):
            tasks.task_ocrmypdf.apply_async(params_task, countdown=5)
        else:
...
class _DraftMidia(ResponseFileMixin):
    ...
    def decompor(self, request, *args, **kwargs):
        ...
        self.get_queryset().filter(
            draft_id=dm_atual.draft_id, sequencia__gt=dm_atual.sequencia
        ...
        for p in range(ldoc):
            ...
            dm_new.metadata["ocrmypdf"] = {"pdfa": DraftMidia.METADATA_PDFA_NONE}
            dm_new.save()
    ...
    def rotate(self, request, *args, **kwargs):
        ...
        nd.insert_pdf(doc, from_page=p, to_page=p, rotate=doc[p].rotation + angulo)
        if p != lp - 1:
