...
from cmj.api.serializers import (
    DocumentoChoiceSerializer,
    DocumentoSerializer,
    DocumentoUserAnonymousSerializer,
)
from cmj.core.models import Bi
from cmj.sigad.models import Documento, Midia, ReferenciaEntreDocumentos, VersaoDeMidia
...
class AppVersionView(APIView):
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            ...
            try:
                avatar = (
                    get_static("img/perfil.jpg")
                    if not request.user.avatar
                    else get_backend().get_thumbnail_url(
                        request.user.avatar,
                        {
                            "size": (128, 128),
        ...
        if perm:
            if not request.user.has_perm(perm):
                return Response(
                    {
                        "status": "fail",
                        "detail": _("Usuário não possui permissão: %s") % perm,
                    }
                )
            return Response(
                {"status": "ok", "detail": _("Usuário possui permissão: %s") % perm}
            )
...
class DocumentoViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        self.queryset = Documento.objects.qs_bi(user=request.user).order_by("-created")
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if cita:
            ...
            response = viewsets.ModelViewSet.destroy(self, request, *args, **kwargs)
            ...
            for file in files:
                try:
                    with transaction.atomic():
                        ...
                        image.tipo = (
                            Documento.TPD_IMAGE
                            if instance.tipo != Documento.TPD_CONTAINER_FILE
                            else Documento.TPD_FILE
                        )
