...
def task_refresh_states_from_visaodepainel_function(*args, **kwargs):
    ...
    # logger.info("Iniciando atualização dos estados das visões de painel")

    visoes_changed = {"deactivated": [], "activated": []}

    eventos = {}

    for v in (
        VisaoDePainel.objects.filter(
            painel__evento__end_real__isnull=True, painel__auto_select_visoes=True
        )
        .annotate(
            evento_id=F("painel__evento_id"),
            sessao_id=F("painel__sessao_id"),
        )
        .order_by("painel__evento_id", "painel__id", "position")
    ):
    ...
    for evento_id, paineis in eventos.items():
        # logger.info(f"Avaliando visões de painel para evento {evento_id}")
        for painel_id, visoes in paineis.items():
            # logger.info(f"Avaliando visões de painel para painel {painel_id}")
                    ...
                    for key, lookups in activate.items():
                        ...
                        try:
                            for key_lookup, condition in lookups.items():
                                filter_maps = {}
                                app_label, model_name, field_name = key_lookup.split(
                                    "__", 2
                                )
                                filter_maps.setdefault((app_label, model_name), {})[
                                    field_name
                                ] = condition["value"]
                                ...
                                for (
                                    app_label,
                                    model_name,
                                ), filter_fields in filter_maps.items():
                                    ...
                                    for (
                                        ...
                                        if related_model == Evento:
                                            ...
                                            visao.sessao_id
                                            and related_model == SessaoPlenaria
                                        ):
                                            filter_fields[f"{field_name}"] = (
                                                visao.sessao_id
                                            )
                            ...
                            logger.error(
                                f"Erro ao avaliar condição de ativação da visão de painel {visao.id}: {e}"
                            )
                            filter_match.append(False)
