...
from sapl.utils import PortalImageCropField, SaplGenericForeignKey
...
class CronometroMixin:
    ...
    def get_or_create_unique_cronometro(self, duration=None):
        ...
        if (
            ...
            and cronometro.duration != duration_owner
        ):
...
class Cronometro(models.Model):
    ...
    state = models.CharField(
        choices=CronometroState.choices, default=CronometroState.STOPPED, max_length=20
    )
    ...
    pause_parent_on_start = models.BooleanField(
        default=False, help_text="Pausar o cronômetro pai quando este iniciar"
    )
...
class CronometroEvent(models.Model):
    ...
    cronometro = models.ForeignKey(
        Cronometro, on_delete=models.CASCADE, related_name="events"
    )
    event_type = models.CharField(max_length=20, choices=EVENT_TYPES)
    timestamp = models.DateTimeField(auto_now_add=True)
    triggered_by_child = models.ForeignKey(
        Cronometro,
...
class Evento(models.Model, CronometroMixin):
    ...
    start_previsto = models.DateTimeField(
        null=True, blank=True, verbose_name="Data e hora Prevista de Início"
    )

    start_real = models.DateTimeField(
        null=True, blank=True, verbose_name="Data e hora Real de Início"
    )
    end_real = models.DateTimeField(
        null=True, blank=True, verbose_name="Data e hora Real de Término"
    )

    duration = models.DurationField(
        help_text="Duração total planejada do evento", verbose_name="Duração do Evento"
    )
    ...
    comunicar_com_mesas = models.BooleanField(
        default=False,
    ...
    def reset_to_defaults(self):
        ...
        fixture_file = settings.BASE_DIR.child(
            "painelset", "fixtures", "painelset_defaults.yaml"
        )
            ...
            for visao in painel.visoes.all():
                ...
                for widget in visao.widgets.all():
                    widget_data = {
                        ...
                        "parent": (
                            f"widget-{widget.parent_id}" if widget.parent_id else None
                        ),
                        "name": widget.name,
...
class IndividuoManager(models.Manager):
    ...
    def reset_ordem(self):
        ...
        individuos = sorted(
            list(individuos), key=lambda x: (RoleChoices.names.index(x.role), x.name)
        )
...
class Individuo(models.Model, CronometroMixin):
    ...
    order = models.PositiveIntegerField(
        help_text="Ordem do indivíduo no Evento", default=0
    )

    canal = models.PositiveIntegerField(
        help_text="Canal do indivíduo no Evento", default=0
    )
    ...
    status_microfone = models.BooleanField(
        default=False, help_text="Indica se o microfone está ativo para este indivíduo"
    )
    auto_corte_microfone = models.BooleanField(
        default=False, help_text="Indica se o microfone foi cortado automaticamente"
    )

    microfone_sempre_ativo = models.BooleanField(
        default=False,
        help_text="Indica se o microfone deste indivíduo deve estar sempre ativo",
    )
    tempo_de_corte_microfone = models.DurationField(
        default=timedelta(seconds=5),
        help_text="Duração do corte de microfone após o tempo do cronômetro zerar",
    )

    com_a_palavra = models.BooleanField(
        default=False, help_text="Indica se o indivíduo está com a palavra"
    )
    ...
    fotografia = PortalImageCropField(
        verbose_name=_("Fotografia"), upload_to=foto_upload_path, null=True, blank=True
    ...
    def save(
        ...
        if not self.pk and self.order == 0:
            max_order = Individuo.objects.filter(evento=self.evento).aggregate(
                models.Max("order")
            )["order__max"]
...
class Painel(models.Model):
    ...
    auto_select_visoes = models.BooleanField(
        default=True,
    ...
    def ws_serialize(self):
        ...
        PainelSerializer = ApiViewSetConstrutor._built_sets[self._meta.app_config][
            self._meta.model
        ].serializer_class
...
class VisaoDePainel(models.Model):
    ...
    name = models.CharField(
        max_length=256, blank=True, help_text="Nome da Visão no Painel Associado"
    )
    ...
    position = models.PositiveIntegerField(
        help_text="Posição da visão no painel", default=0
    )

    active = models.BooleanField(
        default=False, help_text="Indica se a visão está ativa no painel"
    )
    ...
    def save(
        ...
        if not self.pk and self.position == 0:
            max_position = VisaoDePainel.objects.filter(painel=self.painel).aggregate(
                models.Max("position")
            )["position__max"]
    ...
    def ws_serialize(self):
        ...
        ][self._meta.model].serializer_class
...
class Widget(models.Model):
    ...
    name = models.CharField(
        max_length=256, blank=True, help_text="Nome do Widget na Visão Associada"
    )
    description = models.TextField(
    ...
    position = models.PositiveIntegerField(
        help_text="Posição do widget na visão", default=0
    )
    visible = models.BooleanField(
        default=True, help_text="Indica se o widget está visível na visão"
    )
