...
def task_cancel_auto_corte_microfone(self, individuo_id):
    ...
    try:
        ...
        logger_celery.error(
            f"Individuo com ID {individuo_id} não encontrado para cancelar auto corte de microfone"
        )
...
def task_check_finished_cronometros_function():
    ...
    if SEND_MESSAGE_MICROPHONE or not settings.DEBUG:
        ...
        for ip, client_info in clients.items():
            ...
            if "last_send" in client_info and (
                timezone.now() - client_info["last_send"]
            ) > timedelta(seconds=300):
                logger_celery.info(
                    f"Removendo cliente OSC {client._address}:{client._port} por inatividade"
                )
                remove_ips.append(ip)
            elif "last_ping" in client_info and (
                timezone.now() - client_info["last_ping"]
            ) > timedelta(seconds=7):
                try:
                    logger_celery.debug(
                        f"Enviando comando /xremote para {client._address}:{client._port} por inatividade"
                    )
                    client.send_message("/xremote", None)
                    client_info["last_ping"] = timezone.now()
                except Exception as e:
                    logger_celery.error(
                        f"Erro ao enviar mensagem OSC para {client._address}:{client._port}: {e}"
                    )
                    remove_ips.append(ip)
    ...
    if zero_time_cronometros:
        ...
        for cronometro in zero_time_cronometros:
            ...
            corte_religar = (
                individuo.tempo_de_corte_microfone.total_seconds() if individuo else 10
            )
            ...
            for ip in ips:
                if ip not in clients and (
                    SEND_MESSAGE_MICROPHONE or not settings.DEBUG
                ):
                    clients[ip] = {
            ...
            logger_celery.info(
                f"Cronometro {cronometro.name} (ID: {cronometro.id}) tempo encerrado"
            )
            ...
            for ip in ips:
                ...
                try:
                    if (
                        timedelta(seconds=int(corte_religar * (1.5)))
                        < remaining_time
                        < timedelta(seconds=corte_religar)
                    ):
                        logger_celery.debug(
                            f"Microfone religado para o cronômetro {cronometro.name} (ID: {cronometro.id})"
                        )
                        ...
                        if individuo.status_microfone:
                            if client and (
                                ...
                                client.send_message(
                                    f"/ch/{individuo.channel_display}/mix/on", 1
                                )
                        ...
                        logger_celery.debug(
                            f"Corte de microfone detectado no cronômetro {cronometro.name} (ID: {cronometro.id})"
                        )
                        if (
                            individuo.status_microfone
                            and not individuo.auto_corte_microfone
                        ):
                            ...
                            task_cancel_auto_corte_microfone.apply_async(
                                args=[individuo.id], countdown=-1 * corte_religar
                            )

                        if (
                            not individuo.status_microfone
                            and individuo.auto_corte_microfone
                        ):
                            if client and (
                                SEND_MESSAGE_MICROPHONE or not settings.DEBUG
                            ):
                                client_info["last_ping"] = timezone.now()
                                client.send_message(
                                    f"/ch/{individuo.channel_display}/mix/on", 0
                                )
...
def task_painelset_refresh_states(self):
    ...
    zero_time_cronometros, running_cronometros = (
        task_check_finished_cronometros_function()
    )
