...
def app_vue_v6(request, slug=None):
    return render(request, "app_vue_v6.html")
...
class EventoCrud(Crud):
    ...
    class DetailView(Crud.DetailView):
        ...
        def hook_start_previsto(self, obj, verbose_name, field_display):
            if obj.start_previsto:
                return verbose_name, formats.date_format(
                    timezone.localtime(obj.start_previsto), "d/m/Y - H:i"
                )
            return verbose_name, ""
        ...
        def hook_start_real(self, obj, verbose_name, field_display):
            if obj.start_real:
                return verbose_name, formats.date_format(
                    timezone.localtime(obj.start_real), "d/m/Y - H:i"
                )
            return verbose_name, ""
        ...
        def hook_end_real(self, obj, verbose_name, field_display):
            if obj.end_real:
                return verbose_name, formats.date_format(
                    timezone.localtime(obj.end_real), "d/m/Y - H:i"
                )
            return verbose_name, ""
...
class IndividuoCrud(MasterDetailCrud):
    ...
    class ListView(MasterDetailCrud.ListView):
        ...
        def hook_name(self, obj, default, url):
            return '<a href="{}" pk="{}">{}</a>'.format(url, obj.id, obj.name), ""
