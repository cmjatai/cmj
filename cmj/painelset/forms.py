...
class EventoForm(forms.ModelForm):
    ...
    duration = forms.DurationField(
        ...
        required=True,
        help_text=_(
            "Duração planejada do cronômetro para este evento (formato: HH:MM:SS)"
        ),
    ...
    individuos_extras = forms.IntegerField(
        ...
        max_value=50,
        help_text=_(
            "Número de indivíduos, além dos vinculados a parlamentares e tribunas, que este evento terá"
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if evento and evento.pk:
            ...
            self.fields["individuos_extras"].initial = evento.individuos.filter(
                parlamentar__isnull=True, role=RoleChoices.INDIVIDUO
            ).count()
            self.fields["tribunas"].initial = evento.individuos.filter(
                role=RoleChoices.TRIBUNA
            ).count()
            self.fields["vincular_parlamentares"].initial = evento.individuos.filter(
                parlamentar__isnull=False
            ).exists()
    ...
    def save(self, commit=True):
        ...
        if vincular_parlamentares:
            parlamentares = Parlamentar.objects.filter(ativo=True).order_by(
                "nome_parlamentar"
            )
        ...
        qs_individuos_extras = Individuo.objects.filter(
            role=RoleChoices.INDIVIDUO, evento=evento
        )
