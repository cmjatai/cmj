...
class PerfilAbstractCrud(Crud):
    ...
    class BaseMixin(Crud.BaseMixin):
        ...
        def detail_url(self):
            return (
                self.resolve_url(base.ACTION_DETAIL)
        ...
        def update_url(self):
            return (
                self.resolve_url(base.ACTION_UPDATE)
                if self.request.user.has_perm(self.permission(map_rules.RP_CHANGE))
                else ""
            )
        ...
        def delete_url(self):
            return (
                self.resolve_url(base.ACTION_DELETE)
                if self.request.user.has_perm(self.permission(map_rules.RP_DELETE))
                else ""
            )
    ...
    class DeleteView(Crud.DeleteView):
        ...
        def check_permission(self):
            if (
                self.object.perfil_user.groups.all()
                .exclude(name=GROUP_SOCIAL_USERS)
                .exists()
            ):
    ...
    def get_urls(cls):
        ...
        return [
            ...
            for regex, view, suffix in [
                (CrudListView.get_url_regex(), CrudDetailView, base.ACTION_DETAIL),
                (CrudCreateView.get_url_regex(), CrudCreateView, base.ACTION_CREATE),
...
class PerfilDetailCrudPermission(MasterDetailCrud):
    ...
    class BaseMixin(MasterDetailCrud.BaseMixin):
        ...
        def list_url(self):
            return (
                self.resolve_url(base.ACTION_LIST)
        ...
        def create_url(self):
            return (
                self.resolve_url(base.ACTION_CREATE)
        ...
        def detail_url(self):
            return (
                self.resolve_url(base.ACTION_DETAIL, args=(self.object.id,))
        ...
        def update_url(self):
            return (
                self.resolve_url(base.ACTION_UPDATE, args=(self.object.id,))
        ...
        def delete_url(self):
            return (
                self.resolve_url(base.ACTION_DELETE, args=(self.object.id,))
        ...
        def get_context_data(self, **kwargs):
            ...
            context = super(
                MasterDetailCrudPermission.BaseMixin, self
        ...
        def _as_row(self, obj):
            return [
                (
                    ...
                    self.resolve_url(base.DETAIL, args=(obj.id,)) if i == 0 else None,
                )
    ...
    class CreateView(MasterDetailCrud.CreateView):
        ...
        def get_form(self, form_class=None):
            form = super(CrudCreateView, self).get_form(self.form_class)
    ...
    def get_urls(cls):
        ...
        return [
            re_path(regex, view.as_view(), name=view.url_name(suffix))
            for regex, view, suffix in [
                (CrudListView.get_url_regex(), CrudListView, base.ACTION_LIST),
                (CrudCreateView.get_url_regex(), CrudCreateView, base.ACTION_CREATE),
                (CrudDetailView.get_url_regex(), CrudDetailView, base.ACTION_DETAIL),
                (CrudUpdateView.get_url_regex(), CrudUpdateView, base.ACTION_UPDATE),
                (CrudDeleteView.get_url_regex(), CrudDeleteView, base.ACTION_DELETE),
            ]
        ]
