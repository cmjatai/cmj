...
RENAMING_IGNORED_FIELDS = [
    ...
    (
        sapl.sessao.models.SessaoPlenaria,
        {"finalizada", "upload_pauta", "upload_ata", "iniciada"},
    ),
...
def test_get_renames():
    ...
    for model in all_models:
        ...
        if model not in field_renames:
            ...
            match_msg_template = (
                "All %s field names mentioned in renames " "must match a %s field"
            )

            # all renamed field references correspond to a current field
            assert renamed <= field_names, match_msg_template % ("new", "current")
            ...
            assert (
                set(field_renames[model].values()) <= legacy_field_names
