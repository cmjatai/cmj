...
from cmj.crispy_layout_mixin import heads_and_tails
...
def extract_title_and_fieldsets(model):
    filename = os.path.join(
        os.path.dirname(__file__), "original_forms/%s.html" % model.__name__
    )
    try:
...
old_names_adjustments = yaml.load(
    pkg_resources.resource_string(__name__, "old_names_adjustments.yaml")
)
