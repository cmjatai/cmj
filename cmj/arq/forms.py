...
class ArqClasseForm(ModelForm):
    ...
    perfil = forms.ChoiceField(
        label=ArqClasse._meta.get_field("perfil").verbose_name, choices=PERFIL_ARQCLASSE
    )
    ...
    parent = forms.ModelChoiceField(
        queryset=ArqClasse.objects.all(), widget=forms.HiddenInput(), required=False
    )

    render_tree2 = forms.TypedChoiceField(
        label=_("Renderização Tree2"), required=False, choices=YES_NO_CHOICES
    )

    class Meta:
        model = ArqClasse
        fields = ["codigo", "titulo", "perfil", "descricao", "parent", "render_tree2"]
    ...
    def clean_perfil(self):
        ...
        if perfil != str(self.parent.perfil):
            raise ValidationError(f"""Os perfis das SubArqClasses devem ser iguais!""")
        return perfil
...
class ArqDocForm(ModelForm):
    ...
    classe_logica = forms.ModelChoiceField(
        queryset=ArqClasse.objects.filter(perfil=ARQCLASSE_LOGICA), required=True
    )

    classe_estrutural = forms.ModelChoiceField(
        queryset=ArqClasse.objects.filter(perfil=ARQCLASSE_FISICA), required=True
    )
...
class ArqDocBulkCreateForm(ModelForm):
    ...
    class Meta:
        model = ArqDoc
        fields = ["data", "classe_logica", "classe_estrutural", "draftmidia"]
    ...
    def save(self, commit=True):
        ...
        for dm in dm_qs.all():
            try:
                ...
                inst.titulo = (
                    f'{dm.draft.descricao} - {dm.metadata["uploadedfile"]["name"]}'
                )
