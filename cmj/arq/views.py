...
from sapl.crud.base import CrudBaseMixin, CrudDetailView
...
class ArqClasseParentMixin(CheckCheckMixin):
    ...
    def cancel_url(self):
        if "pk" not in self.kwargs:
            ...
            return reverse_lazy(
                "cmj.arq:subarqclasse_list", kwargs={"pk": self.kwargs["pk"]}
            )

    def get_success_url(self):
        return reverse_lazy("cmj.arq:subarqclasse_list", kwargs={"pk": self.object.id})


class ArqClasseCreateView(
    ArqClasseParentMixin, FormMessagesMixin, PermissionRequiredMixin, CreateView
):
    ...
    def get_initial(self):
        ...
        cod__max = (
            ArqClasse.objects.filter(parent=self.parent)
            .order_by("codigo")
...
class ArqClasseUpdateView(
    ArqClasseParentMixin, FormMessagesMixin, PermissionRequiredMixin, UpdateView
):
...
class ArqClasseDeleteView(PermissionRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse_lazy("cmj.arq:subarqclasse_list", kwargs={"pk": self.parent.id})
...
class ArqClasseListView(ArqClasseParentMixin, PermissionRequiredMixin, ListView):
        ...
        if toggle_padlock is not None:
            if not request.user.is_superuser:
                messages.add_message(
                    ...
                    messages.ERROR,
                    _(
                        "Seu usuário não possui permissão para Trancar/Destrancar ArqClasse"
                    ),
                )
    ...
    def create_url(self):
        ...
        if "pk" not in self.kwargs:
            ...
            return reverse_lazy(
                "cmj.arq:subarqclasse_create", kwargs={"pk": self.kwargs["pk"]}
            )
    ...
    def update_url(self):
        if not self.request.user.has_perm("arq.change_arqclasse"):
            return ""
        return reverse_lazy("cmj.arq:arqclasse_edit", kwargs={"pk": self.kwargs["pk"]})
    ...
    def delete_url(self):
        if not self.object.parent and not self.request.user.is_superuser:
            ...
            return reverse_lazy(
                "cmj.arq:arqclasse_delete", kwargs={"pk": self.kwargs["pk"]}
            )
    ...
    def get_queryset(self):
        ...
        qs = list(qpub)
        pubs = ArqClasse.objects.all().select_related("parent", "parent__parent")
        for pub in pubs:
            parents = pub.parents
            for p in parents[::-1]:
...
class ArqDocMixin(CheckCheckMixin):
    ...
    def title(self):
        ...
        return "%s<br><small>(%s)</small>" % (
            self.object.titulo,
            _("Edição de ArqDocumento"),
        )

    @property
    def cancel_url(self):
        return reverse_lazy(
            "cmj.arq:subarqclasse_list", kwargs={"pk": self.kwargs["classe_id"]}
        )

    def get_success_url(self):
        return reverse_lazy(
            "cmj.arq:arqdoc_detail",
            kwargs={"classe_id": self.kwargs["classe_id"], "pk": self.object.id},
        )


class ArqDocUpdateView(
    ArqDocMixin, FormMessagesMixin, PermissionRequiredMixin, UpdateView
):
...
class ArqDocDetailView(
    ...
    def create_url(self):
        return reverse_lazy(
            "cmj.arq:arqdoc_create", kwargs={"classe_id": self.kwargs["classe_id"]}
        )
    ...
    def update_url(self):
        return reverse_lazy(
            "cmj.arq:arqdoc_edit",
            kwargs={"pk": self.object.pk, "classe_id": self.kwargs["classe_id"]},
        )
    ...
    def hook_classe_estrutural(self, obj):
        return (
            ...
            f"{obj.conta_estrutural} - {obj.classe_estrutural}",
        )
    ...
    def hook_created(self, obj):
        return "Em", formats.date_format(
            timezone.localtime(obj.created), "d/m/Y - H:i:s"
        )
    ...
    def hook_modified(self, obj):
        return "Em", formats.date_format(
            timezone.localtime(obj.modified), "d/m/Y - H:i:s"
        )

    def hook_pagina1(self, obj):
        return (
            "Primeira Página do Docuemnto",
            f"""
...
class ArqDocCreateView(
    ArqDocMixin, FormMessagesMixin, PermissionRequiredMixin, CreateView
):
    ...
    def get_initial(self):
        ...
        cod__max = (
            ArqDoc.objects.filter(classe_estrutural=self.kwargs["classe_id"])
            .order_by("codigo")
            .aggregate(Max("codigo"))
        )
...
class ArqDocDeleteView(ArqDocMixin, PermissionRequiredMixin, DeleteView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "cmj.arq:subarqclasse_list", kwargs={"pk": self.object.classe_estrutural_id}
        )
