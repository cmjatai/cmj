...
from cmj.diarios.models import DiarioOficial, TipoDeDiario, VinculoDocDiarioOficial
...
class DiarioOficialCrud(Crud):
    ...
    class DetailView(Crud.PublicMixin, Crud.DetailView):
        ...
        def hook_vinculodocdiariooficial_set(self, obj):
            v = (
                force_str(_("Documentos Públicados no PortalCMJ"))
                if self.object.tipo.principal
                else force_str(_("Republicação de documentos no PortalCMJ"))
            )
            text = []
        ...
        def get(self, request, *args, **kwargs):
            ...
            if request.GET.get(
                "extrato", None
            ) is not None and self.request.user.has_perm(
                "diarios.change_diariooficial"
            ):
                self.template_name = "diarios/diariooficial_extrato.html"
                self.extrato = True

            elif request.GET.get(
                "associar_certidoes", None
            ) is not None and self.request.user.has_perm(
                "diarios.change_diariooficial"
            ):
...
class VinculoDocDiarioOficialCrud(MasterDetailCrud):
    ...
    class ListView(MasterDetailCrud.ListView):
        ...
        def hook_object_id(self, *args, **kwargs):
            ...
            if self.request.user.has_perm(f"{ct.app_label}.change_{ct.model}"):
