...
class DiarioOficial(CommonMixin):
    ...
    tipo = models.ForeignKey(
        TipoDeDiario, on_delete=models.PROTECT, verbose_name=_("Tipo do Diário")
    )
...
class VinculoDocDiarioOficial(CmjCleanMixin, models.Model):
    ...
    diario = models.ForeignKey(
        DiarioOficial, on_delete=models.PROTECT, verbose_name=_("Diário Oficial")
    )

    pagina = models.PositiveIntegerField(default=0, verbose_name=_("Página Inicial"))
    ...
    def reverse_link_content_object(self):
        return reverse(
            f"{self.content_object._meta.app_config.name}:{self.content_object._meta.model_name}_detail",
            kwargs={"pk": self.content_object.id},
        )
