...
from cmj.core.views import (
    AreaTrabalhoCrud,
    BairroCrud,
    BiView,
    CepCrud,
    CertidaoPublicacaoCrud,
    DistritoCrud,
    ImpressoEnderecamentoCrud,
    LogradouroCrud,
    MediaPublicView,
    ...
    chanel_room,
    template_render,
    time_refresh_log_test,
)
from cmj.core.views_auth import (
    CmjLoginView,
    CmjPasswordResetCompleteView,
    CmjPasswordResetConfirmView,
...
user_urlpatterns = [
    ...
    re_path(
        r"^template/(?P<template_name>[^/]+)$", template_render, name="template_render"
    ),
    re_path(
        r"^sistema/auditlog/$",
...
if settings.DEBUG:
    user_urlpatterns += [
        ...
        re_path(r"^channel/(?P<room_name>[^/]+)/$", chanel_room, name="channel_room"),
        re_path(
            r"^time-refresh/$",
            time_refresh_log_test,
...
vue_urlpatterns = [
    re_path(r"^v6/(?P<slug>[^.]*)$", app_vue_view_v6, name="app_vue_view_v6_url"),
    re_path(
...
urlpatterns = (
    ...
    + [
        re_path(
            r"^media/(?P<path>.*)$", MediaPublicView.as_view(), name="mediapublic_view"
        ),
        # url(r'^enderecos/', login_required(
        #    TrechoSearchView.as_view()), name='search_view'),
        re_path(
            r"^areatrabalho",
            include(AreaTrabalhoCrud.get_urls() + OperadorAreaTrabalhoCrud.get_urls()),
        ),
        re_path(r"^cert", include(CertidaoPublicacaoCrud.get_urls())),
        re_path(
            r"^notificacao/(?P<pk>[0-9]+)$",
            NotificacaoRedirectView.as_view(),
            name="notificacao_redirect",
        ),
        re_path(r"^estatisticas$", BiView.as_view(), name="bi_render"),
        re_path(
            r"^api/enderecos.json",
            TrechoJsonSearchView.as_view({"get": "list"}),
            name="trecho_search_rest_json",
        ),
        re_path(
            r"^api/trecho.json/(?P<pk>[0-9]+)$",
            TrechoJsonView.as_view({"get": "retrieve"}),
            name="trecho_rest_json",
        ),
        re_path(r"^sistema/core/cep", include(CepCrud.get_urls())),
        re_path(
            r"^sistema/core/regiaomunicipal", include(RegiaoMunicipalCrud.get_urls())
        ),
        re_path(r"^sistema/core/distrito", include(DistritoCrud.get_urls())),
        re_path(r"^sistema/core/bairro", include(BairroCrud.get_urls())),
        re_path(
            r"^sistema/core/tipologradouro", include(TipoLogradouroCrud.get_urls())
        ),
        ...
        re_path(
            r"^sistema/core/impressoenderecamento",
