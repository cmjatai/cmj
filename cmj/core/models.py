...
class AuditLog(models.Model):
    ...
    email = models.CharField(
        max_length=100, verbose_name=_("email"), blank=True, db_index=True
    )
    ...
    timestamp = models.DateTimeField(
        verbose_name=_("timestamp"), editable=False, auto_now_add=True
    )
    ...
    obj_id = models.PositiveIntegerField(verbose_name=_("object_id"), db_index=True)
...
class Municipio(models.Model):  # Localidade
    ...
    regiao = models.CharField(max_length=2, blank=True, choices=REGIAO_CHOICES)
...
class RegiaoMunicipal(models.Model):
    ...
    nome = models.CharField(max_length=254, verbose_name=_("Região Municipal"))
    tipo = models.CharField(
        max_length=2, default="AU", choices=TIPO_CHOICES, verbose_name="Tipo da Região"
    )
...
class Distrito(models.Model):
    nome = models.CharField(
        max_length=254, verbose_name=_("Nome do Distrito"), unique=True
    )
...
class Bairro(models.Model):
    nome = models.CharField(max_length=254, verbose_name=_("Bairro"), unique=True)
    ...
    outros_nomes = models.TextField(
        blank=True, verbose_name=_("Outros Nomes"), help_text=_("Ocorrências similares")
    )
...
class Logradouro(models.Model):
    nome = models.CharField(max_length=254, verbose_name=_("Logradouro"), unique=True)
...
class Trecho(CmjSearchMixin, CmjModelMixin):
    ...
    # futuros.
    cep = models.ManyToManyField(Cep, related_name="trechos_set", verbose_name=_("Cep"))
    ...
    def __str__(self):
        ...
        rm = self.regiao_municipal.nome + " - " if self.regiao_municipal else ""
...
class ImpressoEnderecamento(models.Model):
    ...
    TIPO_CHOICES = [("ET", _("Folha de Etiquetas")), ("EV", _("Envelopes"))]
    ...
    colunasfolha = models.PositiveSmallIntegerField(verbose_name=_("Colunas"))
    linhasfolha = models.PositiveSmallIntegerField(verbose_name=_("Linhas"))
    ...
    rotate = models.BooleanField(
        default=False, choices=YES_NO_CHOICES, verbose_name=_("Rotacionar Texto")
    )
...
class AreaTrabalho(CmjAuditoriaModelMixin):
    ...
    TIPO_AREATRABALHO_CHOICE = CmjChoices(
        ...
        (TIPO_ADMINISTRATIVO, "tipo_administrativo", _("Setor Administrativo")),
    ...
    nome = models.CharField(
        max_length=100, blank=True, default="", verbose_name=_("Nome")
    )
    ...
    ativo = models.BooleanField(
        _("Área de trabalho Ativa"), choices=YES_NO_CHOICES, default=True
    )
...
class Notificacao(CmjModelMixin):
    ...
    read = models.BooleanField(_("Lida"), choices=YES_NO_CHOICES, default=False)
...
class OcrMyPDF(models.Model):
    ...
    field = models.CharField(default="", max_length=254, verbose_name=_("Field"))
    ...
    concluido = models.DateTimeField(
        verbose_name=_("created"), editable=False, auto_now=True
    )

    sucesso = models.BooleanField(_("Sucesso"), choices=YES_NO_CHOICES, default=True)
...
class Bi(models.Model):
    ...
    ano = models.PositiveSmallIntegerField(verbose_name=_("Ano"), default=0)
...
class CertidaoPublicacao(CmjAuditoriaModelMixin):
    ...
    hash_code = models.TextField(
        verbose_name=_("Hash de Publicação"), max_length=200, blank=True
    )

    field_name = models.CharField(
        verbose_name=_("Field Name"), max_length=200, blank=True
    )

    cancelado = models.BooleanField(
        _("Cancelado "), choices=YES_NO_CHOICES, default=False
    )

    revogado = models.BooleanField(
        _("Certidão Revogada"), choices=YES_NO_CHOICES, default=False
    )
...
class IAQuotaManager(models.Manager):
    ...
    def quotas_with_margin(self, ascending=True):
        ...
        qs = (
            ...
            .annotate(
                total=models.Sum(
                    "iaquotaslog_set__peso", filter=models.Q(iaquotaslog_set__data=hoje)
                )
            )
            .filter(
                models.Q(total__isnull=True)
                | models.Q(total__lt=models.F("quota_diaria"))
            )
        )
        if ascending:
...
class IAQuota(models.Model):
    ...
    quota_diaria = models.PositiveIntegerField(
        verbose_name=_("Quota Diária"), default=0
    )
    ...
    def has_margin(self):
        ...
        hoje = timezone.localtime().date()
        sum_log = (
            self.iaquotaslog_set.filter(data=hoje).aggregate(total=models.Sum("peso"))[
                "total"
            ]
            or 0
        )
    ...
    def create_log(self, peso=1):
        ...
        """
        log = IAQuotasLog(quota=self, peso=peso)
        log.save()
        return log
...
class IAQuotasLog(models.Model):
    ...
    peso = models.PositiveIntegerField(verbose_name=_("Peso"), default=1)
