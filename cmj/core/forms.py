...
class OperadorAreaTrabalhoForm(ModelForm):
    ...
    class Meta:
        ...
        fields = ["user", "grupos_associados"]

    def __init__(self, *args, **kwargs):

        super(OperadorAreaTrabalhoForm, self).__init__(*args, **kwargs)
        self.fields["grupos_associados"].widget = forms.CheckboxSelectMultiple()
        ...
        self.fields["grupos_associados"].queryset = (
            self.fields["grupos_associados"].queryset.all().order_by("name")
        )
...
def get_username():
    try:
        return [
            (u.pk, f"{u.get_full_name()} ({u.email})")
...
class AuditLogFilterSet(django_filters.FilterSet):
    ...
    user = django_filters.ChoiceFilter(choices=get_username(), label=_("Usuário"))
    busca = django_filters.CharFilter(label=_("Filtrar Campo"), method="filter_busca")
    ...
    class Meta:
        model = AuditLog
        fields = ["user", "operation", "model_name", "timestamp", "busca"]
