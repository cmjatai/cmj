...
def sigad_navbar(context, field=None):
    ...
    def get_how_menu(classes):
        ...
        for classe in classes:
            ...
            item["children"] = get_how_menu(
                classe.childs.qs_classes_publicas().filter(**params)
            )
    ...
    def recursive_classes(classe, parent):
        ...
        """if classe.url_redirect and classe.url_redirect.startswith("/"):
            ...
            if classe_redirect:
                docs.extend(
                    list(
                        classe_redirect.documento_set.public_all_docs().filter(pntp=True).values(
        ...
        for classe_referenciada in classe.classes_referenciadas.qs_classes_publicas().order_by(
        ...
        for child in classe.childs.qs_classes_publicas().order_by(order):
