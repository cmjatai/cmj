...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for idx, p in enumerate(doc):
            ...
            for idxi, i in enumerate(images):
                ...
                try:
                    ...
                    # , jpg_quality=50)
                    igray.save(gray_path, output=img["ext"], jpg_quality=__quality)
                    # ,
        ...
        normas = NormaJuridica.objects.filter(
            numero__gte=1228, ano__gte=1987, numero__lte=1399, ano__lte=1990
        ).order_by("numero")
        ...
        materias = MateriaLegislativa.objects.filter(tipo_id=1, ano__gte=2020).order_by(
            "-ano", "-numero"
        )
    ...
    def criar_pdfs__simone(self, *args, **options):
        ...
        os.makedirs(folder_in_images, exist_ok=True)
        os.makedirs(folder_in_images.replace("in_images", "out_images"), exist_ok=True)

        lfimagens = glob.glob(glob.escape(folder_in_images) + "**", recursive=True)
        lfimagens.sort()
        ...
        for index, folders_in in enumerate(arquivos_d_saida):
            ...
            for f in pages_out:
                ...
                os.makedirs("/".join(f_out_image.split("/")[:-1]), exist_ok=True)
                ...
                try:
                    ...
                    def adjust_contrast_brightness(
                        img, contrast: float = 1.0, brightness: int = 0
                    ):
                        """
                    ...
                    c.drawImage(f_out_image, 0, 0, width=595, height=841)
                    c.showPage()
                    # c.save()
                    # return
                except Exception as e:
    ...
    def get_all_tas(self):
        ...
        for norma in NormaJuridica.objects.all().order_by("-norma_de_destaque", "-id"):
    ...
    def clear_html_attrs(self, nd):
        ...
        childs = list(filter(lambda child: isinstance(child, Tag), nd.children))
    ...
    def sanitize_dispositivos_compilacao(self):
        ...
        ds = (
            Dispositivo.objects.filter(ta_publicado_id=9693)
            .exclude(texto__icontains="<table")
            .order_by("ordem")
        )
        ...
        for i, d in enumerate(ds):
            ...
            if d.dispositivo_substituido:
                ...
                txt_alter_sanitize = self.sanitize_html(d.dispositivo_substituido.texto)
