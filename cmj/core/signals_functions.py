...
from cmj.videos.models import PullExec, VideoParte
...
def notificacao_signal_function(sender, instance, **kwargs):
    ...
    if instance.user.be_notified_by_email:
        ...
        send_mail(
            ...
            {"notificacao": instance, "site_url": settings.SITE_URL},
            EMAIL_SEND_USER,
            instance.user.email,
        )

        print(
            "Uma Notificação foi enviada %s - user: %s - user_origin: %s"
            % (instance.pk, instance.user, instance.user_origin)
        )
...
def redesocial_post_function_time_call_documento(inst):
    ...
    vp = VideoParte.objects.filter(content_type=ct, object_id=inst.id).first()
...
def redesocial_post_function(sender, instance, **kwargs):
    ...
    if (
        not hasattr(instance, "_meta")
        or instance._meta.app_config is None
        or not instance._meta.app_config.name in settings.BUSINESS_APPS
    ):
        return
    ...
    for i, r in enumerate(redes, 1):
        ...
        try:
            td = timedelta(
                seconds=running[instance._meta.object_name]["time_call"](instance)
            )
        except:
            return
        ...
        tasks.task_send_rede_social.apply_async(
            (r, instance._meta.app_label, instance._meta.model_name, instance.id),
            eta=now + td,
        )
...
def signed_files_extraction_post_signal_function(sender, instance, **kwargs):
    ...
    params_tasks = (sender._meta.app_label, sender._meta.model_name, instance.pk)

    if not settings.DEBUG or (
        settings.DEBUG and settings.FOLDER_DEBUG_CONTAINER == settings.PROJECT_DIR
    ):
        tasks.signed_files_extraction.apply_async(params_tasks, countdown=3)
    else:
