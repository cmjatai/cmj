...
MEDIA_ROOT = (
    PROJECT_DIR.ancestor(1)
    .child("cmj_media{}".format("_local" if MEDIA_DEV_LOCAL else ""))
    .child("media")
)
...
if DEBUG and "_local" not in str(MEDIA_ROOT) and "/var/cmjatai" not in str(MEDIA_ROOT):
    ...
    MEDIA_PROTECTED_ROOT = (
        Path("/mnt/volumes").child("cmj_media").child("media_protected")
    )
    MEDIA_CACHE_ROOT = Path("/mnt/volumes").child("cmj_media").child("media_cache")
...
THUMBNAIL_SOURCE_GENERATORS = ("cmj.utils.pil_image",)
