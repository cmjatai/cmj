...
class ChatManager:
    ...
    def query_gemini(self, user_message, history):
        ...
        rag_results = {"rag_query": rag_query, "rag_infos": {}}
        try:
            for field in [
                "total_token_count",
                "prompt_token_count",
                ...
                if hasattr(response.usage_metadata, field):
                    rag_results["rag_infos"][field] = getattr(
                        response.usage_metadata, field
                    )
        except Exception:
            logger.warning(
                "Não foi possível obter metadata de token counts da resposta da IA."
            )
    ...
    def process_user_message(self, session_id, user_message, user):
        ...
        msg = self.add_message_to_context(
            session_id,
            "model",
            text_response,
            user,
            metadata={"rag_results": rag_results},
        )
    ...
    def add_message_to_context(self, session_id, role, content, user, metadata=None):
        ...
        msg = ChatMessage.objects.create(
            chat=chat_session, role=role, content=content, metadata=metadata
        )
