...
class UpLoadImportFileForm(forms.Form):
    import_file = forms.FileField(
        required=True, label=_("Arquivo formato ODF para Importanção")
    )

    def __init__(self, *args, **kwargs):
        super(UpLoadImportFileForm, self).__init__(*args, **kwargs)
        self.fields["import_file"].widget.attrs.update({"multiple": "multiple"})
...
class ClasseForm(ModelForm):
    ...
    url_redirect = forms.CharField(
        label=Classe._meta.get_field("url_redirect").verbose_name, required=False
    )

    atricon = forms.CharField(
        label=Classe._meta.get_field("atricon").verbose_name, required=False
    )

    styles = forms.CharField(
        label=_("Estilos CSS Personalizados"),
        widget=forms.Textarea,
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.attrs.update({"class": "form-compact"})
        self.helper.layout = SaplFormLayout(
            Fieldset(_("Identificação Básica"), row1, row2, row3, row4)
        )
        ...
        def get_pc_order_by(nd=None):
            ...
            if not nd:
                pc.append(("", "--------------"))
                childs = Classe.objects.filter(parent__isnull=True)
            else:
    ...
    def save(self, commit=True):
        ...
        if "styles" in data:
            inst.metadata.update({"styles": data["styles"]} if data["styles"] else {})
...
class DocumentoForm(ModelForm):
    ...
    capa = forms.TypedChoiceField(
        label=_("Capa de sua Classe"),
    ...
    classes_relacionadas = forms.ModelMultipleChoiceField(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["classes_relacionadas"].choices = [
            ...
            for c in Classe.objects.filter(
                ...
                template_classe=models.CLASSE_TEMPLATES_CHOICE.subsites,
            )
        ]
...
class CaixaPublicacaoForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if classe:
            ...
            qs = Documento.objects.qs_news().filter(nodes__midia__isnull=False)
        qs = qs.distinct()
                ...
                % (
                    ...
                    (
                        (
                            " - (%s)"
                            % ", ".join(
                                d.caixapublicacao_set.values_list("nome", flat=True)
                            )
                        )
                        if d.caixapublicacao_set.exclude(id=self.instance.pk).exists()
                        else ""
                    ),
                ),
            )
    ...
    def save(self, commit=False):
        ...
        for doc in self.cleaned_data["documentos"]:
            CaixaPublicacaoRelationship.objects.create(
                caixapublicacao=inst, documento=doc
            )
