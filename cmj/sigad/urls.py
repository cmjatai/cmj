...
from cmj.sigad.views import (
    CaixaPublicacaoClasseCrud,
    CaixaPublicacaoCrud,
    PermissionsUserClasseCrud,
...
urlpatterns_sigad = [
    ...
    re_path(r"^classe/create$", views.ClasseCreateView.as_view(), name="classe_create"),
    re_path(
        r"^classe/(?P<pk>[0-9]+)/create$",
        views.ClasseCreateView.as_view(),
        name="subclasse_create",
    ),
    re_path(
        r"^classe/(?P<pk>[0-9]+)/edit$",
        views.ClasseUpdateView.as_view(),
        name="classe_edit",
    ),
    re_path(
        r"^classe/(?P<pk>[0-9]+)/delete",
        views.ClasseDeleteView.as_view(),
        name="classe_delete",
    ),
    re_path(r"^classe$", views.ClasseListView.as_view(), name="classe_list"),
    re_path(
        r"^classe/(?P<pk>[0-9]+)$",
        views.ClasseListView.as_view(),
        name="subclasse_list",
    ),
    re_path(
        r"^classe/(?P<pk>[0-9]+)/documento/create$",
        views.DocumentoCreateView.as_view(),
        name="documento_create",
    ),
    re_path(
        r"^classe/(?P<pk>[0-9]+)/documento/construct$",
        views.DocumentoConstructCreateView.as_view(),
        name="documento_construct_create",
    ),
    re_path(
        r"^documento/(?P<pk>[0-9]+)/construct$",
        views.DocumentoConstructView.as_view(),
        name="documento_construct",
    ),
    re_path(
        r"^documento/(?P<pk>[0-9]+)/edit$",
        views.DocumentoUpdateView.as_view(),
        name="documento_edit",
    ),
    re_path(
        r"^documento/(?P<pk>[0-9]+)/delete$",
    ...
    re_path(
        r"^classe",
        include(
            PermissionsUserClasseCrud.get_urls() + CaixaPublicacaoClasseCrud.get_urls()
        ),
    ),
]
...
urlpatterns = [
    ...
    re_path(r"^$", views.PaginaInicialView.as_view(), name="pagina_inicial_view"),
    re_path(
        r"^parlamentar/?(?P<parlamentar>[^/.]*)/?(?P<slug>[^.]*)\.?(?P<resize>\d+)?(\.midia)?\.?(?P<page>\w+)?$",
        views.PathParlamentarView.as_view(),
        name="path_parlamentar_view",
    ),
    re_path(
        r"^(?P<slug>[^.]*)\.?(?P<resize>\d+)?(\.midia)?\.?(?P<page>\w+)?$",
        views.PathView.as_view(),
        name="path_view",
    ),
]
"""
