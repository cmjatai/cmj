...
from cmj.cerimonial.forms import (
    AnexoProcessoForm,
    ContatoForm,
    ContatoFragmentPronomesForm,
    ContatoFragmentSearchForm,
    EnderecoForm,
    GrupoDeContatosForm,
    ListWithSearchProcessoForm,
    LocalTrabalhoForm,
    LocalTrabalhoPerfilForm,
    ProcessoContatoForm,
    ProcessoForm,
    TipoAutoridadeForm,
    VisitaForm,
...
from cmj.globalrules.crud_custom import PerfilAbstractCrud, PerfilDetailCrudPermission
from sapl.crispy_layout_mixin import CrispyLayoutFormMixin
from sapl.crud.base import Crud, CrudAux, ListWithSearchForm, MasterDetailCrud
...
TipoLocalTrabalhoCrud = CrudAux.build(TipoLocalTrabalho, "tipolocaltrabalho")
StatusProcessoCrud = CrudAux.build(StatusProcesso, "statusprocesso")
ClassificacaoProcessoCrud = CrudAux.build(
    ClassificacaoProcesso, "classificacaoprocesso"
)
TopicoProcessoCrud = CrudAux.build(TopicoProcesso, "topicoprocesso")
...
class VisitaCrud(Crud):
    ...
    class DetailView(Crud.DetailView):
        ...
        def hook_fotografia(self, *args, **kwargs):
            if args[0].fotografia:
                return (
                    "Registro Fotogrático",
                    f'<div class="text-center"><img src="{args[0].fotografia.url}"</div>',
                )
            else:
                return "", ""
    ...
    class ListView(Crud.ListView):
        ...
        def hook_created(self, *args, **kwargs):
            return (
                '<div class="text-center">{}</div>'.format(
                    formats.date_format(
                        timezone.localtime(args[0].created), "d/m/Y - H:i:s"
                    )
                ),
            ...
            try:
                ...
                messages.error(
                    ...
                    (
                        ve.messages[0]
                        if ve.messages
                        else "Erro na validação do formulário de cadastro."
                    ),
                )
...
class PreferencialMixin:
    ...
    def post(self, request, *args, **kwargs):
        response = super(PreferencialMixin, self).post(self, request, *args, **kwargs)
...
class TelefoneCrud(MasterDetailCrud):
    ...
    class BaseMixin(MasterDetailCrud.BaseMixin):
        list_field_names = ["telefone", "tipo", "operadora", "preferencial"]
...
class ContatoFragmentFormPronomesView(FormView):
    ...
    def get_initial(self):
        ...
        try:
            initial["instance"] = TipoAutoridade.objects.get(pk=self.kwargs["pk"])
        except:
            pass
...
class EnderecoPerfilCrud(PerfilDetailCrudPermission):
    ...
    class BaseMixin(PerfilDetailCrudPermission.BaseMixin):
        list_field_names = ["endereco", "numero", "complemento", "bairro", "cep"]
...
class AssuntoProcessoCrud(Crud):
    ...
    class BaseMixin(Crud.BaseMixin):
        ...
        def get_context_data(self, **kwargs):
            context = super().get_context_data(**kwargs)
            context["subnav_template_name"] = "cerimonial/subnav_assuntoprocesso.yaml"
            return context

    class DetailView(Crud.DetailView):
        list_field_names_set = ["data", "titulo", "contatos"]
...
class ContatoFragmentFormSearchView(FormView):
    ...
    def get_initial(self):
        ...
        try:
            ...
            initial["q"] = self.request.GET["q"] if "q" in self.request.GET else ""
...
class ProcessoContatoCrud(MasterDetailCrud):
    ...
    class BaseMixin(MasterDetailCrud.BaseMixin):
        list_field_names = ["data", "titulo", "assuntos", "status", "classificacoes"]
...
class GrupoDeContatosMasterCrud(Crud):
    ...
    class BaseMixin(Crud.BaseMixin):
        ...
        def get_context_data(self, **kwargs):
            context = super().get_context_data(**kwargs)
            context["subnav_template_name"] = "cerimonial/subnav_grupocontato.yaml"
            return context
