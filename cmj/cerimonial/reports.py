...
from cmj.cerimonial.forms import (
    ContatoAgrupadoPorGrupoFilterSet,
    ContatoAgrupadoPorProcessoFilterSet,
    ImpressoEnderecamentoContatoFilterSet,
)
...
class ImpressoEnderecamentoContatoView(PermissionRequiredMixin, FilterView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "print" in request.GET and self.object_list.exists():
            if self.filterset.form.cleaned_data["impresso"]:
                ...
                self.filterset.form._errors["impresso"] = ErrorList(
                    [_("Selecione o tipo de impresso a ser usado!")]
                )
        ...
        if (
            len(request.GET)
            and not len(self.filterset.form.errors)
            and not self.object_list.exists()
        ):
            messages.error(
                request,
                _("Não existe Contatos com as " "condições definidas na busca!"),
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super(ImpressoEnderecamentoContatoView, self).get_context_data(
            **kwargs
        )
        context["count"] = count
    ...
    def build_pdf(self, response):
        ...
        pagesize = (
            float(impresso.largura_pagina) * cm,
    ...
    def createParagraphs(self, contato, stylesheet):
        ...
        imprimir_cargo = (
            (cleaned_data["imprimir_cargo"] == "True")
            if "imprimir_cargo" in cleaned_data and cleaned_data["imprimir_cargo"]
            else False
        )

        local_cargo = (
            cleaned_data["local_cargo"]
            if "local_cargo" in cleaned_data and cleaned_data["local_cargo"]
            else ""
        )
        ...
        if contato.pronome_tratamento:
            if (
                "imprimir_pronome" in cleaned_data
                and cleaned_data["imprimir_pronome"] == "True"
            ):
        ...
        if (
            local_cargo == ImpressoEnderecamentoContatoFilterSet.DEPOIS_PRONOME
            and imprimir_cargo
            and (linha_pronome or contato.cargo)
        ):
        ...
        linha_nome = (
            "%s %s" % (prefixo_nome, contato.nome) if prefixo_nome else contato.nome
        )

        if (
            local_cargo == ImpressoEnderecamentoContatoFilterSet.LINHA_NOME
            and imprimir_cargo
        ):
        ...
        linha_nome = (
            linha_nome.upper()
            if "nome_maiusculo" in cleaned_data
            and cleaned_data["nome_maiusculo"] == "True"
            else linha_nome
        )
        ...
        if (
            local_cargo == ImpressoEnderecamentoContatoFilterSet.DEPOIS_NOME
            and imprimir_cargo
            and contato.cargo
        ):
            story.append(Paragraph(contato.cargo, stylesheet["endereco_style"]))
...
class RelatorioContatoAgrupadoPorGrupoView(PermissionRequiredMixin, FilterView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            ...
            messages.error(
                request, _("Não existe Contato com as " "condições definidas na busca!")
            )
    ...
    def get_context_data(self, **kwargs):
        count = self.object_list.count()
        context = super(RelatorioContatoAgrupadoPorGrupoView, self).get_context_data(
            **kwargs
        )
        context["count"] = count

        context["title"] = _("Relatório de Contatos Com Agrupamento Por Grupos")
        paginator = context["paginator"]
        page_obj = context["page_obj"]
    ...
    def build_pdf(self, response):
        ...
        context = {"titulo": "Relatório de Contatos por Grupo", "contatos": contatos}

        return render_pdf_to_response(self.request, context, template_name)


class RelatorioContatoAgrupadoPorProcessoView(PermissionRequiredMixin, FilterView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            ...
            messages.error(
                request,
                _("Não existe Processo com as " "condições definidas na busca!"),
            )
    ...
    def get_context_data(self, **kwargs):
        count = self.object_list.count()
        context = super(RelatorioContatoAgrupadoPorProcessoView, self).get_context_data(
            **kwargs
        )
        context["count"] = count

        context["title"] = _("Relatório de Contatos Com Agrupamento Por Processos")
        paginator = context["paginator"]
        page_obj = context["page_obj"]
    ...
    def get_data(self):
        ...
        if not agrupamento:
            ...
            self.object_list = (
                self.object_list.order_by(agrupamento)
                .distinct(agrupamento)
        ...
        for p in self.object_list.all():
            ...
            if not p or isinstance(p, str):
                ...
                label_agrupamento = force_str(label_agrupamento[agrupamento])

            if not data:
                tit_relatorio = _("Relatório de Contatos e Processos")
                tit_relatorio = (
                    force_str(tit_relatorio)
                    + " "
                    + (
                        (force_str(_("Agrupados")) + " " + label_agrupamento)
                        if label_agrupamento
                        else (force_str(_("Sem Agrupamento")) + " " + label_agrupamento)
                    )
                )
            ...
            if not p or isinstance(p, str):
                ...
                ).distinct("processo_set__" + agrupamento, "processo_set__data", "nome")
                ...
                item = [
                    ...
                    Paragraph(
                        (
                            ...
                            else p.titulo[:MAX_TITULO] + force_str(_(" (Continua...)"))
                        ),
            ...
            for contato in contatos_query:
                ...
                tels = (
                    "\n".join(map(lambda x: str(x), list(contato.telefone_set.all())))
                    if contato.telefone_set.exists()
                    else ""
                )
                        ...
                        if agrupamento != "titulo":
                            titulo.append(
                                Paragraph(
                                    ...
                                    s if len(ps.titulo) < MAX_TITULO else s_min,
                                )
                            )
                    if not ps:
                        data_abertura.append(Paragraph("-----", s_center))
