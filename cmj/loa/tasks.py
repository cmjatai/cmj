...
def task_register_emendaloa_proposicao_function(*args, **kwargs):
    from cmj.loa.models import EmendaLoa, Loa
    ...
    operadores = OperadorAutor.objects.filter(autor=autor_operado).values_list(
        "user_id", flat=True
    )

    emendas = (
        loa.emendaloa_set.filter(
            fase=EmendaLoa.LIBERACAO_CONTABIL, owner_id__in=operadores
        )
        .order_by("tipo", "id")
        .distinct()
    )
    ...
    for emenda in emendas:
        try:
            if emenda.materia:
                logger.warning(
                    f"EmendaLoa ID: {emenda.id} já está vinculada à Matéria ID: {emenda.materia.id}, pulando registro."
                )
                continue

            if emenda.proposicao and (
                emenda.proposicao.data_envio or emenda.proposicao.data_recebimento
            ):
                logger.warning(
                    f"EmendaLoa ID: {emenda.id} já está vinculada à Proposição ID: {emenda.proposicao.id} que já foi enviada ou aprovada, pulando registro."
                )
                continue
            ...
            emenda.save(update_fields=["metadata"])
            logger.error(
                f"Erro ao registrar proposição para EmendaLoa ID: {emenda.id}: {e}"
            )

    logger.info(
        f"Registering emendaloa proposicao for LOA ID: {loa_id} by User ID: {user_id}"
    )
